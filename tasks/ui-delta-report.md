...
|---|---|---|---|
| **MNU-12** | LOW | Import Emitters dialog has "Select All" but no "Clear" ([ImportEmittersDialog.tsx:240](web/apps/editor/src/screens/ImportEmittersDialog.tsx:240)) | fell between fix-plan P3 and P4; touches one a11y golden |
...
*(VPT-6/7/8 status-bar parity shipped 2026-06-06 — see "Already shipped" above.)*
...
| **SEL-12** | LOW | ✅ SHIPPED | **Drag autoscroll added.** A reorder drag near the tree's top/bottom edge now autoscrolls the viewport (proportional speed, clamps at bounds), with the drop indicator tracking rows under a held pointer ([EmitterTree.tsx:1257](web/apps/editor/src/screens/EmitterTree.tsx:1257) rAF loop + pure [drag-autoscroll.ts](web/apps/editor/src/lib/drag-autoscroll.ts), 7 unit tests; live-verified in-browser). |
| **SEL-13** | LOW | ✅ SHIPPED | **Esc + right-click cancel an in-progress reorder drag.** Listeners attach on drag activation; right-click also suppresses the row context menu (capture-phase `contextmenu`). Pre-active right-click still opens the menu ([EmitterTree.tsx:1289](web/apps/editor/src/screens/EmitterTree.tsx:1289)); 2 unit tests + live menu-suppression check. |
