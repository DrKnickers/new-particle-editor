...
## 2026-06-09 (session 32) — **Multi-drag preview polish (5 items incl. chip-magnetize, user-smoked) + single-drag parity + highlight-follow fix (browser-verified, NOT yet user-smoked) — pushed to PR [#106] (`154514d`, OPEN, NOT merged). Emitter-chain in-game investigation DEFERRED mid-experiment (test file still planted in the user's mod!). Glide animation (stable id + FLIP) BUILT + adversarially reviewed on `claude/reorder-glide` (awaiting user smoke).** NEXT: user-smoke single-drag + glide, then merge #106 (+ stacked glide PR), then resume the chain test.

**Branch state.** `origin/master` = **`3479fe6`** (unchanged this session). **PR [#106]** (branch `claude/multiselect-drag`) advanced `16d1988` → **`154514d`** (two commits this session: `dea7f82` preview polish, `154514d` single-drag parity + follow fix); **OPEN, NOT merged**. Glide work lives on **`claude/reorder-glide`**, stacked on `claude/multiselect-drag` (#106 must merge first; the glide PR follows, or fold it into #106 with user OK).

### What shipped this session (on the #106 branch; item 1 user-smoked in the host, item 2 browser-verified only — user pivoted to the chain question before smoking it)
1. **Preview polish (`dea7f82`)** — subtree dimming (children dim with their roots, both drag paths), dragged≠hidden styling (`opacity-40 saturate-50`), chip cap (4 names + "+k more"), **geometric gap resolver** (drag-activation snapshot + un-shift + midpoint rule; no dead zones, replaced the HOLD workaround and its tall-block stick; gap now true block height, renders by gap index, end-gap after the last subtree), **chip magnetize** (user's idea: `computeChipTarget` blends chip Y 60% toward the gap center + rAF spring `CHIP_SPRING 0.25`; reduced-motion skips the glide). Resolver stability pinned by a vitest **fixed-point/no-cycle property** (caught a real transient on first run).
2. **Single-drag parity + highlight-follow (`154514d`)** — user feedback: gap+chip should apply to single drag too, and the highlight didn't follow a single drop. A single root is now a size-1 block (same gap+chip, commits via `reorder-many` → follow via `newIds`); reparent keeps the onto-ring, resolved geometrically (`resolveSingleRootDrop` + per-row `captureRowGeometry`, one pass → gap | onto). The 2px insertion line is gone; indicator is `{kind:"gap"|"onto"}`. **Root cause of the follow bug: `emitters/drop` returned only `{ok}` and never re-selected** — host now re-selects the moved `Emitter*` (scan for its new index) + emits `emitters/selected`; mock parity. → **L-076** (positional-id mutations must re-select; stable-id mocks hide the bug).
...
### Verified baseline at `154514d` (#106 branch)
web **620/620**, `tsc -b` 0, vite build clean, native **174/0**, host Debug x64 clean (VS18). Browser-preview drove multi drag, single reorder (gap+chip+follow), reparent (ring+follow) — zero console errors. This worktree (`elastic-rosalind-5cd21d`) is fully restored (L-039/L-040 done; exe + dist built).
...
- **Experiment design (chain test, not yet a numbered lesson):** when testing a *depth* question on a per-particle-spawning system, control the combinatorics — chain the lowest-count emitter and keep depth minimal, or a crash proves nothing. Also: our editor accepts a `.alo` CLI arg — load-check planted test files in the editor before costing the user a game launch.
- **Radix long-press gotcha** (in CHANGELOG): synthetic `pointerdown` with default `pointerType: ""` held over a ContextMenu.Trigger opens the menu via the touch path — drive synthetic drags with `pointerType: "mouse"`.

### Kickoff (full) — paste into a fresh session
> Pick up `new-particle-editor` (AloParticleEditor — Win32 host + WebView2/React + D3D9Ex-via-DComp particle editor for SW:EaW). **`master` is the trunk**; `origin/master` = `3479fe6`. **PR [#106]** (multi-select drag-reorder + preview polish + single-drag parity, branch `claude/multiselect-drag`, HEAD **`154514d`**) is **OPEN, NOT merged**. Glide-animation work is **on `claude/reorder-glide`** (stacked on #106) — check `git log` there for how far it got. Read `tasks/HANDOFF.md` top (session 32), `tasks/todo.md` (Part 3 = glide plan), `tasks/next-reorder-glide-animation.md`, `tasks/next-emitter-chain-investigation.md` (**deferred mid-experiment — a patched test file is still planted in the user's EmpireAtWarExpanded mod, see its Live state section**), and `tasks/lessons.md` (L-076 positional-id re-select; L-022/L-033/L-046/L-039/L-040/L-068/L-070/L-072). VERIFY claims against code. Pre-flight: `git fetch origin master`; check out the working branch; from `web/` `pnpm install` if needed, `pnpm --filter @particle-editor/editor test` → **620** at `154514d` (more if glide tests landed), `tsc -b` 0; host Debug x64 (VS18); native **174/0** (fresh worktree needs L-039 NuGet copy + L-040 `pnpm build`). **NEXT — confirm with the user:** (1) continue/finish the **glide** (stable id + FLIP per todo.md Part 3; user smoke per L-033); (2) **merge #106** with explicit OK (reconcile `git merge origin/master` first — CHANGELOG both-prepend, mock.ts isMutating keep all three -many kinds, .gitignore dedup), then the glide as a stacked PR; (3) resume the **chain investigation** (user fires the planted v2 test; decision matrix in the note); (4) CHANGELOG TODO backfills (#107 → `3479fe6`; #106 + glide after merge). Summarize understanding + confirm scope before changing anything.

### Kickoff (short)
> Pick up AloParticleEditor. `origin/master` = `3479fe6`. PR [#106] (`claude/multiselect-drag` @ `154514d`, OPEN): multi-drag + polish + single-drag parity + follow fix — user-smoked. Glide (stable id + FLIP) in progress on `claude/reorder-glide` (stacked). Chain investigation DEFERRED — **patched test file still planted in the user's mod** (`tasks/next-emitter-chain-investigation.md`, Live state). Read HANDOFF top (s32) + todo.md Part 3 + lessons (L-076). Pre-flight: web tests (620 @ `154514d`), tsc 0, native 174/0. NEXT (confirm): finish glide / merge #106 / resume chain test / backfills.
