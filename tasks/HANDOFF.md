...
## 2026-06-09 (session 32) — **SHIPPED the whole emitter-tree drag feature to `master` via PR [#108] (`71b5c41`): multi-drag preview polish + single-drag parity + highlight-follow + reorder GLIDE animation + chip spawn/despawn. PR [#106] superseded (GitHub auto-marked it MERGED — #108 is a superset). Two adversarial review passes hardened it; two investigations DEFERRED (emitter-chain in-game test still planted in the user's mod; drag audit rate-limited mid-run).** NEXT: user host-smoke (optional — it's on master); resume the chain test + drag audit; the `post-108-backfill` PR (CHANGELOG TODO backfills + this handoff) awaits OK.
...
3. **Reorder GLIDE animation (`49dc692`+, the glide branch)** — rows glide to new positions on every reorder (drag-drop, Move Up/Down, delete, paste) **and during a drag** as the gap moves (`FLIP_DRAG_MS` 120 ms / `FLIP_SETTLE_MS` 200 ms; interrupted glides fold the in-flight transform residual). Root cause fixed first: runtime-only **`stableId`** on `Emitter` (all 3 ctors + preserved through `copySharedParamsFrom`), surfaced on the DTO/schema/mock; React keys rows by it + a FLIP `offsetTop` pass. **Chip spawn pop-in + despawn flight into the landing gap / reparent row** (`CHIP_EXIT_MS`). `prefers-reduced-motion` disables all motion.
