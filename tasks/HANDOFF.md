...
| **HEAD (committed)** | `TODO-HASH` (this P6 docs commit) at the top of the session branch. Top non-docs commit is `ec486d9` (`feat(LT-4): inspector layout follow-ups — always-mounted tab strip + flex split (B1.3.1)`). Plan commit `24f3f8b` sits between them. |
| **Working tree** | clean. |
| **Ahead of origin/lt-4** | 3 (plan + impl + docs) — pending FF + push to `origin/lt-4` with the user's OK. Pre-FF `origin/lt-4` HEAD is `f12d6f2`. |
...
- Default sizes for the inner left-column split: B1.3.1 ships 50/50, but the user may want a different default once they've lived with the new layout for a few minutes. Confirm during brainstorm.
...
- **Items 2 + 3 — Tree + tabs flex on the panel-body axis.** `App.tsx` swaps `h-72 shrink-0` for `flex-1 min-h-0` on the lower-left slot, matching the sibling EmitterTree aside. Both children now share the column's vertical extent on a 50/50 default split. B1.4 will make the boundary draggable.
