# Session Handoff — AloParticleEditor / LT-4 (B1.3.1 inspector layout follow-ups + 9 polish rounds — ends with frosted-glass modal-backdrop plan teed up for next session)

**Last updated:** 2026-05-21 (end-of-long-session — B1.3.1 [NT-7] inspector layout follow-ups shipped + 9 subsequent polish rounds covering file-open behaviour, occlusion bugs, modal compositing, BridgeContext, and an interim modal-mask compositor pipeline that has a known inner-shadow artifact. The next session implements **B1.3.1.1 — frosted-glass modal backdrop via engine-snapshot capture** per the full plan in [`tasks/todo.md`](todo.md), which replaces the modal-mask code.)
**Last conversation context:** What started as a B1.3.1 dispatch (3 layout follow-ups) accreted 9 polish rounds across a long debugging arc. Sequence:
- **Rounds 1-2** (`c3b76b2`, `e655b78`): 25/75 split ratio, inspector right-padding, toolbar File wiring, tree toolbar pinning.
- **Round 3** (`b1d9266`): emit `emitters/tree/changed` + `ReloadTextures` on `file/open` / `file/new` — without these the React tree stayed on the prior file after opening a new one, and particles rendered as the missing-texture white-fallback placeholder.
- **Round 4** (`cf643e3`): register ViewportPill + Recents submenu with AlphaCompositor — both were rendering behind the engine viewport popup because their CSS shadows extended past the occlusion pad, AND the submenu was using `Menubar.SubContent` without an `OccludingMenubarSubContent` wrapper.
- **Round 5** (`a1a3c42`): Shift+LMB → spawn cursor-bound instance. WebView2 holds focus from React UI clicks so the legacy WM_KEYDOWN(VK_SHIFT) path was silently swallowed; clicking in the viewport with Shift held now triggers the same SpawnParticleSystem call.
- **Round 6** (`7c45a7b`): Modal overlay registers with AlphaCompositor. The crucial debugging step here used a diagnostic-logs round (effect-fires + ref-state + send-fires + cleanup) to pin down a layered failure mode: `window.bridge` was getting swapped to `TestHostBridge` by `exposeBridgeForTests` whenever `chrome.webview.hostObjects.hostBridge` was truthy — which WebView2 returns a proxy for even without `--test-host`, so calls rejected with HRESULT 0x80070490. AND Radix Dialog.Content's Portal+Presence delays ref attachment past the parent's useEffect.
- **Rounds 7-9** (`52bb032`): Polish round 7 swaps to opaque chrome where HTML effects can't reach the engine — `.vp-tools` drops `backdrop-filter: blur(8px)` for `var(--panel)` solid bg (the blur was painting a near-solid dark smudge because WebView2 has nothing useful behind the pill); Modal swaps `shadow-2xl` for `shadow-md` (shadow-2xl extended past the 8 px occlusion pad and drew a hard halo); regression-guard vitest tests lock the policy. Polish round 8 introduces BridgeContext (replaces the broken `window.bridge`) and the Modal callback-ref + useState pattern that handles Radix Portal's delayed ref attachment. Polish round 9 attempts to dim+blur the engine viewport via a server-side modal-mask compositor pipeline so the engine looks like the panels under Dialog.Overlay's `bg-black/60 backdrop-blur-sm`. The dim and blur work, but the popup-edge-feather attempt produces a visible inner-shadow vignette (pixel math: center luminance ~60 from `engine*0.4 + panel*0.24`; mid-fade luminance ~35 because dst dominates as alpha fades; edge ~10 = pure panel*0.4). Structurally unfixable by tuning — the cause is CSS effects can't span the engine compositing layer, so any popup-edge fade reveals the dim underneath instead of bridging gradients. **The fix queued for the next session is to capture the engine as a PNG snapshot, render it as an `<img>` inside the WebView2 DOM, and let Dialog.Overlay's `backdrop-blur-sm` blur engine+panels uniformly** (full plan in todo.md).

**Test counts:** vitest **281 / 281** (was 277; +4 regression guards for opaque pill bg + no-backdrop-filter + no large-shadow on Modal, +1 set-modal-mask dispatch test that's removed in B1.3.1.1's Phase 3). Playwright **83 / 83**. MSBuild Debug x64 clean. The current state on the session branch has the **modal-mask inner-shadow artifact visible** when the modal opens — known + documented + planned-for-replacement.

**Next dispatch:** **B1.3.1.1 [NT-9] — Frosted-glass modal backdrop via engine-snapshot capture**. The full plan is in [`tasks/todo.md`](todo.md). High-level: new `viewport/capture-snapshot` bridge surface returns the current engine DIB as a base64 PNG; React Modal renders it as an `<img>` portaled into the viewport-quadrant DOM; engine popup is fully alpha-cut while modal is open; CSS `backdrop-filter` then blurs everything uniformly. Phase 3 of the new dispatch deletes the modal-mask path that landed in this session as interim work. **Other queued dispatches** (B1.4 splitters, B2 audit, MT-1 textures, NT-5/NT-6 link-group cleanups) remain queued behind B1.3.1.1.
...
| Thing | Value |
|---|---|
| **Worktree** | `C:\Modding\Particle Editor\.claude\worktrees\agitated-margulis-854108` (this session's; next session gets a fresh `claude/<random>` path) |
| **Branch** | `claude/agitated-margulis-854108` → integrates back into `lt-4` per the standard end-of-session FF. Tracks `origin/lt-4`. |
| **HEAD (committed)** | `52bb032` (polish rounds 7-9 commit) + a docs commit on top of it. The session branch has 10 implementation/polish commits + 2 docs commits ahead of origin/lt-4. Plan commit `24f3f8b` is the oldest in the sequence. |
| **Working tree** | clean. |
| **Ahead of origin/lt-4** | 11+ (plan + impl + P6 docs + polish rounds 1-9 + this docs sync) — pending FF + push to `origin/lt-4` with the user's OK. **NOT YET FF'd.** The current state has the modal-mask inner-shadow artifact visible; the next session's B1.3.1.1 dispatch (Phase 3) removes the modal-mask path entirely. Decide whether to FF this interim state to `origin/lt-4` OR let B1.3.1.1 complete first and FF the whole arc together. Pre-FF `origin/lt-4` HEAD is `f12d6f2`. |
| **Behind master** | `lt-4` is ~365+ commits ahead of `master` (`b28f624`); none merged yet, all backed up to `origin/lt-4`. |
| **Open PRs** | none |
| **Build status** | MSBuild Debug x64 clean (preexisting LIBCMTD warning). C++ touched: AlphaCompositor + LayoutBroker + BridgeDispatcher (modal-mask compositor pipeline). Vitest **281 / 281** (+4 polish regression guards). Playwright **83 / 83**. |
| **Phase status** | Particle Editor 2026 redesign — **Phase 1 + Phase 2 + curve editor polish + B1 + B1.2 + B1.2.1 + B1.3 + B1.3.1 inspector layout follow-ups + 9 polish rounds shipped on the session branch (not yet FF'd to `origin/lt-4`).** Next dispatch is **B1.3.1.1 [NT-9] — frosted-glass modal backdrop via engine-snapshot capture** (see todo.md). Queued behind: **B1.4 [NT-8]** (resizable splitters), **B2 obsolescence audit**, **MT-1 follow-up** (texture-picker "…" buttons). Phase 3 of the 2026 redesign (dialog re-skin, Tailwind cleanup, theme-persistence test) remains not started. Legacy `--legacy-ui` mode is untouched throughout. |
...
## What landed this session — B1.3.1 + 9 polish rounds (12 commits + this docs commit)

In execution order (oldest → newest):

| Commit | What |
|---|---|
| `24f3f8b` | **docs(LT-4): plan — B1.3.1 inspector layout follow-ups.** Three smoke-test findings deferred from B1.3. Two architectural decisions confirmed via question-chip (50/50 split, free tab clicking). |
| `ec486d9` | **feat(LT-4): B1.3.1 — always-mounted tab strip + flex split.** Bundled P3+P4 because each alone is half-baked. `Tabs.Root` + `Tabs.List` mount unconditionally; `renderBody((p) => …)` helper in each Tabs.Content swaps placeholder / Loading / form. App.tsx slot `h-72 shrink-0` → `flex-1 min-h-0`. Two vitest waitFor anchors retuned. |
| `f1d1f27` | **docs(LT-4): CHANGELOG + HANDOFF + ROADMAP for B1.3.1** (P6 docs). |
| `c3b76b2` | **fix(LT-4): polish 1 — bump inspector split 50/50 → 25/75 favouring tabs + docs sync.** Post-smoke-test polish; tabs slot `flex-1` → `flex-[3_1_0%]`. |
| `e655b78` | **fix(LT-4): polish 2 — inspector right-padding + toolbar File wiring + tree toolbar pinning.** Three smoke-test findings: `.section-body { padding-right: 12px }` so form rows have breathing room from the scrollbar (Name + section headers unaffected); Toolbar's File buttons wired through `promptSaveChanges` + bridge `file/new`/`file/open`/`file/save`/`file/save-as` (were scaffolded to a `todoFile()` console.log); EmitterTree toolbar pinned to the pane bottom via `overflow-hidden flex flex-col` on the aside + `flex-1 min-h-0 overflow-y-auto` on the inner list container. |
| `b1d9266` | **fix(LT-4): polish 3 — emit `emitters/tree/changed` + `ReloadTextures` on `file/new` / `file/open` (C++).** Without the tree-changed emit the React tree stayed on the prior file. Without ReloadTextures, particles rendered as the missing-texture white fallback because the engine's lazy texture binding via `ParticleEmitterInstance::onParticleSystemChanged` doesn't fire for instances that exist before the swap. Calling ReloadTextures explicitly mirrors what View → Reload Textures does. |
| `cf643e3` | **fix(LT-4): polish 4 — register ViewportPill + Recents submenu with AlphaCompositor.** ViewportPill never called `useViewportOcclusion`; engine viewport painted on top, only drop-shadow leaked through. Added new `OccludingMenubarSubContent` wrapper (analogous to `OccludingMenubarContent`) and routed the File → Recent Files submenu through it. |
| `a1a3c42` | **fix(LT-4): polish 5 — Shift+LMB in viewport spawns cursor-bound instance.** Legacy keydown-only path failed because WebView2 holds focus from React UI clicks; WM_KEYDOWN(VK_SHIFT) went to WebView2 not the viewport HWND. New WM_LBUTTONDOWN branch checks MK_SHIFT and spawns directly, skipping the camera drag. Existing WM_KEYUP cleanup still kills the instance on shift release. |
| `7c45a7b` | **fix(LT-4): polish 6 — Modal overlay registers with AlphaCompositor.** Caught a layered failure mode via a diagnostic-logs round: `window.bridge` was getting swapped to `TestHostBridge` (broken when no `--test-host`) AND Radix Dialog.Content's Portal+Presence delays ref attachment past the parent's useEffect. Initial attempts (Dialog.Overlay ref, asChild wrapper, hardcoded full-window rect, requestAnimationFrame deferral) all failed for different reasons until the diagnostic round confirmed `hasBridge=true; hasEl=false` at open time. |
| `52bb032` | **feat(LT-4): polish rounds 7-9 — opaque chrome + BridgeContext + modal-mask compositor (interim).** Three layered changes. Round 7: `.vp-tools` drops backdrop-filter for solid `var(--panel)`, Modal swaps `shadow-2xl` for `shadow-md`, regression-guard vitest tests lock the policy. Round 8: new `lib/bridge-context.ts` carries the live NativeBridge through React Context (replacing the broken `window.bridge`); Modal uses callback ref + useState pattern to handle Radix's delayed ref attachment. Round 9: server-side modal-mask compositor pipeline (`AlphaCompositor::SetModalMask` + box-blur + alpha multiply + `FadePopupEdges`); produces a known inner-shadow artifact at the popup boundary that **the next session's B1.3.1.1 dispatch replaces** with a snapshot-into-DOM approach. |
| `TODO-HASH` | **docs(LT-4): handoff — frosted-glass snapshot plan teed up for next session.** This docs commit. Drafts the full B1.3.1.1 plan in `tasks/todo.md`; refreshes HANDOFF + CHANGELOG + ROADMAP; adds lessons L-011 (HTML CSS effects can't reach engine compositing layer) + L-012 (`window.bridge` may be TestHostBridge proxy — use BridgeContext). |

Plus this docs commit (handoff).

---

## Previously landed (kept for context)

B1.3 (the immediately preceding dispatch, shipped at `f12d6f2`) restructured the three property tabs to legacy `IDD_EMITTER_PROPS1/2/3` shape — three Sections per tab matching legacy GROUPBOX structure section-for-section, twelve field placements migrated to legacy homes, tri-state Generation radio mutex with full a11y plumbing (`role="radiogroup"` + roving tabIndex + arrow-key cycling), a bundled `displayInvertedPercent` correctness fix on `FieldSpinner` (the new UI was reading `randomLifetimePerc=0.25` as `0.25%` instead of legacy's `75%` minimum), trailing-colon label convention applied to every field, "World Oriented" → "Always face camera" with semantic flip, four fields dropped from UI but retained on the wire (`nTriangles`, `weatherFadeoutDistance`, `groups[1]` Lifetime random-param, `index`). Two two-stage-review fix commits caught real issues — `b929e47` (a11y RadioRow extraction) and `3b191fd` (weather-disable cascade parity per legacy `src/UI/Emitter.cpp:175-190`). Two polish rounds folded user smoke-test findings — `3ae940e` (dark scrollbar inside Tabs.Content + form-row template tuning) and `82917f0` (per-axis X/Y/Z micro-labels above every Vec3 cluster + texture-input widening + SpawnerPanel scroll fix). 14 commits total, FF'd to `origin/lt-4` at the end of that session. **Lessons.md L-010** filed: inspector field labels are public API; sweep BOTH vitest and Playwright on every rename.
...
### 0. B1.3.1.1 [NT-9] — Frosted-glass modal backdrop via engine-snapshot capture (NEXT DISPATCH — full plan in [`tasks/todo.md`](todo.md))

**Goal.** Replace the interim modal-mask compositor pipeline (which produces a visible inner-shadow artifact at the popup boundary) with a captured engine-snapshot approach. New `viewport/capture-snapshot` bridge surface returns the current engine DIB as a base64 PNG; React Modal renders it as an `<img>` portaled into the viewport-quadrant DOM; engine popup is fully alpha-cut while modal is open; CSS `backdrop-filter` then blurs panels + snapshot uniformly. Phase 3 deletes the modal-mask C++ machinery that this session landed as interim work.

**Why this is the right approach (don't re-debate — investigated thoroughly in this session):**
- HTML CSS effects (`backdrop-filter`, `box-shadow` of any large extent) can't sample engine viewport pixels — engine is a separate compositing layer (FD9b layered Win32 popup), not a DOM element.
- Server-side dim+blur of the engine (the interim modal-mask path) works for the engine pixels themselves, but the popup HWND boundary against the CSS-dimmed panels still draws a visible rectangle. Any attempt to feather the popup-edge alpha reveals the underlying Dialog.Overlay `bg-black/60` which is darker than the dim engine, producing an inner-shadow vignette.
- The snapshot-into-DOM approach lifts engine pixels INTO the WebView2 DOM tree (frozen at one frame), so CSS effects sample them natively. No layer boundary visible. No luminance algebra needed.

**Estimated commits:** 3 (capture surface + bridge → Modal rewiring → Phase 3 cleanup of modal-mask). Plus docs.
