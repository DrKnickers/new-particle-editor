# Session Handoff — AloParticleEditor / LT-4 (B1.3.1 inspector layout follow-ups: always-mounted tab strip + flex split)

**Last updated:** 2026-05-21 (end-of-session — B1.3.1 [NT-7] inspector layout follow-ups shipped: Tabs.Root + Tabs.List always mount, with a body-level placeholder inside the active tab when no emitter is selected; the lower-left tabs slot trades its fixed 288 px height for a flex axis with the EmitterTree above it, on a **25/75 split favouring the tabs** after a post-smoke-test polish round bumped the default from the originally-planned 50/50. Plan + bundled implementation + docs + polish-round commit.)
**Last conversation context:** Short, well-scoped dispatch executing the B1.3.1 plan that user-deferred from B1.3's smoke test. Started on a fresh `claude/agitated-margulis-854108` worktree branched cleanly from `origin/lt-4` at `f12d6f2` (the prior session's post-FF docs-sync). Plan landed at `24f3f8b` (the new `tasks/todo.md` covering items 1–3 + risks + testing). Implementation landed in a single bundled commit `ec486d9` (P3 always-mounted strip via a `renderBody` helper + P4 `h-72 shrink-0 → flex-1 min-h-0` slot swap), with two existing waitFor patterns retuned because the strip's mount semantics changed (specs that proxied "form is loaded" through `getByTestId("emitter-property-tabs")` now anchor on `getByLabelText("Maximum lifetime:")`). One `JSX.Element` namespace lookup failure during build prompted a switch to `ReactNode` imported from `react` — L-004 reminder: vitest doesn't type-check; truth is `pnpm build`. Two architectural decisions confirmed by the user up-front via the question chip (initially 50/50 even split over biased ratios; free tab clicking over disabled-when-no-selection). After the user smoke-tested the build, a polish round bumped the default split from 50/50 to 25/75 favouring the tabs (`flex-[3_1_0%]` on the tabs slot vs `flex-1` on the tree aside) — same brief the deferred-item list described ("tab strip dominates the visual hierarchy").

**Test counts:** vitest **277 / 277** (no count change — one spec body replaced + two waitFor anchors updated), Playwright **83 / 83**, MSBuild Debug x64 clean. NuGet restore needed on this worktree (HANDOFF's "fresh-worktree pre-flight" item — caught + applied per the prescribed `MSBuild //t:Restore //v:m` step).

**Next dispatch:** **B1.4 [NT-8]** — resizable splitters via `react-resizable-panels`. Now that the default 50/50 split is in place, the natural next step is making the boundary draggable so the user can bias it either direction. Persistence to `localStorage` like the theme toggle; defaults match the new sizing. **B2 obsolescence audit** is the small warm-up alternative — diff current Appearance + Physics implementations against B2's original target spec; B1.3 likely retired most of B2 already. **MT-1 follow-up** (texture-picker `…` buttons) remains queued.
...
| **HEAD (committed)** | `TODO-HASH` (this docs+polish refresh) at the top of the session branch. Non-docs commits: `ec486d9` (always-mounted strip + flex split) and the polish-round bump from 50/50 to 25/75 favouring tabs (folded into this same docs commit). Plan commit `24f3f8b` sits below. |
| **Working tree** | clean. |
| **Ahead of origin/lt-4** | 4 (plan + impl + docs + polish-docs refresh) — pending FF + push to `origin/lt-4` with the user's OK. Pre-FF `origin/lt-4` HEAD is `f12d6f2`. |
| **Behind master** | `lt-4` is ~365+ commits ahead of `master` (`b28f624`); none merged yet, all backed up to `origin/lt-4`. |
| **Open PRs** | none |
| **Build status** | MSBuild Debug x64 clean (preexisting LIBCMTD warning; no C++ change this dispatch — last C++-touching commit was earlier on `lt-4`). Vitest **277 / 277**. Playwright **83 / 83**. |
| **Phase status** | Particle Editor 2026 redesign — **Phase 1 + Phase 2 + curve editor polish + B1 left-pane realignment + B1.2 left-pane polish + B1.2.1 label-truncation polish + B1.3 tab parity reorg + B1.3.1 inspector layout follow-ups shipped. Phase 3 not started.** Next dispatch is **B1.4 [NT-8] (resizable splitters via `react-resizable-panels`)**. Warm-up alternative: **B2 obsolescence audit** — quick diff of Appearance + Physics implementations against B2's original target spec; B1.3 likely retired most of B2 already. After those: **MT-1 follow-up** (texture-picker "…" buttons). Legacy `--legacy-ui` mode is untouched throughout. |
...
## What landed this session — B1.3.1 inspector layout follow-ups (2 commits + this P6 docs commit)

In execution order (oldest → newest):

| Commit | What |
|---|---|
| `24f3f8b` | **docs(LT-4): implementation plan — inspector layout follow-ups (B1.3.1)** — full plan in [`tasks/todo.md`](todo.md). Three smoke-test findings the user deferred from B1.3: (1) tab strip always visible with body-level placeholder when no emitter is selected, (2) tabs slot flexes alongside the EmitterTree on the left column axis at a default 50/50 split, (3) tree breathes into the space the fixed `h-72` used to claim. Two architectural decisions confirmed up-front via question-chip: 50/50 even split over biased ratios; free tab clicking over disabled-when-no-selection. |
| `ec486d9` | **feat(LT-4): inspector layout follow-ups — always-mounted tab strip + flex split (B1.3.1)** — bundled P3 + P4 because each makes coherent sense only together (P3 alone leaves the placeholder centered in a fixed 288 px slice; P4 alone leaves the strip vanishing on no-selection). [`EmitterPropertyTabs.tsx`](../web/apps/editor/src/screens/EmitterPropertyTabs.tsx): `Tabs.Root` + `Tabs.List` mount unconditionally; new `renderBody((p) => …)` helper inside each `Tabs.Content` swaps between placeholder / Loading / form, with the callback signature `(p: EmitterPropertiesDto) => ReactNode` so type narrowing flows through cleanly. Placeholder testid + copy preserved verbatim per L-010. [`App.tsx`](../web/apps/editor/src/App.tsx): lower-left slot `h-72 shrink-0` → `flex-1 min-h-0` so both children of `panel-body` share the column's vertical extent. Two existing vitest waitFor anchors updated from `getByTestId("emitter-property-tabs")` to `getByLabelText("Maximum lifetime:")` because the old anchor used to imply "form is loaded" (strip only mounted with data); now it only implies "strip exists" so the wait would pass too early. |
| `f1d1f27` | **docs(LT-4): CHANGELOG + HANDOFF + ROADMAP for B1.3.1** (P6) — three-section CHANGELOG entry following the convention; HANDOFF refresh (header + resumable-state + what-landed-this-session + open-items §0 swap from B1.3.1 to B1.4 + §1 marked shipped); ROADMAP strike 1.1 [NT-7] + move to 5.1 with 21-entry renumber. |
| `TODO-HASH` | **fix(LT-4): bump inspector split from 50/50 to 25/75 favouring tabs + docs sync (B1.3.1 polish)** — post-smoke-test polish round after the user tested the build and asked for the tabs to dominate. Tabs slot `flex-1 min-h-0` → `flex-[3_1_0%] min-h-0`; tree aside stays at `flex-1`. Same brief the original deferred-item list described ("tab strip dominates the visual hierarchy"). No test impact — vitest waitFor anchors don't care about pixel proportions; Playwright assertions are all on selection-state visibility, not size. Docs (CHANGELOG / HANDOFF / ROADMAP) updated in the same commit to reflect the shipped ratio. |

Plus this P6 docs commit (CHANGELOG + HANDOFF + ROADMAP + todo.md Review).
...
### 0. B1.4 [NT-8] — Resizable splitters via `react-resizable-panels` (NEXT DISPATCH — start in new session)

Make the left / centre / right column boundaries draggable so users can size panes to taste. B1 finished the structural realignment, B1.2 tightened the interior fidelity, B1.3 aligned tab structure with legacy, B1.3.1 just put the lower-left tabs slot on a flex axis with the EmitterTree above it — now B1.4 makes the four boundaries (left-column right edge, centre-column right edge, viewport / curve-editor horizontal split inside the centre column, and the left-column tree / tabs horizontal split inside the panel-body) draggable.

Target library is [`react-resizable-panels`](https://github.com/bvaughn/react-resizable-panels) (battle-tested, accessible drag handles, persistence hooks). Standard CLAUDE.md plan structure expected. Persistence target is `localStorage` like the theme toggle; default sizes match the current fixed widths and the 25/75 inner left-column split B1.3.1 ships. No bridge schema, no C++.

**Open architectural questions for the brainstorm:**
- One `PanelGroup` per axis, or nested? The horizontal split inside the left column (`tree | tabs`) sits inside the vertical split (`left | viewport+curve | spawner`) — react-resizable-panels supports nesting, but the persistence keys need careful naming to avoid clobbering.
- Default sizes for the inner left-column split: B1.3.1 ships 25/75 favouring tabs after one polish round. Confirm during brainstorm whether to anchor that as the persisted default or let the user re-bias on first session.
- Handle visibility: hairline by default, thicker on hover. Match the theme tokens.
...
### 1. ~~B1.3.1 inspector layout follow-ups~~ ✅ SHIPPED this session (3 commits ready to FF at user's OK)

Single-commit implementation `ec486d9` plus the plan commit `24f3f8b` and this docs commit. Three findings deferred from B1.3's smoke test, all addressed by two coupled changes:

- **Item 1 — Tabs always visible.** `EmitterPropertyTabs.tsx` lifts `Tabs.Root` + `Tabs.List` out of the early-return; a new `renderBody((p) => …)` helper inside each `Tabs.Content` swaps between placeholder / Loading / form. Only the active tab's body shows the placeholder, so the user sees Basic / Appearance / Physics immediately and can pre-pick a tab.
- **Items 2 + 3 — Tree + tabs flex on the panel-body axis.** `App.tsx` swaps `h-72 shrink-0` for `flex-[3_1_0%] min-h-0` on the lower-left slot. Tree aside (`flex-1`) and tabs slot (`flex-[3_1_0%]`) sit on the same flex axis with a 1:3 ratio — 25/75 default favouring the tabs after the post-smoke-test polish round bumped it from the originally-planned 50/50. B1.4 will make the boundary draggable.

**Spec updates.** Single structural flip on the "no-selection placeholder" vitest spec (now asserts the strip is mounted with the placeholder inside + all three triggers are clickable). Two `waitFor(getByTestId("emitter-property-tabs"))` anchors retuned to `waitFor(getByLabelText("Maximum lifetime:"))` because the old anchor used to imply "form loaded" (strip only mounted with data); now it only implies "strip exists" so the wait passes too early.

**Status:** 3 commits ready to FF into `origin/lt-4` at user's explicit OK.
