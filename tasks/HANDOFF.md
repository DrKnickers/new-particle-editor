...
## 2026-06-11 (session 36) — **TWO features shipped to `master` via PR [#123] (`bd52588`): the NT-12 styled/animated tooltip pass + modal/banner motion family, AND a configurable preview overload guard (Preferences toggle + tunable cap, default 15,000). Both feel-tested by the user and merged. CHANGELOG hash backfill shipped as [#124] (`57cb93e`). Both built subagent-driven (NT-12 = 13 tasks, guard = 7 tasks), every task spec- + quality-reviewed. NO task in flight; ROADMAP §1 (Near term) is now EMPTY (NT-12 was its only item).**

**`origin/master` = `57cb93e`** (advanced this session: #123 → `bd52588`, #124 → `57cb93e`). Nothing open at handoff. Working branches `claude/tender-satoshi-5ff472` (#123) + `claude/changelog-backfill-123` (#124) both fully merged; this entry is on `claude/session-36-handoff` (awaits OK).

### What shipped — both on `master` via #123, both CI-green, both user-feel-tested

1. **[NT-12] Styled/animated tooltips app-wide + one motion family.** One `Tip` primitive ([`primitives/Tip.tsx`](../web/apps/editor/src/primitives/Tip.tsx)) on **`@radix-ui/react-tooltip`** (8th Radix dep): theme-following panel surface, soft `--shadow-soft` drop shadow (per-theme alpha), fade + 4px directional slip, 400 ms first-open delay with 300 ms skip-delay sweep, **opt-in viewport occlusion** (`occlusionId` prop → `useViewportOcclusion`; tooltips are portaled DOM and the D3D viewport popup overpaints them otherwise — the `OccludingPopover` hazard). The ~45-site **title→Tip sweep** used six site classes (T1 keep aria-label / T2 add aria-label / T3 truncation labels NO label added / T4 conditional / T5 titles in Radix menus DELETED / T6 disabled-control span shim); **zero DOM `title=` attributes remain** (grep gate is authoritative). Rich ⚠ **`ChainWarningTip`** ([`screens/ChainWarningTip.tsx`](../web/apps/editor/src/screens/ChainWarningTip.tsx)) = amber band + aligned per-generation breakdown, consuming `ChainWarning.path` directly (no formula dup; `formatChainWarning` is the aria-label). Modal got real keyframes (its `animate-in` classes were **silent no-ops** — Tailwind v4, no `tailwindcss-animate`); OverloadBanner exits via a new generic **`usePresence`** shim ([`lib/use-presence.ts`](../web/apps/editor/src/lib/use-presence.ts), animationend + timeout fallback so reduced-motion can't leak the occlusion). Motion = hand-rolled CSS keyframes keyed to Radix `data-state`/`data-side` (the shipped `popover-animate` pattern extended; NO animation library), shared `--motion-*`/`--slip-*`/`--shadow-soft` tokens in [`styles/tokens.css`](../web/apps/editor/src/styles/tokens.css). **Verified gotcha:** Radix Tooltip `data-state` is `delayed-open`/`instant-open`/`closed` (NOT Dialog's `open`); and Radix renders Content **twice** (visible + VisuallyHidden a11y dup) so the occlusion arms only the visible copy via `closest('[role="tooltip"]')` (TDD caught the double-registration).

2. **Configurable preview overload guard.** #121's compile-time budgets became **runtime members** behind one clamped setter [`Engine::SetOverloadGuard(enabled, maxParticles)`](../src/engine.cpp) (`engine.h` `kDefaultMaxPreviewParticles=15'000`, `kInstancesDivisor=20` → instances derive as cap/20, `kMin/kMaxConfigurableParticles=1'000/1'000'000`); gates short-circuit to "allow" when disabled, `Update` refill/latch skipped, latch dropped immediately on disable. **"Bail earlier" needed no new code** — a lower cap makes the existing `SpawnBudgetExhausted` bail engage sooner. New bridge command `engine/set/overload-guard {enabled,maxParticles}` (schema is a **TS type-union, no runtime validation** → the engine clamp is load-bearing) + `BridgeDispatcher` **cache-and-reapply on `SetEngine`** (engine-recreation insurance; engine is once-per-process today). Web owns persistence ([`lib/overload-guard.ts`](../web/apps/editor/src/lib/overload-guard.ts), `OVERLOAD_GUARD_DEFAULT` frozen to kill a by-ref singleton-mutation hazard) + **Preferences → Preview** (checkbox + number field greyed when off + **amber crash-warning** when off; applied on every change AND at app mount, the theme pattern). **Off = fully uncapped** (pre-#121 behavior, CAN OOM; UI says so). Default started 25k, user lowered to **15k** at feel test.

### Feel-test fixes (user-driven, folded into #123 before merge)
- **Tooltips off passive non-buttons** — the T3 property-tab labels, the StatusBar readout, the import-path span lost their tooltips entirely (interactive controls + the ⚠ glyph keep theirs). "doesn't seem necessary."
- **Warning copy reworded** — "This {chain|emitter} may spawn too many particles" (emitter when a single emitter pins the threshold, path.length===1); dropped "far"; dropped the "Soft warning — nothing is blocked" sub-line; first breakdown row reads "~N particles" not "~N alive". `formatChainWarning` (aria-label) mirrors it.
- **Modal + banner spawned mis-centered then snapped** → **Tailwind v4 gotcha (L-082):** v4 compiles `-translate-x/y-1/2` to the standalone **`translate` CSS property**, not `transform`; keyframes that also did `translate(-50%,…)` inside `transform` double-shifted the surface. Fixed: keyframes slip via `transform: translateY()` only; `translate` owns centering; the two compose. (The standalone-HTML brainstorm mockup never showed it — it centered via `transform`. mockup ≠ real host.)

### Verified baseline at `57cb93e`
Web **700/700** (`pnpm --filter @particle-editor/editor test` from `web/`), `tsc -b` 0, vite build clean; native harness **180/0** (30 skipped; verified TWICE — after the guard build and after the 15k change — zero golden diff); host Debug + Release x64 clean (benign LNK4098). User live-tested NT-12 (tooltips/⚠/modal/banner) + the guard (Preferences toggle + cap, banner at lowered ceiling, uncapped behavior, restart-survival) and approved. Specs: [`docs/superpowers/specs/2026-06-10-nt12-tooltips-motion-design.md`](../docs/superpowers/specs/2026-06-10-nt12-tooltips-motion-design.md) + [`...-overload-guard-config-design.md`](../docs/superpowers/specs/2026-06-10-overload-guard-config-design.md); plans alongside in `docs/superpowers/plans/`; per-feature review notes appended to [`tasks/todo.md`](todo.md).

### Lessons / validated notes this session
- **L-080 (new):** a piped command's exit code is the LAST stage's — gating on `cmd | tail -N && echo OK` reports success on failure; capture `cmd > log 2>&1; echo $?` or test the artifact. (Bit 3× — a fresh-worktree NuGet check, a `pnpm build` typo'd script, a truncated native-harness failure.)
- **L-081 (new):** one tree-touching subagent per worktree (a reviewer stashed/mis-popped another session's stash → UU conflict); and a11y goldens regenerate ONLY from the full `pnpm a11y:update` run — single-test `--grep` regen captures wrong cross-spec state (earlier specs establish selection/panels). Goldens were also FLAKY until `captureDomA11y` settled tooltip exit animations.
- **L-082 (new):** Tailwind v4 compiles `translate`/`scale`/`rotate` utilities to the standalone CSS PROPERTIES, not `transform`; a keyframe animating `transform` COMPOSES with them rather than overriding (opposite of v3). Diagnosed via `getComputedStyle(el).translate` vs `.transform` in the live preview.
- **Native test-host OOM ceiling:** a 100k-particle plateau OOM-crashes the **Debug test host** at the tail of the 178-spec single-process harness run (cumulative heap, NOT an engine fault — the guard's enabled path is byte-equivalent to #121's 100k). The overload specs pin explicit caps ≤15k, not 100k.

### Housekeeping done
Foreign `T4c.4` stash (orphaned `claude/wizardly-napier-37104b`, superseded by master) **dropped** (recoverable from `28f716d` for ~2 weeks). Leftover preview dev server **stopped**. The a11y-pause-leak background chip is not visible to the user (no `task_id` reachable) — treated resolved.

### Kickoff (full) — paste into a fresh session
> Pick up `new-particle-editor` (AloParticleEditor — Win32 host + WebView2/React + D3D9Ex-via-DComp particle editor for SW:EaW). **`master` is the trunk**; **`origin/master` = `57cb93e`**. Session 36 shipped **two** features to master via PR #123 (`bd52588`): the **NT-12 styled/animated tooltip pass + modal/banner motion family** (one `Tip` primitive on `@radix-ui/react-tooltip`, ~45-site title→Tip sweep, rich ⚠ `ChainWarningTip`, Modal/`usePresence`-banner keyframes, shared motion/shadow tokens — all hand-rolled CSS keyframes keyed to Radix `data-state`, NO animation lib), AND a **configurable preview overload guard** (#121's budgets made runtime behind `Engine::SetOverloadGuard`; `engine/set/overload-guard` bridge command + dispatcher reapply-cache; `lib/overload-guard.ts` persistence + Preferences→Preview toggle/cap; **default 15,000**; off = fully uncapped). CHANGELOG backfill was #124 (`57cb93e`). Read `tasks/HANDOFF.md` top (session 36), the two specs + plans under `docs/superpowers/`, `tasks/todo.md` (both review sections), and `tasks/lessons.md` (**L-080** pipe-masked exit codes; **L-081** one tree-touching agent per worktree + full-run-only golden regen; **L-082** Tailwind v4 translate-property composes with keyframe transform; plus L-033 feel-tests are user-launched, L-039/L-040/L-046 fresh-worktree restore). **VERIFY claims against code.** Pre-flight: `git fetch origin master` + lineage check; fresh worktree needs L-039 (WebView2 NuGet copy into `packages/`) + L-040 (`pnpm build`); from `web/` `pnpm install`, `pnpm --filter @particle-editor/editor test` → **700**, `tsc -b` 0; host Debug x64 (MSBuild VS18, L-046); native harness → **180/0** (the overload specs run LAST; never pin them to 100k — it OOM-crashes the Debug test host at the harness tail). **NO task is queued — ask the user what's next.** Candidates: ROADMAP §1 is now empty; **MT-13 legacy/arch-A removal** is user-greenlit (see memory `project_daily_driving_legacy`); the residual splitter stutter is Chromium's own relayout rate (panel-virtualization is the only lever — L-079, new scope). Summarize understanding + confirm scope before changing anything.

### Kickoff (short)
> Pick up AloParticleEditor. **`origin/master` = `57cb93e`.** Session 36 shipped NT-12 styled/animated tooltips + modal/banner motion (Tip primitive on `@radix-ui/react-tooltip`, ~45-site sweep, rich ⚠ tooltip, hand-rolled keyframes) AND a configurable overload guard (#121 budgets → runtime, Preferences toggle + cap, **default 15k**, off=uncapped) — both via PR #123 (`bd52588`), CHANGELOG backfill #124 (`57cb93e`), both feel-tested + merged. Read HANDOFF top (s36) + the two specs under `docs/superpowers/` + lessons **L-080/L-081/L-082** (+ L-033 user-launched feel tests; fresh worktree → L-039/L-040). Pre-flight: web **700**, `tsc -b` 0, native **180/0** (never pin overload specs to 100k — OOMs the Debug test host), host Debug x64 (VS18). **Nothing queued — ask the user what's next** (candidates: MT-13 legacy removal [greenlit]; splitter panel-virtualization [L-079]).
