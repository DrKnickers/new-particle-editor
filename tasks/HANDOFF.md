...
## 2026-06-13 (session 41) — **Shipped [MT-16] transparency parity — the editor preview no longer renders particles washed-out / see-through vs. in-game (#155, `460c070`; CHANGELOG hash backfilled #156). A ~4-line engine render-state fix (mask alpha writes on the final scene-combine so the layered/DComp compositor presents the engine RGB at full opacity), NO shader edit — the transparency half of MT-16; the tone/colour-grade half moved entirely to [LT-8]. User-feel-tested + merged. Then SCOPED [MT-15] (skydome from game/mod, primary+secondary) via a multi-agent workflow → a code-anchored scoping doc (#157, `92026e1`, no code) — headline: the game skydome is a textured `.alo` MESH the editor can't load today, but `GlyphXTools/alo-viewer` (MIT C++ DX9) is a port source that de-risks it. NEXT: start MT-15 from the scoping doc (ask the user; LT-5/6/7/8 also queued).**

**`origin/master` = `92026e1`** (this session advanced master `98fab37 → 92026e1` entirely via my three PRs: #155 MT-16 `460c070`, #156 backfill `63cabca`, #157 MT-15 scope `92026e1`). **No parallel-session movement observed this period** (master moved only via my merges; still re-check `HEAD..origin/master` next session — parallel sessions are live in general). At handoff the only open item is **this docs-only handoff PR**. Working tree clean; all feature/doc branches merged.

### ★ NEXT TASK (not started) — start MT-15, or pick from the queue
- **[MT-15]** Skydome from the game/mod (primary + secondary) — **scoped this session**; start from [`docs/superpowers/specs/2026-06-13-mt15-skydome-scope.md`](../docs/superpowers/specs/2026-06-13-mt15-skydome-scope.md). `ROADMAP.md` §2.1. **Read the scoping doc first** — esp. §5a (the ~12 format questions that MUST be confirmed against a **real FoC install/mod before any data-model code** — the dev box has no game) and §2.5 (the alo-viewer port source). The user said MT-15 will "begin in a new session."
- **[LT-5] thread 2** — ps_2_0 particle bump-colorize shader. **Its gate is now cleared** (it was deferred "until MT-16 lands"; MT-16 has landed). See `project_mt16_lt5_render_triage`. ⚠ This touches a particle shader — re-read the no-fork rule before assuming scope (the colorization source was decided = **material param**).
- **[LT-6]** Distance-unit parity → projectile-motion emulation (independent foundation; unblocks **[LT-7]** scale refs). **[LT-8]** Recreate the game's `Scene_*.fx` composite — the tone/colour-grade half of MT-16 + depends on MT-15's map-env reader.

**⚠ Render-parity HARD RULE still in force** (memory `feedback_no_shader_fork_1to1_rendering`): parity comes from running the game's REAL shaders + recreating its pipeline, never forking/revising them. MT-16's fix obeyed this (render-state only, `SceneHeat.fx` untouched). Adapting editor-furniture shaders (ground/skydome) is fine.

### What shipped this session

1. **#155 (`460c070`) — [MT-16] transparency parity (the alpha-erosion washout half).** Feel-tested + merged. **Root cause** (from the session-40 triage `docs/superpowers/specs/2026-06-13-mt16-transparency-parity-triage.md`): the new-UI viewport is a per-pixel-alpha layered/DComp surface, so the scene RT's alpha is meaningful (viewport opacity), but the particle path was written for a game backbuffer where alpha is ignored — transparent/modulate blends erode the RT alpha and `SceneHeat.fx` copies it through, so the compositor shows it as see-through. **Fix** ([`src/engine.cpp:1111`](../src/engine.cpp:1111), in the final-combine pass loop): `SetRenderState(D3DRS_COLORWRITEENABLE, RED|GREEN|BLUE)` **after** `BeginPass` (so the effect's pass-state apply can't clobber it — verified `SceneHeat.fx` `t0/p0` sets only AlphaBlend/AlphaTest/ZWrite/ZFunc, never ColorWriteEnable), the existing `DrawPrimitiveUP`, then restore full RGBA. The RT keeps the opaque alpha it was cleared to → editor presents engine RGB at full opacity = parity, blend-mode-agnostic, covers both arch-C DComp (default) + arch-B layered (both sample the same RT, [`src/engine.cpp:1127`](../src/engine.cpp:1127)). **No shader edit.** ROADMAP: MT-16 → Shipped 5.1, tag retired, LT-8 updated to carry the tone half. CHANGELOG entry + #156 hash backfill.
2. **#157 (`92026e1`) — [MT-15] scoping doc (no code).** Produced via a multi-agent workflow (4 parallel investigators → 16 adversarial verifications → synthesis → completeness critic + revise; 23 agents). All §2 claims code-anchored. **Headline findings:** (a) the game skydome is a textured `.alo` MESH and the editor has NO static-mesh loader (its `.alo` parser only knows the particle-system chunk dialect) — the central a1 (textured sphere, cheap, drops SH lighting) vs a2 (faithful real mesh) fork; (b) **`GlyphXTools/alo-viewer`** (MIT, C++, DX9, shares our `ChunkReader`+expat-2.2.0+DX9 deps) is a **port source** that turns a2 from a research spike into a bounded port (and unblocks LT-7) — memory `reference_alo_viewer_mesh_loader`; (c) the map-environment reader MT-15 builds is the **LT-8 keystone** (build reusably: `LoadMapEnvironment(IFileManager&, MapEnvironment&)`); (d) a **real new-UI persistence gap** — skydome selection set via the bridge is never written back to the registry (doesn't survive restart); (e) ~12 format questions gated on a real install (§5a). Recommends a phased split (A: one real skydome; B: primary/secondary + dual-slot picker + persistence).

### Verified baseline at the MT-16 commit (`460c070`) / `92026e1`
Host **Debug + Release x64** clean (benign C4244 + LNK4098). Web **795/795** (`pnpm --filter @particle-editor/editor test` from `web/`), `tsc -b` 0. **Cold-launch smoke** (L-084): 7 s in **composition mode** (arch-C, the fix's target path), AlphaCompositor shared RT created, snapshot readback 1.47 ms, **zero D3D/effect/device-lost errors**. **Native composite/readback** `alpha-compositor-snapshot` 3/3 (12/12 of the snapshot-matching specs). MT-16 **user-feel-tested** (transparent/modulate particles read at correct opacity; no compositor regressions). #155/#156/#157 all green on all three CI legs. Fresh worktree → L-039 (NuGet copy) + L-040 (`pnpm build`) were both needed this session.

### Process notes / lessons this session
- **MT-16 verification has an irreducible visual gap.** The `alpha-compositor-snapshot` path encodes **JPEG** (no alpha channel), so it can prove the render/readback path survives the change but **cannot assert the opaque-alpha result** — that's a user feel-test only (L-033). State this plainly; don't imply the spec proved the fix.
- **`D3DRS_COLORWRITEENABLE` must be set AFTER `BeginPass`**, not before `Begin` — the D3DX effect's pass-state apply runs in `BeginPass`. Verified the specific `.fx` doesn't manage ColorWriteEnable, so it's belt-and-suspenders here, but the rule generalizes. Restore to full RGBA after the draw so later frames' particle blends are unaffected. (No new lesson file entry written — captured here + in the CHANGELOG.)
- **The per-PR merge gate bit again, as warned.** The auto-mode classifier **denied** merging #155 on a vague "yes and yes for followup" — it required an explicit per-PR "may I merge #155 to master now?" The same applied to #156 and #157. Every master merge needs its own unambiguous OK, even docs-only.
- **The multi-agent scoping workflow worked well.** Adversarial verification caught two synthesis overstatements: the game `Skydome.fx` advertises fog but it's **inert** (`Out.Fog = 1.0; //Compute_Fog(...)` commented out), and the curated `W_SKY*.DDS` paths can't be confirmed to exist without a real install. Both corrected in the doc. 0 claims refuted, 2 "partly," 14 confirmed of 16 checked.
- **alo-viewer is verified at the repo-structure level only** (language/license/deps/the NU2C struct via `gh api`) — the precise mesh/material chunk IDs + `ReadMesh` body are NOT yet read; a real port must re-read the live source + validate against a real `.alo`.

### Kickoff (full) — paste into a fresh session
> Pick up `new-particle-editor` (AloParticleEditor — Win32 host + WebView2/React + D3D9Ex-via-DComp particle editor for SW:EaW). **`master` is the trunk; `origin/master` = `92026e1`.** Session 41 shipped **[MT-16] transparency parity** — the editor preview no longer renders particles washed-out/see-through vs in-game (#155, `460c070`; backfill #156): a ~4-line engine render-state fix at [`src/engine.cpp:1111`](src/engine.cpp:1111) masking alpha writes (`D3DRS_COLORWRITEENABLE=RGB`) after `BeginPass` on the final scene-combine so the layered/DComp compositor presents engine RGB at full opacity — **NO shader edit** (the transparency half; tone/colour-grade half moved to [LT-8]). Then **scoped [MT-15]** (skydome from game/mod, primary+secondary) via a multi-agent workflow → [`docs/superpowers/specs/2026-06-13-mt15-skydome-scope.md`](docs/superpowers/specs/2026-06-13-mt15-skydome-scope.md) (#157, `92026e1`, **no code**). **NEXT (not started): start MT-15 from that scoping doc — READ IT FIRST**, esp. §5a (≈12 format questions that MUST be confirmed against a real FoC install/mod *before* any data-model code — the dev box has none) and §2.5 (the MIT C++ DX9 `GlyphXTools/alo-viewer` port source that de-risks faithful-mesh rendering). LT-5 thread 2 / LT-6 / LT-7 / LT-8 also queued — **ask the user which.** ⚠ **Render-parity HARD RULE** (memory `feedback_no_shader_fork_1to1_rendering`): fidelity = run the game's REAL shaders + recreate its pipeline, never fork/revise them; particle shaders are 1:1 and must NOT be revised; editor-furniture shaders (ground/skydome) may be adapted. Memories to read: `reference_alo_viewer_mesh_loader`, `project_mt16_lt5_render_triage` (MT-16 transparency now shipped; tone half = LT-8; LT-5 t2 gate cleared), `project_mt14_ground_lighting`, `project_daily_driving_legacy` (MT-13 = legacy removal). **VERIFY all claims against code.** Pre-flight: `git fetch origin master` + lineage (rebase if `HEAD..origin/master` ≠ 0 — parallel sessions are live); fresh worktree → L-039 (copy `~/.nuget/.../microsoft.web.webview2/1.0.3967.48/*` → `packages/Microsoft.Web.WebView2.1.0.3967.48/`) + L-040 (`pnpm build`); from `web/` `pnpm install`, `pnpm --filter @particle-editor/editor test` → **795**, `tsc -b` 0; host Debug x64 via VS18 MSBuild (`C:\Program Files\Microsoft Visual Studio\18\Community\MSBuild\Current\Bin\MSBuild.exe`, L-046); native harness if needed. **Heads-up:** `gh pr merge` needs explicit user OK **per PR** (the classifier enforces it, even docs); a new `.fx` only compiles at runtime → cold-launch + stderr smoke-check (L-084); the `alpha-compositor-snapshot` spec encodes JPEG so it can't verify alpha (visual = user feel-test, L-033); HWND/UIA JSON a11y goldens are dormant by design. Summarize understanding + confirm scope before changing anything.

### Kickoff (short)
> Pick up AloParticleEditor. **`origin/master` = `92026e1`.** Session 41 shipped **MT-16 transparency parity** (#155 `460c070`) — alpha-erosion washout fix, ~4 lines at [`src/engine.cpp:1111`](src/engine.cpp:1111) masking alpha writes on the final combine, NO shader edit; feel-tested + merged, backfill #156. Then **scoped MT-15** (#157, no code) → [`docs/superpowers/specs/2026-06-13-mt15-skydome-scope.md`](docs/superpowers/specs/2026-06-13-mt15-skydome-scope.md). **NEXT (not started): start MT-15 from the scoping doc** (read §5a real-install gate + §2.5 alo-viewer port source first); LT-5 t2 / LT-6 / LT-7 / LT-8 also queued — **ask which.** ⚠ HARD RULE: render parity = game's REAL shaders, never fork; particle shaders 1:1; editor-furniture (ground/skydome) may be adapted (memory `feedback_no_shader_fork_1to1_rendering`). Memories: `reference_alo_viewer_mesh_loader`, `project_mt16_lt5_render_triage`. VERIFY against code. Pre-flight: re-check `HEAD..origin/master` (parallel sessions live); web **795**, `tsc -b` 0, host Debug x64 (VS18; fresh worktree → L-039/L-040). `gh pr merge` needs explicit OK per PR.
