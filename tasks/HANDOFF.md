...
## 2026-06-14 (session 44) — **Built the render-capture path + fixed 2 MT-15 render-blockers (#165), then STARTED [LT-7] (imported game objects): shipped the `.alo` skeleton/connection decode (PR-A #167) + the `ReferenceObjectMesh` rigid-multi-part renderer (PR-B #168) — a real game turret now renders upright with horizontal barrels. Also fixed a live-test-found MT-15 space-dome bug (the secondary nebula was buried under the opaque primary; #171). 5 PRs merged + 2 multi-agent design passes + 1 multi-agent bug investigation. NEXT: LT-7 PR-C (GameObjectCatalog — enumerate game objects by Name), then PR-D (bridge+picker+gizmo) / PR-E (grid) / PR-F (in-viewport manipulator) per `tasks/todo.md`.**

**`origin/master` = `c399357`** (this session merged, oldest→newest: **#165** `70fc6a8` MT-15 capture-path + 2 fixes + the `debug/capture-frame` bridge → **#166** `aadb48e` CHANGELOG backfill → **#167** `53e8a10` LT-7 PR-A skeleton/connection decode → **#168** `5c84419` LT-7 PR-B `ReferenceObjectMesh` renderer → **#171** `c399357` MT-15 space nebula compose fix). ⚠ **Master also advanced via PARALLEL sessions** this period: **#169** (`8eede18`, docs: an LT-9 "MCP particle toolkit" roadmap entry — not mine) + **#170** (`53eb155`, chore: track `AGENTS.md`, gitignore `rc.exe` RC* temps — not mine). The session branch tip was `claude/mt15-space-compose-fix` (merged); **branch fresh off `master` `c399357` for PR-C.**

### ★ NEXT TASK (not started) — LT-7 PR-C: `GameObjectCatalog`
Full LT-7 plan in [`tasks/todo.md`](todo.md) (§1–7, the 6-PR sequence A→F + ranked risks). **PR-A/B are SHIPPED**; the rest:
- **PR-C (start here, independent of A/B):** NEW `src/GameObjectCatalog.{h,cpp}` (pure `IFileManager`, paralleling `SkydomeEnvironment`) — read `Data\XML\GameObjectFiles.xml` (~120 `<File>` entries) → **two-phase** parse: (1) every listed object file into a global `Name→{rawModelPath, variantOf, tag, category}` map; (2) resolve `Variant_Of_Existing_Type` inheritance AFTER all files load (cross-file + cyclic-safe). Model field varies (`Land_Model_Name`→`Space_Model_Name`→`Model_Name`→…). **Lazy** `ProbeModelSkinned(fm, modelPath)` on-select (NOT at build — ~3,900 objects would freeze startup), cached. Mock-FM unit tests + dump-mode vs the real install. *(Deep-dive verified in `tasks/todo.md` §3 PR-C + the design-workflow output.)*
- **PR-D:** Engine ref-object state (name/visible/position/rotation, mirror skydome-environment + `m_wind`/`m_gravity` Vec3) + bridge kinds (`engine/set/reference-object`, `-visible`, `-transform`, `-grid-visible`, `-grid-spacing`, `engine/query/reference-object-list`) + registry persistence + HostWindow restore + a React `ReferenceObjectPicker` (clone `BackgroundPicker.tsx`; Name list grouped by category, skinned greyed) + a **minimal numeric 6-`Spinner` transform** (the precise/fallback path) + bridge-schema typing (`ResponseForA`) + mock + Vitest.
- **PR-E:** unit grid — the engine's FIRST line primitive (`DrawPrimitiveUP D3DPT_LINELIST` reusing `EmitterInstance::Vertex`), co-planar with the ground, major line every 5 cells, no labels (→ LT-6).
- **PR-F (user-chosen):** the **in-viewport 3D drag manipulator** (grabbable axis handles) — its OWN sub-project: viewport ray-picking + screen-to-world + host-side hit-test, NO precedent. **Needs its own focused design pass** when reached (after PR-D's transform plumbing exists). The user confirmed this is wanted over numeric-only.

### What shipped this session — 5 PRs, all CI-green + merged
1. **#165 (`70fc6a8`) — MT-15 render feel-test → 2 blocker fixes + the `debug/capture-frame` bridge.** The dome shipped (#163) render-UNVERIFIED; a capture feel-test found it BROKEN two ways on any MEG install: (a) [`SubFile::read`](../src/files.cpp) over-read (didn't clamp to the sub-view → `XMLTree::parse`'s 32 KB chunks spilled adjacent MEG bytes → `LoadSkydomeList` failed → **0 game domes**); (b) [`loadMaterialTexture`](../src/SkydomeMesh.cpp) missing the engine's `.tga`→`.dds` swap → **black dome**. Both fixed + a `SubFile` regression test ([`tests/test_subfile_read.cpp`](../tests/test_subfile_read.cpp), proven to fail without the clamp). Plus the live `debug/capture-frame { path }` bridge (reuses `AlphaCompositor::CaptureSnapshotToFile`, Debug/`--test-host`-gated). **Most of the planned "capture tool" already existed** (CaptureSnapshotToFile + the `--capture <alo> <png>` headless mode + the MT-15 env hook) — a new D3DX method was redundant. Adversarial multi-agent review (0 blockers). Lessons **L-086** (bounded-view reads must clamp + test the real MEG container, not loose-file mocks) / **L-087** (`.tga`→`.dds` asset fallback).
2. **#166 (`aadb48e`)** — CHANGELOG hash/PR# backfill for #165.
3. **#167 (`53e8a10`) — LT-7 PR-A: `.alo` skeleton (`0x200`) + connections (`0x600`) decode.** [`src/AloModel.{h,cpp}`](../src/AloModel.h) gains `AloBone`/`AloConnection` (stored verbatim; root sentinel NOT normalized) via **tolerant** readers (never throw on an odd shape — these chunks were skipped wholesale before). Format pinned vs the `max2alamo` writer (`alo_build.cpp`) AND **validated byte-for-byte vs the real `AI_Bunker_Turret1.alo`** (6 bones, `Root` parent=`0xFFFFFFFF` + a `parent<self` chain; 7 connections map Body/Barrel/Turret to their bones). +17 unit assertions; existing 30 green.
4. **#168 (`5c84419`) — LT-7 PR-B: `ReferenceObjectMesh` renderer.** NEW [`src/ReferenceObjectMesh.{h,cpp}`](../src/ReferenceObjectMesh.h) cloned from `SkydomeMesh`, diverging in: an **extended transcoder** adding `alD3dVertNU2U3`/`NU2U3U3`/`NU2U3U3C` (`TANGENT0`@56 + `BINORMAL0`@68, verified vs `MeshBumpColorize`'s VS_INPUT_MESH); **skinned (`RSkin*`/`B4I4*`) + collision/shadow shader filtering**; **rigid bone placement** (`computeBoneObjectMatrices` accumulates `obj[i]=local[i]*obj[parent[i]]`; column-major `float[12]`→`D3DXMATRIX` **no transpose**, the editor being row-vector RH Z-up). `Engine::RenderReferenceObject()` — SOLID state (`ZENABLE`/`ZWRITE` TRUE, `CULL_CCW`, L-032 save/restore), drawn after the ground + before particles; device lost/reset wired (3 phases); a `#ifndef NDEBUG ALO_LT7_TEST_OBJECT` bring-up hook (kept, like MT-15's). **Verified via capture:** `AI_Bunker_Turret1.alo` renders UPRIGHT with the **barrels HORIZONTAL** (bone-chain proven), solid + correctly culled (not inside-out), lit, `MeshBumpColorize.fx` 1:1; collision/shadow hulls filtered (3 visible sub-meshes). Winding/up-axis right on the first model.
5. **#171 (`c399357`) — MT-15 space nebula compose fix.** Live feel-test (user) found the SPACE secondary nebula invisible. Cause: `Engine::RenderSkydomes` drew space `{secondary, primary}` with depth off, so the **opaque** primary star sphere (`MeshGloss`, `SKYBLEND_OPAQUE`) overwrote the **additive** secondary nebula (`MeshAdditiveVColor`, `ONE/ONE`) drawn first. **One-line fix:** `primaryFirst = true` (primary opaque background first, secondary on top, both contexts; **Land byte-identical**). Verified via the `[SkyDraw]` blend/order trace (opaque MeshGloss now draws before the additive nebula) + a 4-agent first-party investigation. Seam = the star dome's UV-sphere longitude closure (asset-level; the game's `.fxo` re-applies WRAP, so the editor samples identically → likely matches in-game) — **deferred**.

### ⚠ Pending USER verification (GUI-coupled, I can't see it)
- **MT-15 space nebula (#171):** confirm the nebula now composites OVER the starfield in the live picker (Space → e.g. `Stars_Medium` + a `Star_Backdrop_*`), and whether the **seam** matches in-game. The fixed-downward headless `--capture` camera **cannot show a space dome** — the exact gap the live `debug/capture-frame` bridge will close. *(If the order flip alone doesn't match — i.e. the nebula should also show THROUGH dark star gaps — the deeper-faithful fix is to honor each sub-mesh's authored blend instead of the editor's `SKYBLEND_OPAQUE` approximation.)*
- **LT-7 turret (#168):** I verified it via capture (scaled up; default `--capture` camera frames the 1600-unit ground so a ~13-unit object is tiny). A live feel-test (once PR-D's picker lands) confirms it at a normal zoom.

### Verified baseline at `c399357`
Native **Debug x64 clean** (Release link is locally blocked when the user's editor holds the `.exe` lock, but **CI builds Release clean** — all 5 PRs green: C++ Debug+Release + web Vitest). Leaf cores: `tests/build_test_alo_model.bat` **30+17** (skeleton/connection added), `tests/build_test_skydome_environment.bat` 25, `tests/build_test_subfile_read.bat` PASS (new). Web Vitest 799. Capture feel-tests: MT-15 land dome (blue), the turret (upright/barrels-horizontal), the space compose order. **NOT verified:** the space-dome VISUAL (#171) + the turret at normal zoom — both need the user's live eye.

### Process notes / lessons this session
- **The capture tool's first use paid for itself** — it found the 2 MT-15 blockers (#165) AND the bone-placement verification (#168). The headless `--capture` reuses the REAL `Engine::Render()`; the live `debug/capture-frame` bridge is built but its in-session round-trip + TS bridge-schema typing are DEFERRED (need a Playwright/dev-UI driver).
- **`--capture` can't show a space dome** (fixed-downward camera + always-on ground) — and the **HostWindow registry restore clobbers the ctor `ALO_MT15_TEST_*` env hook** in capture mode (restores the persisted dome over the env-hook's). Verified the space fix by temporarily setting the registry to the space scenario + restoring it. The live bridge (camera/ground control) is the real fix for space-dome verification.
- **`.alo` skeleton format** (max2alamo + real turret): `0x200`→`0x201` info(128B, ignore count)→N×`0x202` bone(`0x203` name + `0x205`(56B)/`0x206`(60B) data = parentIndex,visible,[billboard],matrix[12] **column-major**); `0x600`→`0x602` mini id2=objectIndex id3=boneIndex. Root sentinel = `0xFFFFFFFF` + **direct** parent/bone indices (NOT the format-notes "subtract-1" variant — the real files use direct). `objectIndex == mesh ordinal` (meshes precede lights; we skip lights).
- **Two multi-agent design passes for LT-7** (understand→deepen→integrate) pinned the hard formats (skeleton placement + the GameObject XML enumeration graph) against first-party sources BEFORE coding — winding/up-axis were right on the first real model.
- **Decisions locked (user):** full LT-7 scope = static + **rigid multi-part** (skinned deferred); **enumerate by Name** picker (not browse-only); a **full in-viewport drag manipulator** (PR-F, not numeric-only); grid defaults (spacing 20 / ±800 / major-every-5 / no labels).
- **`gh pr merge` needs explicit user OK per PR** (the classifier enforces it). Parallel sessions move master — re-check `HEAD..origin/master` before every push.

### Kickoff (full) — paste into a fresh session
> Pick up `new-particle-editor` (AloParticleEditor — Win32 host + WebView2/React + D3D9Ex-via-DComp particle editor for SW:EaW). **`master` is the trunk; `origin/master` = `c399357`.** Session 44 (a) shipped the MT-15 render feel-test + 2 blocker fixes + the `debug/capture-frame` bridge (**#165**), and (b) **STARTED [LT-7] (in-preview imported game objects + a unit grid)**: shipped **PR-A** (**#167**, `src/AloModel` `.alo` skeleton `0x200`/connections `0x600` decode — validated vs the real `AI_Bunker_Turret1.alo`) and **PR-B** (**#168**, `src/ReferenceObjectMesh.{h,cpp}` + `Engine::RenderReferenceObject` — loads a real game `.alo` and renders it as inert SOLID geometry, each rigid sub-mesh placed by its skeleton bone, running each sub-mesh's own game shader 1:1; a turret renders upright with horizontal barrels). Also fixed a live-test-found MT-15 SPACE bug (**#171**: the secondary nebula was buried under the opaque primary — `Engine::RenderSkydomes` `primaryFirst=true`). **NEXT (not started): LT-7 PR-C — `src/GameObjectCatalog.{h,cpp}`** (pure `IFileManager`, parallels `SkydomeEnvironment`): read `Data\XML\GameObjectFiles.xml` → two-phase parse all object files → resolve `Variant_Of_Existing_Type` inheritance; lazy `ProbeModelSkinned` on-select; mock-FM unit tests. THEN PR-D (engine ref-object state + bridge + registry persistence + a React `ReferenceObjectPicker` cloned from `BackgroundPicker.tsx` + a numeric 6-`Spinner` transform), PR-E (unit grid — the engine's first `D3DPT_LINELIST` primitive), PR-F (the user-chosen in-viewport 3D drag manipulator — its own focused design pass, needs viewport ray-picking, no precedent). **Read FIRST:** `tasks/HANDOFF.md` top (session 44), the full LT-7 plan in `tasks/todo.md` (§1–7, the A→F sequence + ranked risks); memories `project_render_capture_tool`, `project_mt15_skydome_5a_closed`, `reference_max2alamo_format`, `reference_foc_install_dev_box`, `feedback_no_shader_fork_1to1_rendering`. **Open:** `src/AloModel.h` (AloBone/AloConnection), `src/ReferenceObjectMesh.{h,cpp}`, `src/engine.cpp` (`RenderReferenceObject` ~the LT-7 block, `RenderSkydomes` ~2577, the `ALO_LT7_TEST_OBJECT`/`ALO_MT15_TEST_DOME` ctor hooks), `src/SkydomeEnvironment.cpp` (the clone target for the catalog). **VERIFY claims against code.** **⚠ Pending USER verification:** the MT-15 space nebula visual (#171) + the seam — the headless `--capture` camera can't show a space dome; ask the user to live-test the Space picker. **Pre-flight:** `git fetch origin master` + lineage (rebase if `HEAD..origin/master` ≠ 0 — parallel sessions move master, e.g. this period's #169/#170); fresh worktree → **L-039** (copy `~/.nuget/.../microsoft.web.webview2/1.0.3967.48/*` → `packages/Microsoft.Web.WebView2.1.0.3967.48/`) + **L-040** (`pnpm -C web install` then `pnpm -C web --filter @particle-editor/editor build`); host Debug x64 via VS18 MSBuild (`C:\Program Files\Microsoft Visual Studio\18\Community\MSBuild\Current\Bin\MSBuild.exe`, L-046); leaf tests via the `.bat`s (create `tests\obj\` + build the host first so `expatw_static.lib` exists). **Heads-up:** `gh pr merge` needs explicit user OK per PR; engine diagnostics go to STDERR (L-084); the Release `.exe` link fails locally if a running editor holds the lock (CI builds Release fine); to capture a reference object headlessly use `ALO_LT7_TEST_OBJECT=Data\Art\Models\<x>.alo` with `--capture` (it's tiny at the default camera — scale it up to inspect); **`.alo` rigid placement is verified, skinned units are deferred-and-skipped.** Summarize understanding + confirm scope before changing anything.

### Kickoff (short)
> Pick up AloParticleEditor. **`origin/master` = `c399357`.** Session 44 shipped the MT-15 capture-path + 2 render-blocker fixes + the `debug/capture-frame` bridge (**#165**), then **started [LT-7] (imported game objects)**: **PR-A #167** (`.alo` skeleton/connection decode, validated vs the real turret) + **PR-B #168** (`ReferenceObjectMesh` — a real game turret renders upright, barrels horizontal, each sub-mesh's own shader 1:1, rigid bone placement). Also fixed a live-found MT-15 SPACE bug (**#171**: secondary nebula was buried under the opaque primary; `primaryFirst=true`). **NEXT (not started): LT-7 PR-C** — `src/GameObjectCatalog.{h,cpp}` (enumerate game objects by Name from `GameObjectFiles.xml` + `Variant_Of` inheritance; lazy skinned-probe; clone `SkydomeEnvironment`). THEN PR-D (bridge+picker+numeric-gizmo) / PR-E (grid) / PR-F (in-viewport drag manipulator — user-chosen, own design pass). Plan in `tasks/todo.md` (§1–7, A→F). ⚠ **USER must live-test the MT-15 space nebula visual (#171)** — headless `--capture` can't show a space dome. Read HANDOFF top (s44) + `tasks/todo.md`; memories `project_render_capture_tool`/`project_mt15_skydome_5a_closed`/`reference_max2alamo_format`/`reference_foc_install_dev_box`. Pre-flight: `git fetch`+lineage (parallel sessions move master); fresh worktree → L-039/L-040; host Debug x64 (VS18); leaf `.bat`s. `gh pr merge` needs OK per PR. VERIFY vs code; summarize before changing.

---

## 2026-06-14 (session 43) — **Shipped [MT-15] the full faithful skydome in ONE all-in-one PR (#163, `e710f47`): the render core (`SkydomeMesh` — load real `.alo` + run each sub-mesh's OWN game shader 1:1), nebula multi-submesh (per-shader blend + phase-faithful compose), and the dual-slot plumbing (bridge + persistence + restore + a React two-section game-dome picker replacing the fake tiles). Built clean (native Debug+Release x64, editor tsc+vite, 799 Vitest, leaf 30/30+25/25), 3 code reviews (caught + fixed an inverted vertex-format mapping that would've rendered the nebula unlit), CI-green + merged. ⚠ The RENDER ITSELF IS UNVERIFIED — GUI-coupled, needs a visual feel-test. NEXT: build the render-capture tool (option A — `debug/capture-frame` → PNG) so the dome (and all future render work) can be self-verified + filmstrip-tested; then feel-test MT-15 + do its deferred follow-ups.**

**`origin/master` = `e710f47`** (session 43: MT-15 all-in-one #163 `0176526`→merge `e710f47`). Lineage was clean all session (branched off `12e4cff`; re-checked 0 before the PR — no parallel-session drift this period). The MT-15 *foundation* (session 42: `AloModel` + `SkydomeEnvironment` + Step-0 spike) was already on master; this session built the render core + plumbing ON TOP and shipped the whole feature.

### ★ NEXT TASK (not started — user said save for next session) — the render-capture tool (option A)
**Goal:** give Claude a way to *see* the D3D render output, killing the per-render-item feel-test bottleneck (MT-15 now, MT-16/LT-5/LT-8 later). **Decision locked (this session): option A**, not C. Build **`debug/capture-frame { path }`** — a new bridge kind + an engine method that copies the scene render-target / back-buffer to a SYSTEMMEM surface and `D3DXSaveSurfaceToFileW(path, D3DXIFF_PNG, …)` (d3dx9 already linked; the device + the compositor's off-screen RT already exist — smoke log shows `[AlphaCompositor] shared RT 1264x761`). Debug/`--test-host`-gated. Then `Read` the PNG (the Read tool ingests images). **Why A over C:** A captures the REAL `Engine::Render()` pipeline (the whole point of fidelity work); C (a headless render harness) is a divergent second renderer to maintain. A + a golden-PNG diff under `--test-host` even subsumes C's CI-regression use case. **Particle filmstrips (a key driver for A):** the engine already has a pausable clock + `engine/action/step-frames` + `engine/set/paused`, so `pause → step-frames → capture` loop = a deterministic lifecycle filmstrip (verify alpha/scale/colour tracks, additive blend, death). Caveats: each PNG read costs context (targeted 5–8-frame strips, not 60fps); determinism holds within one stepped run (seed RNG for frame-exact goldens); any GPU capture *in CI* needs the runner to have a usable D3D device (WARP/reference) — non-issue locally. **Its own branch/PR, off `e710f47`.**

### ⚠ MT-15 is code-complete but render-UNVERIFIED — the feel-test + its follow-ups
The dome render can only be confirmed by eye (I can't see it; the capture tool above is being built to fix exactly this). **To feel-test:** (1) point the editor at the FoC install / a mod so the FileManager resolves the `.alo` + shaders + textures (my smoke launches showed `[GroundLit] … flat-fallback` → that launch had no install wired; graceful-degrades, no crash); (2) pick a dome via the new **Background → Game dome** picker (Land/Space toggle + Primary/Secondary), *or* the debug env hook `ALO_MT15_TEST_DOME=Day_Blue_Sky ALO_MT15_TEST_CTX=land`; (3) **run with stderr visible** — `[SkyEnv]`/`[AloVtx]`/`[SkyDraw]` confirm load, the selected technique, and (the Risk-2 check) the *actual* render state after `BeginPass`. **Look for:** `Day_Blue_Sky` SH-lit (rotate the sun → dome diffuse shifts); a space env (`Stars_Low` + a nebula secondary) showing additive stars over the nebula backdrop; no L-032 particle whitening. **Deferred until the feel-test confirms it works** (so we don't mark an unverified render Shipped): (a) `ROADMAP.md` MT-15 ✅ Shipped (#163) + the *Actual:* line + move to Shipped; (b) the `CHANGELOG.md` entry; (c) remove the `#ifndef NDEBUG` env-var bring-up driver (TODO in [`engine.cpp`](../src/engine.cpp) Engine ctor); (d) the Playwright two-section-picker spec. If a dome renders inside-out/wrong, the likely knobs: cull mode (currently `CULL_NONE`) or the per-shader blend/compose — all in `Engine::RenderSkydomeMesh`/`RenderSkydomes`.

### What shipped this session — MT-15 all-in-one (#163, `e710f47`), merged
- **M1 render core** — [`src/SkydomeMesh.{h,cpp}`](../src/SkydomeMesh.h): decode (`AloModel`) → transcode the 144 B on-disk vertices into compact runtime decls → `getShader` per sub-mesh → DEFAULT-pool VB/IB + material textures; survives device lost/reset by refilling cached blobs (no re-parse). `Engine::RenderSkydomeMesh`/`RenderSkydomes` ([`src/engine.cpp`](../src/engine.cpp)): particle-template SH + cloud-`TIME` with the REAL world matrix; full per-sub-mesh state set + save/restore delta (L-032); wired into device lost/reset (two-phase), `ReloadTextures` (mod-switch), and `Engine::Render()`.
- **M2 nebula** — 3-way blend (additive `ONE/ONE` / alpha `SRCALPHA/INVSRCALPHA` / opaque) from the shaders' SB blocks; full lighting binding (eye + dir-light0 + SH) for MeshGloss specular; **phase-faithful draw order** (opaque then transparent, independent of `.alo` order); context-dependent compose (space: secondary behind; land: secondary overlay on top).
- **M3 dual-slot** — `engine/set/skydome-environment` + `engine/query/skydome-list` bridge kinds + DTO `skydomeContext`/`PrimaryName`/`SecondaryName`; dispatcher handlers + DTO snapshot + `PersistSkydomeEnvironment` (registry, gated) + HostWindow startup restore (post-device-init → also closed the M1-deferred device-null re-resolve); `BackgroundPicker` rewritten (game-dome section + solid/custom fallback; fake gradient tiles removed); mock + 4 new Vitest.
- **Decisions locked this session:** all-in-one (one PR, not split); option A for the capture tool; the vertex-format layouts (`N`=Pos+Normal, `NU2`=+UV, `NU2C`=+Color, prefix-superset — verified vs max2alamo + shader VS inputs).

### Verified baseline at `e710f47`
Native **Debug + Release x64** clean (0 errors; benign C4244/LNK4098). Editor `tsc -b` + vite clean. **Web 799 Vitest** (4 new MT-15: contract round-trip ×2 + picker component ×2). Leaf cores `tests/build_test_alo_model.bat` **30/30** + `tests/build_test_skydome_environment.bat` **25/25**. Cold-launch smoke (full M1+M2+M3): no startup crash. CI green on #163 (web Vitest + C++ x64 Debug/Release). **NOT verified:** the dome render (feel-test); the Playwright two-section picker spec (deferred).

### Process notes / lessons this session
- **Trust-but-verify the plan caught a real bug.** The plan's vertex-format mapping (`alD3dVertN`=Pos+Normal+UV, `alD3dVertNU2`=Pos+UV-only) was **inverted** vs first-party truth (max2alamo `vertex_format_selector` table: MeshGloss/MeshAdditive→`alD3dVertNU2`; `alD3dVertN` only on no-UV shadow/collision; + the shader VS inputs read `In.Norm`). Correct: `N`=Pos+Normal (24B), `NU2`=Pos+Normal+UV (32B), `NU2C`=+Color (36B). Would've rendered the nebula gloss base unlit. *Verify format/layout claims against max2alamo + the actual shader, not the plan.*
- **`ALAMO_STATE_BLOCKS 0` (Risk 2) — premise may be inverted; verify empirically.** Empty `SB_START/SB_END` makes the inner render-states *bare pass-state*, which D3DX `BeginPass` normally APPLIES (opposite of "no-op'd"). Whether the shipped `.fxo` applies them depends on Petroglyph's compile flag (undeterminable statically). Implementation is robust either way (app sets state + saves/restores), but the `[SkyDraw]` dump of `GetRenderState` AFTER `BeginPass` (stderr) settles it during the feel-test.
- **MT-15 diagnostics go to STDERR** (`[SkyEnv]`/`[AloVtx]`/`[SkyDraw]`) — the GUI exe's stdout isn't captured when launched detached (L-084); engine diagnostics use stderr. Launch with stderr redirected to see them.
- **Accepted-and-named limitation:** dome Names flow as `WideToAnsi`/CP_ACP end-to-end (SkydomeEnvironment → SkydomeRef → DTO/registry); a *non-ASCII* GameObject Name would break restore (persist `Utf8ToWide` vs restore `WideToAnsi`) and could throw in the snapshot `json.dump()`. GameObject Names are ASCII identifiers → never triggers; full fix needs `WideToUtf8` plumbed through the leaf module + utils.h + restore (`WideToUtf8` is NOT currently in utils.h).
- **Fresh-worktree quirk:** the leaf-test `.bat`s need `tests\obj\` created first AND the host solution built first (so `libs\expat-2.2.0\x64\Debug\expatw_static.lib` exists, which the skydome test links).
- **Multi-agent code reviews** (7-angle M1 / 3-angle M2 / 3-angle M3) drove quality; the cross-cutting M3 contract review came back clean (schema↔dispatcher↔mock↔picker consistent).

### Kickoff (full) — paste into a fresh session
> Pick up `new-particle-editor` (AloParticleEditor — Win32 host + WebView2/React + D3D9Ex-via-DComp particle editor for SW:EaW). **`master` is the trunk; `origin/master` = `e710f47`.** Session 43 shipped **[MT-15]** — the full faithful game/mod skydome — in ONE all-in-one PR (**#163**, `0176526`→merge `e710f47`): the render core (`src/SkydomeMesh.{h,cpp}` — load the real `.alo` + run each sub-mesh's OWN named game shader 1:1 via `ShaderManager`; `Engine::RenderSkydomeMesh`/`RenderSkydomes`), the nebula multi-submesh (3-way per-shader blend + phase-faithful compose + full SH/specular lighting), and the dual-slot plumbing (`engine/set/skydome-environment` + `engine/query/skydome-list` bridge + registry persistence + startup restore + a React two-section game-dome picker replacing the fake gradient tiles). Built clean (native Debug+Release x64, editor tsc+vite, **799 Vitest**, leaf 30/30+25/25, cold-launch smoke), 3 code reviews (caught + fixed an **inverted vertex-format mapping** — verified the correct `N`/`NU2`/`NU2C` prefix layouts vs max2alamo + the shader VS inputs), CI-green + merged. **⚠ The dome RENDER is UNVERIFIED — GUI-coupled, needs a visual feel-test** (point the editor at the FoC install/a mod, then the **Background → Game dome** picker or the debug env hook `ALO_MT15_TEST_DOME=Day_Blue_Sky ALO_MT15_TEST_CTX=land`; run with stderr visible for `[SkyEnv]`/`[AloVtx]`/`[SkyDraw]`). **NEXT TASK (not started — explicitly saved for this session): build the render-capture tool, OPTION A** — a `debug/capture-frame { path }` bridge kind + an engine method that `GetRenderTargetData` → `D3DXSaveSurfaceToFileW(... PNG)` (d3dx9 already linked; debug/`--test-host`-gated), so Claude can `Read` the rendered frame. It captures the REAL `Engine::Render()` pipeline (chosen over C, a divergent headless harness); combined with the existing `engine/set/paused` + `engine/action/step-frames` it enables deterministic **particle-lifecycle filmstrips** (pause→step→capture). Own branch/PR off `e710f47`. **After the capture tool + feel-test confirm MT-15 renders, do the deferred follow-ups:** `ROADMAP.md` MT-15 ✅ Shipped (#163) + *Actual:* + move to Shipped; the `CHANGELOG.md` entry; remove the `#ifndef NDEBUG` env-var bring-up driver (TODO in the `engine.cpp` Engine ctor); the Playwright two-section-picker spec. **Read FIRST:** `tasks/HANDOFF.md` top (session 43), `tasks/todo.md` §6 (the M1/M2/M3 execution log + review outcomes); memories `project_mt15_skydome_5a_closed`, `reference_max2alamo_format`, `reference_foc_install_dev_box`, `feedback_no_shader_fork_1to1_rendering`. **Open:** `src/SkydomeMesh.{h,cpp}`, `src/engine.cpp` (`RenderSkydomeMesh`/`RenderSkydomes` + the capture-target plumbing: the compositor RT / back-buffer), `src/host/BridgeDispatcher.cpp` (handler pattern for a path-returning query). **VERIFY claims against code.** Pre-flight: `git fetch origin master` + lineage (rebase if `HEAD..origin/master` ≠ 0); fresh worktree → L-039 (copy `~/.nuget/.../microsoft.web.webview2/1.0.3967.48/*` → `packages/Microsoft.Web.WebView2.1.0.3967.48/`) + L-040 (`pnpm -C web install` then `pnpm -C web --filter @particle-editor/editor build`); host Debug x64 via VS18 MSBuild (`C:\Program Files\Microsoft Visual Studio\18\Community\MSBuild\Current\Bin\MSBuild.exe`, L-046); leaf tests via the `.bat`s (create `tests\obj\` + build the host first so `expatw_static.lib` exists). **Heads-up:** `gh pr merge` needs explicit user OK per PR; MT-15 diagnostics go to STDERR (L-084); the DComp window-capture trap is *why* option A captures the D3D RT directly. **Summarize understanding + confirm scope before changing anything.**

### Kickoff (short)
> Pick up AloParticleEditor. **`origin/master` = `e710f47`.** Session 43 shipped **MT-15** (faithful game/mod skydome) all-in-one (#163): render core `SkydomeMesh` (real `.alo` + each sub-mesh's own game shader 1:1) + nebula (per-shader blend, phase-faithful compose, SH/specular) + dual-slot bridge/persistence/restore + React two-section game-dome picker (fake tiles removed). Clean builds + 799 Vitest + leaf 30/30+25/25 + 3 reviews (caught an inverted vertex-format mapping); CI-green + merged. **⚠ render UNVERIFIED — needs a visual feel-test** (editor → FoC install/mod, Background→Game dome picker or env `ALO_MT15_TEST_DOME=Day_Blue_Sky`; stderr shows `[SkyDraw]`). **NEXT (saved for this session): build the capture tool, OPTION A** — `debug/capture-frame`→PNG bridge+engine (GetRenderTargetData→D3DXSaveSurfaceToFileW, d3dx9 linked, debug-gated) so Claude can `Read` the render; + particle filmstrips via the existing pause/step-frames. Own branch off `e710f47`. After capture+feel-test confirm: do MT-15's deferred `ROADMAP` ✅Shipped + `CHANGELOG` + remove the env-var driver + Playwright spec. Read HANDOFF top (s43) + `tasks/todo.md` §6; memories `project_mt15_skydome_5a_closed`/`reference_max2alamo_format`/`reference_foc_install_dev_box`/`feedback_no_shader_fork_1to1_rendering`. Pre-flight: `git fetch`+lineage; fresh worktree → L-039/L-040; host Debug x64 (VS18); leaf `.bat`s (mk `tests\obj\` + build host first). VERIFY vs code; `gh pr merge` needs OK per PR.

---

## 2026-06-13 (session 42) — **De-risked + foundation-shipped [MT-15] (skydome from game/mod). Closed §5a against a REAL FoC install (found on the dev box at `D:\SteamLibrary\…\corruption`), then shipped the foundation across 3 merged PRs: the `.alo` static-mesh decoder (`AloModel`), the skydome XML reader (`SkydomeEnvironment`), a GREEN Step-0 device spike, and a pre-existing background-picker persistence fix. The two hardest unknowns are retired (the `.alo` format + ps_1_1 validation on D3D9Ex). NEXT: the render core (`SkydomeMesh` + `Engine::RenderSkydomes`), fully specified in `tasks/todo.md` §3.2.**

**`origin/master` = `91a2133`** (this session: #159 `247a716` bg-picker persistence → #160 `72d55bf` MT-15 foundation → #161 `91a2133` docs). Lineage was clean at session start (branched off `92026e1`). ⚠ Master had also advanced via **session 41** (not in this file): **MT-16 transparency parity #155** (`460c070`, a `D3DRS_COLORWRITEENABLE=RGB` alpha mask on the final scene-combine — no shader edit; memory `project_mt16_lt5_render_triage`) + the MT-15 scoping doc #157. MT-16 transparency is **Shipped**; its colour-grade/tone half is **[LT-8]**.

### ★ NEXT TASK (not started) — the MT-15 render core
Full plan in [`tasks/todo.md`](todo.md) §3 (esp. §3.2). **Branch fresh off `master` `91a2133`.** Build `SkydomeMesh` + `Engine::RenderSkydomeMesh`/`RenderSkydomes`: per-sub-mesh **multi-effect** draw — each sub-mesh runs its OWN game shader via `ShaderManager::getShader`/`Effect` (validated GREEN by Step-0); SH-lighting + cloud-scroll-`TIME` binding mirroring the particle template ([`src/engine.cpp:746-777`](../src/engine.cpp)); the FULL render-state save/restore delta set (L-032 class — **incl. the confirmed empty-state-block finding: `reference/foc-shaders/AlamoEngine.fxh` sets `ALAMO_STATE_BLOCKS 0`, so the game `.fxo` NO-OP their `SB_START…SB_END` blocks → the app MUST itself set `ZWriteEnable`/`ZFunc`/`AlphaBlendEnable`/`SrcBlend`/`DestBlend`/`CullMode` per sub-mesh**); primary+secondary draw order (space: secondary nebula behind via larger `Scale_Factor`+`Sort_Order_Adjust`; land: secondary = a partial overlay — rings/horizon/traffic); device lost/reset lifecycle (cache the decoded `.alo` blobs, refill DEFAULT-pool VB/IB on reset — no re-parse); integrate into `Engine::Render()` after Clear/viewport, before ground/particles. THEN dual-slot engine state (additive secondary beside the frozen `m_skydomeIndex`) + bridge schema + persistence (the #159 pattern, extended) + a React two-section picker. **This phase is GUI-coupled → needs the user's visual feel-test (the dome renders; I can't see it).**

**Decisions locked (user, this session):** full-faithful in ONE pass (no a1 textured-sphere phase); **replace** the 8 fake-gradient bundled tiles with the **real enumerated dome `Name`s** (keep solid-colour + custom-path fallback); **Land/Space toggle, default Space**; both slots default **Off**; `0x10005` old-vertex domes **graceful-skip**.

**⚠ Render-parity HARD RULE** (memory `feedback_no_shader_fork_1to1_rendering`): fidelity = run the GAME's REAL shaders 1:1, never fork/revise; particle shaders must NOT be touched; editor-furniture (ground/skydome) may be adapted. The skydome's faithful render — **load the real `.alo` + run each sub-mesh's own game `.fxo`** — IS 1:1 (not a fork).

### What shipped this session — 3 PRs, all CI-green + merged
1. **#159 (`247a716`) — Background-picker persistence** ([`src/host/BridgeDispatcher.cpp`](../src/host/BridgeDispatcher.cpp)). New-UI `engine/set/skydome-slot`/`-custom-path`/`background` handlers now write the registry (`SkydomeIndex` DWORD / `SkydomeCustomSlot%d` SZ / `BackgroundColor` DWORD under `HKCU\Software\AloParticleEditor`), mirroring the legacy writers value-for-value, gated like `settings/lighting-force-align/set`. Closes a pre-existing gap (new-UI dome/bg choice was lost on restart; `HostWindow` already READ those keys). `reset-view` and the color picker were verified NOT to over-write/churn the registry (reset uses engine-direct calls; `<input type=color>` uses `onChange`/commit).
2. **#160 (`72d55bf`) — MT-15 foundation (leaf cores + Step-0).** [`src/AloModel.{h,cpp}`](../src/AloModel.h) — decoder for the static-mesh+material subset of `.alo`: one `AloMesh` per `0x400`, each sub-mesh's shader name (`0x10101`) + typed material params (`0x10102-0x10106`) + raw **144-byte** vertex blob + uint16 indices kept verbatim; tolerantly skips skeleton/lights/connections + legacy `0x10005`; rejects malformed (stride≠144, count/payload mismatch, count overflow, truncation). [`src/SkydomeEnvironment.{h,cpp}`](../src/SkydomeEnvironment.h) — enumerates `{Land,Space}{Primary,Secondary}Skydomes.xml` via `IFileManager` (mod→base→MEG, canonical `Data\XML\` paths) + resolves a primary+secondary pair by `Name`; the `MapEnvironment` struct is **LT-8's** colour-grade extension point. Both **device-free leaf modules** (reused by the render core, LT-7 game-object import, LT-8). Wired into [`src/ParticleEditor.vcxproj`](../src/ParticleEditor.vcxproj). Tests: `tests/test_alo_model.cpp` (30) + `tests/test_skydome_environment.cpp` (25) + `tests/spike_skydome_technique.cpp` (Step-0). §5a closure recorded in `docs/superpowers/specs/2026-06-13-mt15-skydome-scope.md` **§0** + `tasks/todo.md`.
3. **#161 (`91a2133`) — docs** (CHANGELOG entry for the foundation + Step-0 marked PASSED in `tasks/todo.md` §3.0).

### §5a closed — the format truth (verified vs the REAL install)
- **The dev box HAS a FoC install** at `D:\SteamLibrary\steamapps\common\Star Wars Empire at War\corruption` (base data in MEGs: `config.meg`=XML, `models.meg`=`.alo`, `shaders.meg`=`.fxo`) + a **vanilla source extraction** (`Mods\Empire-at-War-Source-Files\src\{Base,FOC}\Data\XML`) + content mods (`Chelmod`, `EmpireAtWarExpanded`). **Corrects the recurring "no game install on the dev box" claim** (memory `reference_foc_install_dev_box`) — MT-14's `_bc`, LT-6 units, LT-7 meshes are now all verifiable here too.
- A skydome = an `.alo` **mesh + baked material**. The GameObject XML carries only `Name` + a model path (`Land_/Space_/Galactic_Model_Name`) + `Scale_Factor`/`Sort_Order_Adjust`/`In_Background` — **NOT** textures or cloud params (those live in the `.alo` material). Primary/secondary are **independent** `{Land,Space}` lists. **Route a1 (textured sphere) is DEAD**: textures aren't in the XML, and land-secondary overlays (rings/horizon/traffic) have no spherical form → must load the real mesh + run each sub-mesh's own shader.
- `.alo` format pinned via the **user's own MIT exporter `max2alamo-2026`** (`C:\Modding\max2alamo-2026`, memory `reference_max2alamo_format`): on-disk vertex stride is a **fixed 144 bytes** for all formats (color = float4 @offset 80; `0x10100` material + `0x10000` geometry are siblings under `0x400`). Validated against real `.alo` (clearblue `Skydome.fx`/180v/298p; nebula `MeshGloss.fxo`+`MeshAdditive.fx` multi-submesh).

### Step-0 device spike — GREEN (`tests/spike_skydome_technique.cpp`)
On a standalone D3D9Ex HAL device (vs_3_0/ps_3_0 GPU): `Skydome.fxo` (`sph_t2`) / `MeshAdditive.fxo` (`t0`) / `MeshGloss.fxo` (`sph_t0`) — all **DX8 / ps_1_1** — `ValidateTechnique`=OK + `Begin()`=1 pass → **RENDER**. The `Effect` ctor's LOD-annotation selection ([`src/Effect.cpp:39-55`](../src/Effect.cpp)) correctly **skips** MeshGloss's un-annotated `max_viewport`. **No FIXEDFUNCTION fallback / re-scope needed.**

### Verified baseline at `91a2133`
Host **Debug x64** clean (rebuilt repeatedly via VS18 MSBuild, L-046). Leaf-core unit tests: `tests/build_test_alo_model.bat` → **30/30**, `tests/build_test_skydome_environment.bat` → **25/25** (both also dump-validated against real install assets via an argv mode). Step-0 spike builds + runs GREEN (`tests/build_spike_skydome_technique.bat`). CI green on all 3 PRs (web Vitest + C++ x64 Debug/Release). **NOT run this session:** the web suite + native a11y harness (no web/UI changes — the foundation is C++-only). Fresh worktree → L-039 (NuGet copy) + L-040 (`pnpm -C web --filter @particle-editor/editor build`) — both done this session.

### Process notes / lessons this session
- **Precompiled ps_1_1 `.fxo` VALIDATE + render on D3D9Ex — even though the runtime D3DX compiler REJECTS ps_1_x SOURCE (X3539, L-084).** Critical for the render core: **load the game's precompiled `.fxo`** (the existing `ShaderManager`/`Effect` path) — do NOT compile ps_1_x `.fx` source. This is *why* running the game's real dome shaders 1:1 works.
- **MSVC `long` is 32-bit even on x64.** A 32-bit `(long)count * stride` product can overflow + bypass a size check (a code-review catch in `AloModel`). Use 64-bit (`unsigned long long`) products for any file-derived size validation, and bound counts before `resize()`.
- **`small` is a Windows macro** (`rpcndr.h`: `#define small char`) — unusable as a C++ identifier in any TU that includes `<windows.h>` (hit + renamed in a test).
- **C++ leaf-core unit tests** build via standalone `tests/build_*.bat` (`cl` + the pure data-model TUs, no engine/D3D) — NOT in CI, consistent with the repo's other `test_*.cpp`. The host vcxproj build (which CI runs) compiles the new TUs. Each test exe has an **argv dump mode** that validates against real install assets (dev-box only).
- **Multi-agent workflow + code-review** drove this session: a Workflow generated the impl plan (verify→design→critique→synthesize); `/code-review high` over the leaf cores caught the overflow bug + 2 robustness gaps; a consolidated review over the full session diff confirmed no further code defects.

### Kickoff (full) — paste into a fresh session
> Pick up `new-particle-editor` (AloParticleEditor — Win32 host + WebView2/React + D3D9Ex-via-DComp particle editor for SW:EaW). **`master` is the trunk; `origin/master` = `91a2133`.** Session 42 closed **MT-15 §5a** against a REAL FoC install (on the dev box at `D:\SteamLibrary\…\corruption`) and shipped the MT-15 **foundation** across 3 merged PRs: **#159** (`247a716`, new-UI background-picker persistence), **#160** (`72d55bf`, the device-free leaf cores `AloModel` [.alo static-mesh+material decoder] + `SkydomeEnvironment` [skydome-XML reader] + a GREEN Step-0 device spike), **#161** (`91a2133`, docs). **The two hardest unknowns are retired:** the `.alo` mesh/material format (decoded + validated against real assets; on-disk vertex stride pinned to 144 B via the user's own MIT exporter `max2alamo-2026`) and **ps_1_1 technique validation on D3D9Ex** (Step-0 GREEN — the game dome shaders' DX8/ps_1_1 `.fxo` techniques validate + render). **NEXT (NOT started): the MT-15 render core** — `SkydomeMesh` + `Engine::RenderSkydomeMesh`/`RenderSkydomes`, fully specified in `tasks/todo.md` §3 (esp. §3.2): per-sub-mesh multi-effect draw (each runs its OWN game shader via `ShaderManager`/`Effect`), SH-lighting + cloud-scroll-`TIME` binding (mirror the particle template `engine.cpp:746-777`), the FULL L-032 render-state save/restore (**incl. the empty-state-block finding — `AlamoEngine.fxh ALAMO_STATE_BLOCKS 0` means the game `.fxo` no-op their state blocks, so the app sets blend/zwrite/zfunc/cull itself**), primary+secondary compose (space: secondary behind; land: secondary = partial overlay), device lost/reset (cache `.alo` blobs, refill DEFAULT VB/IB), `Engine::Render()` integration; THEN dual-slot state + bridge + persistence (extend #159) + a React two-section picker. **GUI-coupled → needs the user's visual feel-test.** **Decisions locked:** full-faithful one pass; real enumerated Names replace the fake tiles (keep solid-colour + custom-path fallback); Land/Space toggle default Space; both slots default Off; `0x10005` graceful-skip. ⚠ **Render-parity HARD RULE** (memory `feedback_no_shader_fork_1to1_rendering`): run the game's REAL shaders 1:1, never fork — the skydome's faithful path (load real `.alo` + run each sub-mesh's own `.fxo`) IS 1:1. **Read FIRST:** `tasks/HANDOFF.md` top (session 42), `tasks/todo.md` §3, `docs/superpowers/specs/2026-06-13-mt15-skydome-scope.md` §0; memories `project_mt15_skydome_5a_closed`, `reference_max2alamo_format`, `reference_foc_install_dev_box`, `reference_alo_viewer_mesh_loader`, `project_mt16_lt5_render_triage`, `project_mt14_ground_lighting`. **Then open:** `src/AloModel.h`, `src/SkydomeEnvironment.h`, `src/engine.cpp` (`RenderSkydome` ~2342, the lighting/TIME binding ~746-777, device lost/reset ~1540/1592), `src/Effect.cpp`/`src/main.cpp` (`ShaderManager::getShader` ~316). **VERIFY claims against code.** Pre-flight: `git fetch origin master` + lineage (rebase if `HEAD..origin/master` ≠ 0 — parallel sessions move master); fresh worktree → **L-039** (copy `~/.nuget/.../microsoft.web.webview2/1.0.3967.48/*` → `packages/Microsoft.Web.WebView2.1.0.3967.48/`) + **L-040** (`pnpm -C web install` then `pnpm -C web --filter @particle-editor/editor build`); host Debug x64 via VS18 MSBuild (`C:\Program Files\Microsoft Visual Studio\18\Community\MSBuild\Current\Bin\MSBuild.exe`, L-046); leaf-core tests via `tests/build_test_alo_model.bat` (30/30) + `tests/build_test_skydome_environment.bat` (25/25). **Heads-up:** precompiled ps_1_1 `.fxo` validate + render on D3D9Ex even though D3DX rejects ps_1_x SOURCE (L-084) — load the game `.fxo`, don't compile ps_1_x; MSVC `long` is 32-bit on x64 (use 64-bit size math); `gh pr merge` needs explicit user OK per PR (the classifier enforces it); the render core can only be confirmed by the user's visual feel-test. **Summarize understanding + confirm scope before changing anything.**

### Kickoff (short)
> Pick up AloParticleEditor. **`origin/master` = `91a2133`.** Session 42 closed MT-15 §5a (REAL FoC install on the dev box at `D:\SteamLibrary\…\corruption`) + shipped the MT-15 foundation: **#159** bg-picker persistence, **#160** leaf cores (`AloModel` .alo decoder + `SkydomeEnvironment` XML reader + Step-0 spike GREEN), **#161** docs. `.alo` format pinned (144 B vertex stride) via the user's `max2alamo-2026` exporter; **Step-0 GREEN** — the game's ps_1_1 dome `.fxo` validate + render on D3D9Ex. **NEXT (not started): the MT-15 render core** — `SkydomeMesh` + `Engine::RenderSkydomes` per `tasks/todo.md` §3.2 (per-sub-mesh draw running each game shader, SH+cloud-`TIME`, full L-032 state save/restore **incl. the empty-state-block finding**, primary+secondary compose, device lifecycle; then dual-slot + bridge + persistence + React picker). **GUI-coupled → user feel-test.** Decisions locked: full-faithful one pass; real Names replace fake tiles; Land/Space toggle default Space; slots default Off; `0x10005` skip. ⚠ **HARD RULE** (`feedback_no_shader_fork_1to1_rendering`): run the game's REAL shaders 1:1 — the skydome path (load real `.alo` + run each sub-mesh's own `.fxo`) IS 1:1. Read HANDOFF top (s42) + `tasks/todo.md` §3 + scope doc §0; memories `project_mt15_skydome_5a_closed` / `reference_max2alamo_format` / `reference_foc_install_dev_box`. Open `src/AloModel.h`, `src/SkydomeEnvironment.h`, `src/engine.cpp`, `src/Effect.cpp`. Pre-flight: `git fetch` + lineage; fresh worktree → L-039/L-040; host Debug x64 (VS18); leaf tests via the `.bat`s. ps_1_1 `.fxo` validate on D3D9Ex (load, don't compile); MSVC `long`=32-bit; `gh pr merge` needs OK per PR. VERIFY vs code; summarize before changing.

---

## 2026-06-13 (session 40) — **Shipped [MT-14]: the ground plane now responds to the Lighting panel via a game-faithful **bump-mapped terrain** render (#151, `056e2ed`; CHANGELOG hash backfilled #153). A triage-first item that grew — by user direction — from a ~3h "feed the light into the ground draw" fix into a net-new bundled shader (`GroundLit.fx`) + effect pipeline + runtime normal-map resolution. User-feel-tested (one iteration fixed a specular blowout) + merged. NEXT: the rest of the queued render-fidelity batch (MT-15, MT-16, LT-5/6/7) — MT-14 + MT-17 now done.**

**`origin/master` = `a510da5`** (this session added MT-14 #151 `056e2ed` + hash-backfill #153 `fb76afc`). ⚠ **Master also advanced via PARALLEL sessions** this period: **MT-17 #149** (`87a8654`, spawner path → arc+squiggle) + backfill #150, and a **render-fidelity triage** (memory `project_mt16_lt5_render_triage`, **no code** — MT-16/LT-5 fix deferred to a future "recreate the game pipeline" session). My MT-14 branch was rebased onto the updated master mid-session (only scratch `tasks/todo.md` collided — resolved to the MT-14 plan). At handoff the only open item is **this docs-only handoff PR**.

### ★ NEXT TASK (not started) — the remaining queued roadmap batch
The session-39 batch minus what's shipped (MT-14 ✅ #151, MT-17 ✅ #149):
- **[MT-15]** Skydome from the game/mod, primary+secondary — `ROADMAP.md` §2.1.
- **[MT-16]** Editor↔in-game transparency parity — §2.2. **Triaged (no code); read below.**
- **[LT-5]** Particle shaders (transparent-depth lighting + bump-map lighting∧colorization) — §3.
- **[LT-6]** Distance-unit parity → projectile-motion emulation. **[LT-7]** In-preview scale refs (depends on LT-6).

**⚠ Render-parity HARD RULE (user — memory `feedback_no_shader_fork_1to1_rendering`).** Editor↔in-game fidelity must come from running the GAME's REAL shaders + recreating its pipeline — **NOT** from forking/approximating/revising shaders. **The particle shaders (`Engine\Prim*.fx`) are already 1:1 (ShaderManager loads them from the game/mod) and MUST NOT be revised during parity work** (user reconfirmed this session). Adapting *editor-furniture* shaders (the ground `GroundLit.fx`, skydome) is explicitly fine. MT-16/LT-5 are scoped as a LARGE future session that recreates the game's `SceneComposite/Scene_*.fx` composite (multi-RT, per-map colour grade — needs map-env plumbing, ties to MT-15), NOT a patch to the bundled `SceneHeat.fx`. **Read `project_mt16_lt5_render_triage` before touching MT-16/LT-5.** FoC shader source is vendored in-repo at `reference/foc-shaders/`.

### What shipped this session — MT-14 (#151, `056e2ed`), feel-tested + merged
The ground plane was drawn fixed-function with `D3DRS_LIGHTING` off + a hardcoded white diffuse — fully inert (the triage answer: *excluded* from lighting, world lighting never broken). Now: new bundled effect [`GroundLit.fx`](../src/Resources/Engine/GroundLit.fx) (`IDR_SHADER_GROUND_LIT`), a **per-pixel world-space** adaptation of the game's `TerrainRenderBump` path — SPH **fill** per-vertex, dot3 sun off a tangent-space normal map, **gloss-gated specular with the gloss in the `_bc` map's alpha**, diffuse ×2 / specular ×1, cloud/FOW dropped. Driven by `Engine::RenderGroundLit()` + `m_pGroundEffect`, **mirroring the skydome effect lifecycle** (`D3DXCreateEffect` from RCDATA, handles, OnLost/OnReset, compile-fail → graceful-degrade to the old unlit quad). Reuses the engine's live `m_sphLightFill` / `m_lights[0]`. Normal maps resolve `<base>_bc.dds` from the game/mod via `FileManager`; matte 1×1 flat-normal fallback (alpha 0 → no specular) otherwise. Petroglyph FoC shader source vendored at `reference/foc-shaders/`. Design/triage trail: `tasks/todo.md` (MT-14 plan + Review).

### Open follow-up (MT-14) — NOT blocking
**Real `_bc` normal-map path UNVERIFIED.** The dev box has no game/mod, so every ground slot read the matte flat-fallback. The `<base>_bc.dds` naming + the gloss-in-`_bc`-alpha convention are per the user's spec — validate per-pixel relief + gloss-gated specular against a real mod. A wrong slot is a one-line fix in `Engine::ReloadGroundNormalTexture` (the slot→leaf map).

### Verified baseline at `a510da5`
Host **Debug + Release x64** clean (benign LNK4098); web **795/795**; native harness **181 passed / 30 skipped** (no golden shift — engine-only change); CI green on #151 (web Vitest + C++ x64 Debug/Release). Fresh worktree → L-039 (NuGet copy) + L-040 (`pnpm build`).

### Process notes / lessons this session
- **L-084** — the runtime D3DX compiler rejects ps_1_x/vs_1_x (`X3539`); target **vs_2_0/ps_2_0** minimum. A new bundled `.fx` only fails at *runtime* (D3DXCreateEffect), so a clean host build ≠ working shader — **cold-launch the Debug exe with STDERR redirected and grep for the compile error** (engine diagnostics + effect-compile failures go to stderr; this GUI exe's **stdout isn't captured** when launched detached). Caught the X3539 in ~6 s, no `dist` needed.
- **L-085** — porting a game shader: the **gloss-alpha channel** AND the **×1-vs-×2** multiplier differ across sibling variants (`TerrainMeshBump` spec ×2 from base.a vs the heightmap `TerrainRenderBump` spec ×1, gloss in the `_bc`/diffuse alpha). Match the variant the game uses for *that surface*. For a single large flat quad, evaluate lighting **per-pixel in world space** (the per-vertex half-vector smears specular). Neutral fallbacks must make the term **vanish** (flat normal alpha 0, not 1).
- **Parallel sessions are LIVE.** Master jumped 4 commits mid-session (MT-17 #149 + #150). Always re-check `HEAD..origin/master` before committing/PR and rebase if non-zero. The auto-memory also gained parallel-session entries — trust-but-verify against the repo (e.g. the MT-17-shipped memory matched a real master merge; verify any memory's file/flag claims still exist).
- **The master-merge gate is enforced per-PR by the auto-mode classifier.** It denied auto-merging the docs backfill #153 because the user had only OK'd #151 — *every* master merge needs its own explicit OK, including mechanical hash backfills.

### Kickoff (full) — paste into a fresh session
> Pick up `new-particle-editor` (AloParticleEditor — Win32 host + WebView2/React + D3D9Ex-via-DComp particle editor for SW:EaW). **`master` is the trunk; `origin/master` = `a510da5`.** Session 40 shipped **[MT-14]** — the ground plane now responds to the Lighting panel via a game-faithful **bump-mapped terrain** render (#151, `056e2ed`): new bundled `GroundLit.fx` (per-pixel world-space adaptation of the game's `TerrainRenderBump`; SPH fill + dot3 sun off a normal map; **gloss in the `_bc` map's alpha**; diffuse ×2 / spec ×1; cloud/FOW dropped), driven by `Engine::RenderGroundLit()` + `m_pGroundEffect` mirroring the skydome effect lifecycle, normal maps resolved `<base>_bc.dds` from the game/mod via `FileManager` with a matte flat-normal fallback. User-feel-tested + merged; CHANGELOG hash backfilled (#153). **Parallel sessions also shipped MT-17 (#149) + did an MT-16/LT-5 render triage (no code).** **NEXT (not started): the remaining queued batch — MT-15 (skydome), MT-16 (transparency parity), LT-5 (particle shaders), LT-6 (unit parity), LT-7 (scale refs); ask the user which.** ⚠ **Render-parity HARD RULE** (memory `feedback_no_shader_fork_1to1_rendering`): fidelity comes from running the GAME's real shaders + recreating its pipeline, never forking/revising them; **the particle shaders are already 1:1 and must NOT be revised during parity work**; adapting editor-furniture shaders (ground/skydome) is fine; MT-16/LT-5 = a large "recreate the `Scene_*.fx` composite" future session — **read `project_mt16_lt5_render_triage` first**. Also read `tasks/HANDOFF.md` top (session 40) + the matching `ROADMAP.md` entries; memory `project_mt14_ground_lighting`. **MT-14 open follow-up:** the real `_bc` relief path is unverified (no mod on the dev box). VERIFY claims against code. Pre-flight: `git fetch origin master` + lineage (parallel sessions move master — rebase if `HEAD..origin/master` ≠ 0); fresh worktree → L-039 (NuGet copy) + L-040 (`pnpm build`); from `web/` `pnpm install`, `pnpm --filter @particle-editor/editor test` → **795**, `tsc -b` 0; host Debug x64 (VS18 MSBuild, L-046); native harness → 181/30-skipped if a task needs it (overload specs flake at the tail — L-066, re-run in isolation). **Heads-up:** `gh pr merge` needs explicit user OK **per PR** (the classifier enforces it, even on backfills); a new `.fx` only compiles at runtime — cold-launch + stderr smoke-check (L-084); HWND/UIA JSON a11y goldens are dormant/stale by design. Summarize understanding + confirm scope before changing anything.

### Kickoff (short) — session 40 (superseded by session 42 below; kept for history)
> Pick up AloParticleEditor. **`origin/master` = `a510da5`.** Session 40 shipped **MT-14** — the ground plane responds to lighting via a bump-mapped terrain render (#151; `GroundLit.fx`, per-pixel world-space, gloss in the `_bc` alpha, spec ×1, matte fallback; skydome-effect-pattern lifecycle); feel-tested + merged, hash backfilled #153. Parallel sessions shipped MT-17 (#149) + triaged MT-16/LT-5 (no code). **NEXT (not started): remaining queued batch — MT-15 / MT-16 / LT-5 / LT-6 / LT-7; ask which.** ⚠ **HARD RULE** (memory `feedback_no_shader_fork_1to1_rendering`): render parity = run the game's REAL shaders + recreate its pipeline; **particle shaders are 1:1 and must NOT be revised during parity work**; editor-furniture shaders (ground/skydome) may be adapted; MT-16/LT-5 = large "recreate `Scene_*.fx`" future session — read `project_mt16_lt5_render_triage`. MT-14 open: real `_bc` path unverified (no mod on box). Read HANDOFF top (s40) + ROADMAP. Pre-flight: re-check `HEAD..origin/master` (parallel sessions move master); web **795**, `tsc -b` 0, host Debug x64 (VS18; fresh worktree → L-039/L-040); a new `.fx` compiles only at runtime → cold-launch stderr smoke-check (L-084). VERIFY against code; `gh pr merge` needs explicit OK per PR.

---

## 2026-06-12 (session 39) — **Shipped the open-file titlebar + "Particle Editor" rebrand (#142), then a brainstorm-driven emitter-tree link-group rework — spine + tint + `G{n}` badge replacing the dot & bracket gutter (#146). Plus a roadmap sweep: 7 new render-fidelity / preview-accuracy items flagged for next week (#144) and the old spawner-v2 goal (LT-1) retired (#145). All user-feel-tested + merged; CHANGELOG hashes backfilled (#143/#147). NEXT: the user's queued roadmap batch (MT-14…17, LT-5…7) — "pick up next week," not started.**

**`origin/master` = `625b532`** (advanced this session from `5fc55b5`, via merges #142→#147). At handoff the only open item is **this docs-only handoff PR** (`claude/docs-session39-handoff`). All feature branches merged; nothing else of ours in flight. ⚠ A **stale pre-existing PR #125** ("docs: session-36 handoff") is still open — NOT from this session; close or ignore it.

### ★ NEXT TASK (start here, NOT started) — the user's queued roadmap batch

The user spent the end of the session flagging work to "pick up next week." All seven are written up in [`ROADMAP.md`](../ROADMAP.md) with descriptions, difficulty, effort, and cross-refs. **Tier placement was a judgement call — the user may re-tier; confirm scope before building each.** They cluster around render fidelity + preview accuracy:

- **[MT-14] Ground-plane lighting** — triage: is the ground excluded from lighting, or is world lighting broken? Fix follows the root cause.
- **[MT-15] Skydome background from the game/mod** — read & render the actual **primary + secondary** skydomes (like the Petroglyph map editor), replacing the manual `BackgroundPicker` texture choice.
- **[MT-16] Editor↔in-game transparency parity** — particles look darker/more opaque in-game; root-cause (blend op / gamma-sRGB / premultiplied alpha / depth). Coupled to LT-5.
- **[MT-17] Spawner jitter → path perturbation** (arc/squiggle), not exit-velocity randomisation. (Absorbed the old LT-1 arc-path idea.)
- **[LT-5] Particle shaders** — verify the transparent-depth shader inherits world lighting; fix the bump-mapped shader's lighting-AND-colorization-together problem (adjacent — touches the game's `.fx`).
- **[LT-6] Distance-unit parity → projectile-motion emulation** — foundation: make an editor unit == a game world unit; then import a game projectile so the spawner emulates its in-game motion.
- **[LT-7] In-preview scale references** — unit grid at game-unit spacing + import game-object meshes for scale (depends on LT-6).

**MT-13 is reserved** for the greenlit legacy/arch-A removal (see `project_daily_driving_legacy` memory) — don't reuse it. No task is force-assigned; **ask the user which of these to start.**

### What shipped this session — all on `master`, all user-feel-tested + merged

1. **#142 (`8dc8256`) — Open file's name in the titlebar + "Particle Editor" rebrand.** The Win32 titlebar shows the open `.alo`: `Untitled.alo — Particle Editor` fresh, `name.alo — Particle Editor` open, leading `●` when dirty (filename only, visible in taskbar/Alt-Tab). App rebranded "AloParticleEditor" → "Particle Editor" across **visible UI only** (titlebar, `index.html` `<title>`, About dialog, host MessageBoxes), and the brand label next to **File** removed (menu bar starts at File). **Architecture:** React owns the title string (pure [`formatWindowTitle`](../web/apps/editor/src/lib/window-title.ts) + the App.tsx effect set `document.title`); the host MIRRORS it into the titlebar via a new `DocumentTitleChanged` → `SetWindowTextW` handler in [`HostWindow.cpp`](../src/host/HostWindow.cpp) `FinishWebView2ControllerSetup` (token + teardown unsubscribe mirroring `accelKeyTok`). **Hard boundary held:** storage/identity strings (`HKCU\Software\AloParticleEditor`, `%LOCALAPPDATA%`/`%TEMP%\AloParticleEditor`, window class, `AloParticleEditor-DevProbe` UA) keep the old name — renaming them orphans user settings/recents/autosaves. Spec/plan `docs/superpowers/{specs/2026-06-11,plans/2026-06-12}-open-file-titlebar-rebrand*`.
2. **#144 (`1402d15`) — 7 roadmap items flagged** (the NEXT TASK list above). **#145 (`e553d6f`) — retired [LT-1]** (programmable spawner v2): superseded by MT-17 (jitter rework, which absorbed LT-1's arc-paths) + LT-6 (projectile emulation); long-term positions renumbered 3.1–3.5, LT-1 tag vacated.
3. **#146 (`73727f2`) — Emitter-tree link-group indicator reworked.** Started as "move the dot to the right" (an interim commit on the same branch did exactly that), then — via the visualize-tool mockups — landed on **spine + tint + `G{n}` badge** at fixed positions. Linked rows get a group-coloured left border (spine), a soft inline `backgroundColor` wash (12% rest / 20% on group-hover, gated to linked-AND-unselected so selection wins), and an outlined `G{n}` chip at the end of the name. Clicking the spine (an 8px hit-strip) **or** the badge selects the whole group (new `onSelectLinkGroup` prop → existing `handleSelectLinkGroup`); LNK-6 group-hover preserved. **Retired the entire bracket gutter** — it was absolutely positioned at the *measured longest name*, so one long name shoved every bracket out (the reported bug). Deleted `computeLinkGroupBrackets`, `laneCount`, the longest-name `useLayoutEffect`, and the gutter layer (net −305 lines). All in [`EmitterTree.tsx`](../web/apps/editor/src/screens/EmitterTree.tsx) + [`link-group-colors.ts`](../web/apps/editor/src/lib/link-group-colors.ts).

### Verified baseline at `625b532`
Web **795/795** (`pnpm --filter @particle-editor/editor test` from `web/`; was 806 — net −11 as #146 swapped 11 dot/bracket tests for 5 badge/spine tests, and earlier the +6 window-title tests landed), `tsc -b` 0, vite build clean; host **Debug + Release x64** clean (benign LNK4098/C4244). **CI green on all three legs for #142 and #146** (web Vitest + C++ x64 Debug/Release). #142 was natively verified (the `file-ops.spec.ts` title specs run the real host; a11y goldens regenerated for the rebrand). Fresh worktree → L-039 (NuGet copy) + L-040 (`pnpm build`).

> ⚠ **#146's native a11y goldens were NOT re-run** (the change is web-only). The basis for "goldens unchanged" is that the new spine/badge cues are `aria-hidden` (same posture as the retired bracket), so the accessibility tree is unaffected — verified by jsdom unit tests, not the native UIA harness. If the next session touches this and wants certainty, run the native harness (`cd web/apps/editor && node ./scripts/run-native-tests.mjs`) and confirm the emitter-tree composition golden is byte-stable.

### Process notes / lessons this session
- **`gh pr merge --auto` does NOT gate on green without branch protection.** This repo has no required-status-check rule on `master`, so `--auto --merge` fell through to an **immediate merge** (#142 landed while CI was still running). CI passed anyway (verified after), but "auto-merge when green" isn't real here. To get true gating, add a branch-protection rule requiring the three checks; otherwise watch `gh pr checks <n> --watch` post-merge.
- **The spec's "UIA goldens prove the SetWindowText mirror" claim was wrong** and was corrected mid-execution (spec commit `aeccab7`): the HWND/UIA (JSON) golden lane **auto-skips** in the default composition hosting mode (`ALO_HOSTING_MODE !== "legacy"`), and the composition (YAML) lane is a DOM snapshot with no Win32 window-Name node. The mirror is instead proven by `file-ops.spec.ts`'s `document.title` against the real host + a cold-launch `Get-Process … MainWindowTitle` read (which returned `Untitled.alo — Particle Editor`). The dormant HWND-lane JSON goldens were left stale (`AloParticleEditor`) — known, non-CI-gated rebrand debt.
- **Mockup-driven UI iteration worked well:** the visualize tool (inline light+dark mockups) drove the link-group design from dot→spine→spine+tint+badge across two rounds before any code — cheap, and the user steered each step.
- **`DEVELOPMENT_LOG.md` is dormant** (last entry 2026-05-16 / #79). The living per-PR history is `CHANGELOG.md`; per-session narrative is this file. Don't resurrect DEVELOPMENT_LOG without the user asking.

### Kickoff (full) — paste into a fresh session
> Pick up `new-particle-editor` (AloParticleEditor — Win32 host + WebView2/React + D3D9Ex-via-DComp particle editor for SW:EaW). **`master` is the trunk; `origin/master` = `625b532`.** Session 39 shipped the open-file titlebar + "Particle Editor" rebrand (#142, `8dc8256`) and a brainstorm-driven emitter-tree link-group rework — spine + soft tint + `G{n}` badge replacing the per-row dot and the longest-name-measured bracket gutter (#146, `73727f2`) — plus a roadmap sweep (#144 flagged 7 items; #145 retired LT-1); all feel-tested + merged, CHANGELOG hashes backfilled. **NO task is force-assigned — the user queued a roadmap batch for "next week": MT-14 ground-plane lighting triage, MT-15 game/mod skydome (primary+secondary), MT-16 editor↔in-game transparency parity, MT-17 spawner jitter→path, LT-5 particle shaders (transparent-depth lighting + bump-map lighting/colorization), LT-6 distance-unit parity→projectile emulation, LT-7 in-preview scale refs (grid + game-object import). ASK THE USER which to start; confirm tier/scope (placement was a judgement call).** Read `tasks/HANDOFF.md` top (session 39) + the matching `ROADMAP.md` entries; for the titlebar work see `docs/superpowers/{specs/2026-06-11,plans/2026-06-12}-open-file-titlebar-rebrand*`. Memory: `project_open_filename_in_ui` (shipped; rebrand storage-boundary + title-mirror architecture), `project_daily_driving_legacy` (MT-13 = legacy removal, reserved). VERIFY claims against code. Pre-flight: `git fetch origin master` + lineage; fresh worktree → L-039 (NuGet copy) + L-040 (`pnpm build`); from `web/` `pnpm install`, `pnpm --filter @particle-editor/editor test` → **795**, `tsc -b` 0; host Debug x64 (VS18 MSBuild, L-046); native harness → ~180/0 if a task needs it (the overload specs flake at the tail — L-066, re-run in isolation). **Heads-up:** `gh pr merge --auto` merges IMMEDIATELY here (no branch protection), and the HWND/UIA JSON a11y goldens are dormant/stale by design. Summarize understanding + confirm scope before changing anything.

### Kickoff (short)
> Pick up AloParticleEditor. **`origin/master` = `625b532`.** Session 39 shipped the open-file titlebar + "Particle Editor" rebrand (#142) and an emitter-tree link-group rework — spine + tint + `G{n}` badge, bracket gutter retired (#146) — plus roadmap flags (#144) and LT-1 retirement (#145); all merged. **NEXT (not started): the user's queued roadmap batch — MT-14…17 + LT-5…7 (render fidelity + preview accuracy), "next week." ASK which to start + confirm tier/scope.** Read HANDOFF top (s39) + ROADMAP. Pre-flight: web **795**, `tsc -b` 0, host Debug x64 (VS18; fresh worktree → L-039/L-040). VERIFY against code. Note: `gh pr merge --auto` merges immediately here (no branch protection); HWND/UIA JSON a11y goldens are dormant.

---

## 2026-06-11 (session 38) — **The two #138 overload feel-test bugs SHIPPED in one web-only PR [#140] (`541c79a`): the ⚠ glyph now tracks the configurable cap, a predictive system-load chip covers the system-total gap, and the refusal banner no longer flashes stale latch copy on exit. User-feel-tested + merged. CHANGELOG hash-backfilled + this handoff in the docs-only follow-up PR. NEXT (user-requested at session end, NOT started): show the currently-open particle file's name in the UI (the user suggested the titlebar).**

**`origin/master` = `541c79a`** (#140 merge; advanced this session from `b6f81a8` — the session-37 backfill sweep #139 merged first). At handoff the only thing open is the **docs-only backfill+handoff PR** (`claude/docs-session38-backfill` — backfills the #140 CHANGELOG hash + adds this entry; awaits merge). All feature branches merged; nothing else in flight.

### ⏭️ NEXT TASK (start here, NOT started) — show the open file's name in the UI

**User request (session-38 close, verbatim intent):** *"I want the UI to display somewhere (potentially the titlebar) the name of the particle file that is currently open."* No design done yet — **brainstorm placement with the user first** (the user floated the titlebar but left it open). Scoping notes to verify against code before proposing:

- **What "open file" means here.** The editor loads/saves `.alo` particle files. Find the source of truth for the current document path: grep the host (`src/host/`) and web for the open-file path / `file/open` / `file/save` / dirty-flag plumbing (there's a `markDirty`/doc-dirty concept referenced in session-33's drag-audit notes, and `file/new` / `engine/action/clear` bridge commands). The window title is owned by the **Win32 host** (`src/host/HostWindow.cpp` — `SetWindowText`/the main window proc), so a *titlebar* display is a host-side change; a *web/UI* display (e.g. a MenuBar/StatusBar slot) is a React change fed by a bridge field. Decide which surface with the user — they're different code paths.
- **Candidate web surfaces** if not the titlebar: the `MenuBar` (top of the app), the `StatusBar` ([`web/apps/editor/src/components/StatusBar.tsx`](../web/apps/editor/src/components/StatusBar.tsx) — already a readout row; has an import-path cell precedent), or a document-title element. The StatusBar already shows passive readouts and lost its tooltips in the NT-12 sweep, so it's a natural passive home.
- **Dirty indicator?** Common idiom is `filename.alo •` / `filename.alo*` when unsaved — ask the user if they want the dirty marker too (likely yes; it pairs naturally with showing the name).
- **Untitled/new-doc state.** Decide the label for a never-saved doc (`Untitled.alo`? `(unsaved)`?) — ask.

Brainstorm → spec → plan → TDD + subagent-reviewed, same rhythm as session 38. Likely small, but titlebar-vs-web is a real fork that changes the whole approach.

### What shipped this session — on `master` via #140 (`541c79a`), web-only, user-feel-tested + merged

**Overload indicator consistency** (the two #138 feel-test bugs). Spec [`docs/superpowers/specs/2026-06-11-overload-indicator-consistency-design.md`](../docs/superpowers/specs/2026-06-11-overload-indicator-consistency-design.md), plan [`docs/superpowers/plans/2026-06-11-overload-indicator-consistency.md`](../docs/superpowers/plans/2026-06-11-overload-indicator-consistency.md). **Entirely web-side — ZERO C++.** Three parts:

1. **Cap-tracking ⚠ glyph.** [`estimateChainLoad`](../web/apps/editor/src/lib/chain-load.ts) gained an optional `threshold`; [`EmitterTree.tsx`](../web/apps/editor/src/screens/EmitterTree.tsx) passes `guard.enabled ? maxParticles : undefined` so the per-row glyph warns at the configurable cap (guard-off → 10k advisory). **Semantic shift (documented):** with a cap > 10k the glyph means "*will be gated*," not "heavy" — effects between 10k and the cap lose the old advisory glyph.
2. **Predictive `SystemLoadChip`** ([`web/apps/editor/src/components/SystemLoadChip.tsx`](../web/apps/editor/src/components/SystemLoadChip.tsx)) above the emitter rows. Visible ⟺ `(instances + 1) × estimateSystemLoad > cap` — **byte-for-byte the #138 gate's refusal condition** ([engine.cpp:202](../src/engine.cpp), unchanged). Covers the two multipliers the per-row glyph can't (sum-across-roots, × placed-instances). Subscribes to `stats/tick` itself, confining the 4 Hz re-render to that leaf. Two copy variants (pre-first-placement vs. prospective).
3. **Banner exit fix.** Root cause was NOT engine-side (the handoff's `Engine::Clear()` latch reset already existed at [engine.cpp:247](../src/engine.cpp)) — it was the web render layer: [`OverloadBanner.tsx`](../web/apps/editor/src/components/OverloadBanner.tsx)'s `refusal ?? latch` ternary fell through to the latch copy during the 150 ms `usePresence` exit fade. Fix = freeze the rendered variant at exit start (render-time `lastShownRef`) + `setOverload(false)` on refusal (delivery-order belt-and-suspenders; a genuine latch re-asserts on the next 4 Hz tick).

**Reactivity glue:** new [`useOverloadGuardConfig`](../web/apps/editor/src/lib/overload-guard.ts) hook makes the localStorage guard config reactive via an `alo:overload-guard-changed` `CustomEvent` dispatched inside `writeOverloadGuard` (one source of truth — glyph, chip, engine cap all track the same `maxParticles`, updating live on a Preferences edit, no reload).

### Verified baseline at `541c79a`
Web **800/800** (`pnpm --filter @particle-editor/editor test` from `web/`; was 780 + 20 new), `tsc -b` 0, vite build clean; host Debug + Release x64 clean (benign LNK4098); CI green on all three legs (#140). a11y goldens **byte-stable** (the chip/glyph render null at the default 10k cap, so zero golden churn). Fresh worktree → L-039 (NuGet copy) + L-040 (`pnpm build`).

> ⚠ **Native `preview-overload.spec.ts` flakes at the FULL-HARNESS TAIL, not on its own.** In this session the 2 overload specs (cumulative-gate `:181` + edit-time `:229`) failed 1–2 of 3 full `a11y` runs with the host exiting on SIGTERM — the **L-066 cumulative-Debug-host-pressure** class (these specs run last). They pass **5/5 in isolation** (`cd web/apps/editor && node ./scripts/run-native-tests.mjs --grep "overload"` → 5 passed, 11.3 s, exit 0). The SIGTERM is a red herring — it's the runner's normal host teardown and appears even on the clean isolated exit-0 run. The change is web-only and never touches the engine gate these specs drive, so this is environmental, not a regression. **If a full `a11y` run shows only `preview-overload` specs failing with SIGTERM, re-run those specs in isolation to confirm green before assuming a break.** (Note: `pnpm … a11y -- --grep X` mangles the `--`; invoke the runner via `node` directly to pass `--grep`.)

### Process notes this session
- **Plan test values can be wrong against the mock fixture.** The plan's cap-tracking glyph tests used Smoke at 2000/s; the mock fixture's child emitters carry default multipliers (`Smoke puff` id 2 = 10/s × 3 s = ×30) that push the chain over 10k regardless of cap, so the guard-disabled assertions could never pass. The implementer caught it and re-based the tests on 200/s, cap 100; the fixture math is now documented in the test comments. **Lesson:** when writing native/mock-fixture-coupled test values in a plan, compute the fixture's actual decorated estimates, don't assume the patched root value is the whole chain.
- **The handoff's root-cause hypothesis was half-right.** It correctly localized banner-bug-2 to a staleness window but prescribed an engine-side `Engine::Clear()` latch reset that already existed; verifying against code (not just trusting the handoff) relocated the fix to the web render layer. Standard "trust but verify the handoff" win.
- **Subagent-driven execution worked cleanly:** 6 implementation tasks, each fresh-implementer + spec-review + quality-review, plus a final opus whole-implementation pass (verdict READY, chip⟺gate parity confirmed byte-for-byte). Three minor review nits were applied (explicit effect cleanup, a boundary test, a `null`-frozen-variant comment); the rest were below the blocking bar.

### Kickoff (full) — paste into a fresh session
> Pick up `new-particle-editor` (AloParticleEditor — Win32 host + WebView2/React + D3D9Ex-via-DComp particle editor for SW:EaW). **`master` is the trunk; `origin/master` = `541c79a`.** Session 38 shipped the two #138 overload feel-test bugs in one web-only PR [#140] (`541c79a`): cap-tracking ⚠ glyph, predictive `SystemLoadChip`, and the refusal-banner exit-fade fix — all feel-tested + merged; CHANGELOG hash-backfilled. **NEXT (user-requested, NOT started): show the currently-open particle file's name in the UI** — the user floated the **titlebar** but left placement open. Read `tasks/HANDOFF.md` top (session 38, the "NEXT TASK" block) for the scoping notes: the window title is host-owned (`src/host/HostWindow.cpp` → `SetWindowText`) so a titlebar display is a C++ change, while a web display (MenuBar / [`StatusBar.tsx`](../web/apps/editor/src/components/StatusBar.tsx)) is a React change fed by a bridge field — **brainstorm the surface with the user FIRST (it's a real fork), and ask about a dirty marker + the untitled-doc label.** First find the open-file path / dirty-flag source of truth (grep host + web for `file/open`/`file/save`/`markDirty`). Also read `tasks/lessons.md` (L-033 feel-tests are user-launched; L-066 native-host-pressure tail flake; L-039/L-040/L-046 fresh-worktree). VERIFY claims against code. Pre-flight: `git fetch origin master` + lineage; fresh worktree → L-039 (NuGet) + L-040 (`pnpm build`); from `web/` `pnpm install`, `pnpm --filter @particle-editor/editor test` → **800**, `tsc -b` 0; host Debug x64 (VS18, L-046); native harness → ~180/0 (NEVER pin overload specs high; if only `preview-overload` specs fail with SIGTERM at the tail, re-run them in isolation — they pass 5/5 standalone). Brainstorm + spec + plan + TDD/subagent-reviewed. Summarize understanding + confirm scope before changing anything.

### Kickoff (short)
> Pick up AloParticleEditor. **`origin/master` = `541c79a`.** Session 38 shipped the two #138 overload feel-test bugs (web-only PR #140): cap-tracking ⚠ glyph, predictive `SystemLoadChip`, refusal-banner exit-fade fix — feel-tested + merged. **NEXT (not started): display the open particle file's name in the UI** — user floated the titlebar but left it open; titlebar = host C++ (`HostWindow.cpp` `SetWindowText`), web = React (MenuBar/`StatusBar.tsx`) fed by a bridge field — **brainstorm the surface FIRST**, ask about a dirty marker + untitled label, find the open-file/dirty source of truth. Read HANDOFF top (s38) + lessons (L-033; L-066 tail-flake → re-run overload specs in isolation; L-039/L-040). Pre-flight: web **800**, `tsc -b` 0, native ~180/0, host Debug x64 (VS18; fresh worktree → L-039/L-040). VERIFY against code; brainstorm scope first.

---

## 2026-06-11 (session 37) — **A long curve-editor + overload polish session: SEVEN features/fixes shipped to `master` (#126, #128, #130, #133, #134, #135, #136) plus the preemptive overload hard-guard (#138), all user-feel-tested + merged. CHANGELOG fully hash-backfilled (#127/#129/#131/#137 + the in-flight sweep #139). NEXT (user-reported in the #138 feel test, NOT started): TWO overload-guard polish bugs — the ⚠ glyph threshold doesn't track the configurable cap, and a stale "latch" banner is revealed under the transient refusal banner.**

**`origin/master` = `a50db90`** (#138 merge). At handoff the only thing open is the **docs-only CHANGELOG-backfill-sweep PR [#139]** (`claude/changelog-backfill-sweep` — backfills the #133/#134/#135/#138 hashes; CI running, awaits merge — merge it first thing). All feature branches merged; nothing else in flight.

### ⏭️ NEXT TASK (start here) — two overload-guard feel-test bugs (#138 follow-up)

User repro: set the guard cap to **1000** (Preferences → Preview), make the default emitter a continuous **2000 particles/sec** stream, attempt to spawn. Two issues, both root-caused this session (verify against code before fixing):

1. **⚠ glyph doesn't fire even though the spawn is blocked.** The chain-warning glyph uses a FIXED `CHAIN_WARN_THRESHOLD = 10_000` ([`web/apps/editor/src/lib/chain-load.ts:9`](../web/apps/editor/src/lib/chain-load.ts), used at `:60` in `estimateChainLoad`), but the hard-guard uses the CONFIGURABLE `maxParticles` cap ([`overload-guard.ts`](../web/apps/editor/src/lib/overload-guard.ts), default 10k, user set 1000). So a ~2000-estimate emitter is blocked by the gate (2000 > 1000) yet silent on the glyph (2000 < 10000) — inconsistent. **Fix direction:** make the glyph warn at the guard cap when the guard is enabled (thread the configured `maxParticles` into the glyph computation in [`EmitterTree.tsx`](../web/apps/editor/src/screens/EmitterTree.tsx) ~`:1276` where `estimateChainLoad(tree.root)` is memoized; the cap lives in the overload-guard config the web already reads). **Subtlety:** the glyph is PER-ROW cumulative; the gate is SYSTEM-TOTAL × placed-instance-count (`estimateSystemLoad`). For a single root emitter they coincide; for multi-emitter trees the gate can block on the system total while no single row crosses the threshold → a fully-consistent fix may also want a system-level indicator, not just per-row glyphs. Decide with the user.

2. **Stale "latch" banner revealed under the transient refusal banner (jarring).** `OverloadBanner.tsx` has ONE surface with two states: the runtime `overload` latch (state-driven, from `stats/tick`) and the transient `refusal` (event-driven, 5 s, from `engine/overload/refused`); `visible = refusal !== null || overload`, refusal copy takes precedence. **Root cause hypothesis (verify):** a spawn LEAKS in the estimate-push staleness window (spec §3.1 — the `useEstimatedLoadPush` hook lags the rate edit by a refetch round-trip), the leaked instance briefly latches the runtime banner, then the hook's high estimate lands → edit-time `Clear()` + refusal event → refusal banner stacks ON TOP → after its 5 s the latch banner (now stale, instances were cleared) is revealed and fades. **Fix direction:** force-clear the runtime latch when a refusal/`Clear()` fires (the preview was emptied, so the latch is stale) — either engine-side (does `Engine::Clear()` reset the overload latch/`m_spawnBudget`? check `src/engine.cpp` `Clear()`), or web-side (`setOverload(false)` on the `engine/overload/refused` event in `OverloadBanner.tsx`), and/or suppress the latch during + briefly after a refusal window. Add a banner test for the overlap.

Both are small, web-leaning (issue 2 may want an engine-side latch reset). Brainstorm issue 1's glyph-vs-system-total scope with the user before coding. Pre-flight as below, then fix TDD with the subagent rhythm.

### What shipped this session — all on `master`, all user-feel-tested + merged

1. **#126 (`75cbe6d`) — curve-lock airtight read-only** (Part A). Locked RGB channels (Green/Blue → Red) are now genuinely non-editable: gated every mutation path (drag, insert, marquee→Delete/spinner/context-menu, cut) at the panel→renderer boundary + commit-site `focusLocked` guards; render treatment = dashed follower-colour line + hollow markers + a toolbar lock glyph. Data model stays the host pointer-alias (one-way mirror). Mock now derives lock views at read time. Spec/plan `docs/superpowers/{specs,plans}/2026-06-10-curve-lock-readonly*`.
2. **#128 (`dc7f81f`) — curve morph animation** (Part B). Every structural curve change (add/delete/paste/spinner/undo/interp-change/locked-follower re-mirror) morphs old→new via sample-and-tween (the legacy smooth curve reduces to a Newton-inverted fixed cubic + smoothstep; key-time-augmented grid keeps step verticals true), markers glide/pop/ghost, `matchMedia`-gated (jsdom = snap, suite untouched). `lib/curve-morph.ts` + `lib/use-curve-morph.ts`. Flicker fix landed pre-merge (overlay gradient match + synchronous first frame). Spec/plan `*2026-06-11-curve-morph-animation*`.
3. **#130 (`d5b6f1d`) — group-drag live-updates the spinners.** The renderer's group-drag branch now fires `onGroupDragMove`; `multiSelected` averages the LIVE shifted positions via a shared `computeGroupMoves` helper (one source for the clamp math, also used by `applyGroupShift`).
4. **#133 (`297ffd0`) — spinner arrow containment + press state.** Arrow column inset 1px + `rounded-r-[3px] overflow-hidden` so its hover/active bg stays inside the box outline; added `active:bg-accent-soft active:text-accent` press feedback.
5. **#134 (`f85cdc2`) — overload guard default 15k → 10k** (user feel) + native overload specs re-pinned LOW (1k); fixed the L-066 host-death flakes (heavy Debug plateaus were killing the test host); full harness ~5-8 min → ~2 min.
6. **#135 (`e8c1a14`) — morph z-order fix.** Morph overlays render BELOW the focus layer so a locked follower's catch-up line never paints over the focus channel's keys.
7. **#136 (`12c22bd`) — selected-key inverted core** (option A, after two rejected directions: channel-colored ring #132 closed, and the grow+drop-shadow). Selected dot flips to a canvas-bg fill (`--curve-marker-core` → `var(--panel)`, aliased) + thick channel-colour stroke; no blur. Three-way marker precedence: locked-hollow > selected-inverted > unselected-solid.
8. **#138 (`a50db90`) — preemptive overload hard-guard.** Host-enforced gate at the single `Engine::SpawnParticleSystem` choke point (`(placed+1) × estimate > cap` → refuse + `Clear()` + `nullptr`; nullptr now ⟺ refusal), web-supplied estimate (`estimateSystemLoad` in `chain-load.ts`, same walk as the ⚠ glyph, pushed via `engine/set/estimated-load`), cumulative spawn check + edit-time clear + clear-on-refusal + spawner self-disable + transient refusal banner; runtime budget kept as backstop. Spec/plan `*2026-06-11-overload-hard-guard*`. (Design note: the gate proved so comprehensive that the old visible-rate runtime-backstop specs became unreachable — they were retired; the 5 new gate specs prove the over-budget cases.)

### Verified baseline at `a50db90`
Web **780/780** (`pnpm --filter @particle-editor/editor test` from `web/`), `tsc -b` 0, vite build clean; native harness **180/0** (~2 min — the 5 new overload-gate specs run the real host+web end-to-end); host Debug + Release x64 clean (benign LNK4098). Fresh worktree → L-039 (NuGet copy) + L-040 (`pnpm build`).

### Lessons this session
- **L-080 reinforced (hit ~4×):** a piped command's exit code is the last stage's — `grep -c ... && next` runs `next` even on 0 matches (grep exits 1 on zero matches, which can ALSO break a `&&` chain the other way); and `git checkout x 2>&1 | tail -1 && gate` ran the gate against the WRONG tree when the checkout failed (branch held by another worktree). Capture `$?` or test the artifact; never gate after a pipe.
- **L-083 (landed #128):** a visual simplification justified as "imperceptible over N ms" is a feel-test claim, not a code decision — match the real thing if cheap, else flag it as an explicit feel-pass blocker. Corollary: `color-mix()` (and newer CSS color funcs) are unreliable as SVG presentation attributes — put them in a stylesheet rule via a custom property.
- **Parallel-PR CHANGELOG churn:** several small PRs against the same top-of-file CHANGELOG region conflict on every stacked merge; consolidate early or keep CHANGELOG edits out of feature branches. A stray `reset --hard` while ON a feature branch reset it locally (recoverable only because pushed).
- **Integration layer earns its keep:** the hard-guard's native run surfaced that the preemptive gate makes the runtime-backstop test scenario unreachable — invisible to unit tests and static review. The implementer STOPPED rather than loosening an assertion; that's the discipline that keeps green meaningful.

### Kickoff (full) — paste into a fresh session
> Pick up `new-particle-editor` (AloParticleEditor — Win32 host + WebView2/React + D3D9Ex-via-DComp particle editor for SW:EaW). **`master` is the trunk; `origin/master` = `a50db90`.** Session 37 shipped curve-lock read-only (#126), curve morph animation (#128), group-drag live spinners (#130), spinner containment+press (#133), overload default 10k + harness fix (#134), morph z-order (#135), inverted-core selected key (#136), and the **preemptive overload hard-guard (#138)** — all feel-tested + merged. **FIRST: merge the open docs PR [#139]** (CHANGELOG hash backfill sweep) if not already merged. **THEN the NEXT TASK (user-reported, not started): two overload-guard feel-test bugs** — read `tasks/HANDOFF.md` top (session 37, the "NEXT TASK" block) for the repro + root causes: (1) the ⚠ glyph uses fixed `CHAIN_WARN_THRESHOLD=10k` while the gate uses the configurable `maxParticles` cap (`chain-load.ts` + `overload-guard.ts` + `EmitterTree.tsx`), (2) a stale runtime-latch banner is revealed under the transient refusal banner (`OverloadBanner.tsx`; likely a spawn leaking in the `useEstimatedLoadPush` staleness window — consider an engine-side latch reset in `Engine::Clear()`). Also read the hard-guard spec/plan (`docs/superpowers/{specs,plans}/2026-06-11-overload-hard-guard*`) and `tasks/lessons.md` (L-080 piped-exit-codes; L-083; L-033 feel-tests are user-launched; L-039/L-040/L-046 fresh-worktree). VERIFY claims against code. Pre-flight: `git fetch origin master` + lineage; fresh worktree → L-039 (NuGet) + L-040 (`pnpm build`); from `web/` `pnpm install`, `pnpm --filter @particle-editor/editor test` → **780**, `tsc -b` 0; host Debug x64 (VS18, L-046); native harness → **180/0** (~2 min — NEVER pin overload specs high, OOMs the Debug host; the gate specs pin ≤2k). Brainstorm issue 1's glyph-vs-system-total scope with the user before coding; both fixes TDD + subagent-reviewed. Summarize understanding + confirm scope before changing anything.

### Kickoff (short)
> Pick up AloParticleEditor. **`origin/master` = `a50db90`.** Session 37 shipped curve lock read-only (#126), morph animation (#128), group-drag live spinners (#130), spinner containment+press (#133), 10k guard + harness fix (#134), morph z-order (#135), inverted-core selected key (#136), preemptive overload hard-guard (#138) — all merged. Merge the open backfill PR #139 first. **NEXT (not started): two overload feel-test bugs** — read HANDOFF top (s37 "NEXT TASK"): (1) ⚠ glyph uses fixed 10k threshold not the configurable cap; (2) stale latch banner revealed under the refusal banner (estimate-push staleness leak). Pre-flight: web **780**, `tsc -b` 0, native **180/0**, host Debug x64 (VS18; fresh worktree → L-039/L-040). VERIFY against code; brainstorm issue-1 scope with the user first.

---

## 2026-06-10 (session 36) — **TWO features shipped in one PR: NT-12 styled/animated tooltips + one modal/banner/tooltip motion family, AND a configurable preview overload guard (the #121 hard budgets made runtime-tunable from Preferences). Both via PR [#123] (`bd52588`); CHANGELOG hashes backfilled in [#124] (`57cb93e`). Both user-feel-tested + merged. NO task in flight — next session asks the user.**

**`origin/master` = `57cb93e`** (advanced this session from `2ed99d5`: #123 → `bd52588`, the CHANGELOG-backfill #124 → `57cb93e`). Nothing open at handoff; working branch `claude/…` for #123 fully merged. **ROADMAP [NT-12] is marked shipped** (§5.1, `✅ Shipped (#123)`); the guard-config work was a follow-on to the #121 crash fix, not a ROADMAP item, so it has no ROADMAP entry.

### What shipped — both on `master` via #123 (`bd52588`), CI-green, both user-feel-tested

1. **NT-12 — styled/animated tooltips app-wide + one motion family** (greenlit + spec'd in s35; spec [`docs/superpowers/specs/2026-06-10-nt12-tooltips-motion-design.md`](../docs/superpowers/specs/2026-06-10-nt12-tooltips-motion-design.md), plan [`docs/superpowers/plans/2026-06-10-nt12-tooltips-motion.md`](../docs/superpowers/plans/2026-06-10-nt12-tooltips-motion.md)). One `Tip` primitive ([`web/apps/editor/src/primitives/Tip.tsx`](../web/apps/editor/src/primitives/Tip.tsx)) on **`@radix-ui/react-tooltip`** (asChild trigger, portaled content), motion = hand-rolled CSS keyframes keyed to Radix `data-state`/`data-side` (the shipped `popover-animate` pattern extended — **no animation library**). **~45-site sweep** (not the spec's ~95: per the feel-test addendum, passive readouts — truncation labels, status-bar counts, import-path — lost tooltips entirely; tooltips remain only on interactive controls + the deliberate ⚠ glyph). Rich tier: the NT-11 heavy-emitter ⚠ glyph gets an amber-band tooltip (*"This chain/emitter may spawn too many particles"* + aligned per-generation breakdown), consuming `ChainWarning.path` directly — no formula dup; `formatChainWarning` stays the `aria-label`. Modal (fade + 8 px rise; its old entrance classes were silent no-ops) and OverloadBanner (fade + 6 px drop, `--shadow-soft` replacing the hard ring) joined the family via shared `--motion-*`/`--slip-*`/`--shadow-soft` tokens ([`tokens.css`](../web/apps/editor/src/styles/tokens.css)); banner exit plays through a new generic `usePresence` hook ([`web/apps/editor/src/lib/use-presence.ts`](../web/apps/editor/src/lib/use-presence.ts)) — animationend unmount with a timeout fallback so reduced-motion (no animationend) can't leak the occlusion registration.

2. **Configurable preview overload guard** (the #121 hard `kMaxLivePreviewParticles`/`kMaxLiveEmitterInstances` budgets, now runtime-tunable; spec [`docs/superpowers/specs/2026-06-10-overload-guard-config-design.md`](../docs/superpowers/specs/2026-06-10-overload-guard-config-design.md), plan [`docs/superpowers/plans/2026-06-10-overload-guard-config.md`](../docs/superpowers/plans/2026-06-10-overload-guard-config.md)). Compile-time constants became runtime members behind one **clamped** setter [`Engine::SetOverloadGuard`](../src/engine.cpp); the spawn gates short-circuit to "allow" and the `Update` refill+latch block is skipped when disabled (uncapped records no refusals, never lights the banner). **Preferences → Preview** gets a **"Limit preview particle count"** checkbox + a number field (1,000–1,000,000, **default 15,000** — well under #121's fixed 100k); off = fully uncapped (pre-#121, *can* OOM on an extreme effect — an amber line + autosave are the backstops). Instance ceiling derives from the particle cap at the same 20:1 ratio (default → 750 instances). New bridge command `engine/set/overload-guard {enabled, maxParticles}`; the web owns persistence ([`web/apps/editor/src/lib/overload-guard.ts`](../web/apps/editor/src/lib/overload-guard.ts), `lib/theme.ts` localStorage pattern), pushes on every change + once at mount; `BridgeDispatcher` caches the last config and reapplies on `SetEngine` (insurance against a recreated engine reverting). Two late feel-test tweaks: default cap **25k → 15k** (`ababfa5`), and Preferences min/max wired to the shared clamp-bound constants for DRY (`b94320a`).

3. **The clamp is intentionally duplicated** across the web lib and the engine setter — the bridge schema is a TS type-union with **no runtime validation**, so the engine must not trust its caller (a cap of 0 would zero the spawn budget forever and read as "editor broken"). `readOverloadGuard()`'s default path was returning the shared module constant by reference → `Object.freeze` closes the mutate-the-singleton hazard.

### Verified baseline at `57cb93e`
Web **700/700** (`pnpm --filter @particle-editor/editor test` from `web/`), `tsc -b` 0, vite build clean; native harness **180/0**; host Debug x64 clean (benign LNK4098). User live-tested: tooltip delay/skip-delay + slip across the toolbar, the ⚠ rich tooltip over the viewport (occlusion), modal + banner motion both themes, reduced-motion; the guard toggle + cap + the 25k→15k feel verdict.

> ⚠ **NEVER pin the overload regression specs to 100k.** A 100k-particle plateau OOM-crashes the **Debug test host** at the tail of the full ~178-spec single-process harness run (cumulative heap pressure, NOT an engine fault — the enabled path is byte-equivalent to #121's 100k behavior). The bomb tests are pinned to the default cap; three new specs cover 5k-honored / mid-run-lowering-decays / disabled-no-latch.

### Lessons this session
- **L-080 (new):** a piped command's exit code is the LAST stage's — `cmd | tail && echo OK` (and `ls X | head … || echo MISSING`) reports success on failure. Capture `$?` explicitly or test the artifact. Bit three times in one session (false NuGet "present", false BUILD_OK, a truncated failing-spec name costing a 3-min re-run).
- **L-081 (new):** one tree-touching agent per worktree (a "reviewer" stashed the concurrent implementer's WIP and left UU conflicts); and regenerate a11y goldens **only** from the full `pnpm a11y:update` — a `--grep` single-test regen captures a context the full run never has (earlier spec files establish selection/open-panel state), producing a golden that passes in isolation and fails in CI.
- **L-082 (new):** Tailwind v4 compiles `-translate-x/y-1/2` to the standalone `translate:` CSS PROPERTY, which *composes* with `transform` rather than being overridden by it. A motion keyframe for a Tailwind-centered surface must animate ONLY the delta (`transform: translateY(8px)`) and leave the `-50%` centering to `translate` — else it double-shifts and snaps back at animation end. (Opposite of Tailwind v3.) Bit both the modal and the banner; diagnosed via `getComputedStyle().translate` vs `.transform` live.
- **a11y goldens were also flaky** until `captureDomA11y` settled tooltip exit animations (`waitForFunction` for no `.tip-animate[data-state="closed"]`) before snapshotting — the previous tab stop's tooltip was mid-exit at capture time.

### Kickoff (full) — paste into a fresh session
> Pick up `new-particle-editor` (AloParticleEditor — Win32 host + WebView2/React + D3D9Ex-via-DComp particle editor for SW:EaW). **`master` is the trunk**; **`origin/master` = `57cb93e`**. Session 36 shipped **NT-12** (styled/animated tooltips app-wide + one modal/banner/tooltip motion family — `Tip` primitive on `@radix-ui/react-tooltip`, ~45-site sweep, rich ⚠ chain-warning tooltip, hand-rolled CSS keyframes) **and a configurable preview overload guard** (#121's hard budgets made runtime-tunable from Preferences → Preview: toggle + cap, default 15k, off = uncapped) — both via PR [#123] (`bd52588`), CHANGELOG backfill [#124] (`57cb93e`), both feel-tested + merged. Read `tasks/HANDOFF.md` top (session 36), the two specs under `docs/superpowers/specs/2026-06-10-nt12-tooltips-motion-design.md` + `…-overload-guard-config-design.md`, and `tasks/lessons.md` **L-080/L-081/L-082** (+ L-033 feel-tests are user-launched; fresh worktree → L-039 NuGet copy + L-040 `pnpm build`). VERIFY claims against code. Pre-flight: `git fetch origin master` + lineage check; from `web/` `pnpm install`, `pnpm --filter @particle-editor/editor test` → **700**, `tsc -b` 0; host Debug x64 (MSBuild VS18, L-046); native harness → **180/0**. **NEVER pin the overload specs to 100k — OOMs the Debug test host at the tail of the full run.** **NO task is queued — ask the user what's next** (candidates: **MT-13 legacy removal** [greenlit per memory `project_daily_driving_legacy`]; **splitter panel-content virtualization** [L-079 — the residual splitter stutter is Chromium's ~12 fps relayout, host-unfixable; virtualization is the remaining lever, new web-side scope]). Summarize understanding + confirm scope before changing anything.

### Kickoff (short)
> Pick up AloParticleEditor. **`origin/master` = `57cb93e`.** Session 36 shipped NT-12 styled/animated tooltips + modal/banner motion (`Tip` on `@radix-ui/react-tooltip`, ~45-site sweep, rich ⚠ tooltip) AND a configurable overload guard (#121 budgets → runtime, Preferences toggle + cap, default 15k, off = uncapped) — both via PR #123 (`bd52588`), CHANGELOG backfill #124 (`57cb93e`), both feel-tested + merged. Read HANDOFF top (s36) + the two specs under `docs/superpowers/` + lessons L-080/L-081/L-082 (+ L-033; fresh worktree → L-039/L-040). Pre-flight: web **700**, `tsc -b` 0, native **180/0** (NEVER pin overload specs to 100k — OOMs the Debug test host), host Debug x64 (VS18). **Nothing queued — ask the user what's next** (candidates: MT-13 legacy removal [greenlit]; splitter panel-virtualization [L-079]).

---

## 2026-06-10 (session 35) — **TWO features shipped: NT-11 soft chain-load warning (#120) and the preview overload guard (#121). The NT-11 feel test EXPOSED a pre-existing crash — an accidental Shift (spinner ×10) committed a six-figure Particles/sec and the editor OOM-died — which the overload guard now fixes. Both PRs merged + feel-approved. NO task in flight; the queued next work (greenlit, not started) is the NT-12 styled/animated tooltip pass + modal/banner entrance-exit animations.**

**`origin/master` = `2ed99d5`** (advanced this session from `6404d7d`: #120 → `e67e5e5`, #121 → `2ed99d5`). At handoff the only open PR is **`claude/session-35-handoff`** (this entry + the #121 CHANGELOG hash backfill `2ed99d5`; housekeeping, awaits OK). Working branches `claude/amazing-agnesi-edbcc7` (#120) + `claude/preview-overload-guard` (#121) both fully merged.

### What shipped — both on `master`, both CI-green, both user-feel-tested

1. **#120 (`e67e5e5`) — NT-11 soft chain-load warning.** An advisory amber ⚠ on every emitter-tree row of a chain whose estimated alive-particle count exceeds **10,000** (the v1-chain-bomb class), with a per-generation tooltip. Purely advisory — nothing blocks; authored `.alo` values untouched; vanilla effects never trigger it. **Architecture A** (user-approved spec [`docs/superpowers/specs/2026-06-10-chain-warning-design.md`](../docs/superpowers/specs/2026-06-10-chain-warning-design.md), plan [`docs/superpowers/plans/2026-06-10-chain-warning.md`](../docs/superpowers/plans/2026-06-10-chain-warning.md)): host mirrors 6 spawn fields onto `EmitterTreeNode.spawn` (`BuildEmitterTreeNode` + **three** synthetic-root literals incl. the one in `EmitEmittersTreeChanged()` the plan missed — [`src/host/BridgeDispatcher.cpp`](../src/host/BridgeDispatcher.cpp)); pure estimator [`web/apps/editor/src/lib/chain-load.ts`](../web/apps/editor/src/lib/chain-load.ts) (`estimateChainLoad`, `CHAIN_WARN_THRESHOLD=10_000`, Little's law, worst-path-wins ancestor marking); mock parity via a single `emit()`/`emitters/list` decoration choke point (no formula dup); glyph in [`EmitterTree.tsx`](../web/apps/editor/src/screens/EmitterTree.tsx) (5th grid col, DOM-order-last so a11y goldens are byte-stable). No new events — `set-properties` already re-emits `tree/changed`. Built subagent-driven (8 tasks, each spec+quality reviewed).

2. **#121 (`2ed99d5`) — preview overload guard** (the crash the NT-11 feel test surfaced). **Root cause:** no global budget on the live sim; chains allocate one child `EmitterInstance` **per parent particle**, so the per-instance uint16 index cap (16,383, [`EmitterInstance.cpp:273`](../src/EmitterInstance.cpp)) can't bound it. **Fix:** hard engine budgets `kMaxLivePreviewParticles=100'000` / `kMaxLiveEmitterInstances=5'000` ([`engine.h`](../src/engine.h)); per-frame spawn budget refilled at the top of `Engine::Update` with **90% resume hysteresis + 0.5 s clear debounce on the pausable `GetTimeF()` clock** ([`engine.cpp`](../src/engine.cpp)); `TryConsumeSpawnBudget()` in the burst loop + `TryConsumeInstanceBudget()` in `ParticleSystemInstance::SpawnEmitter` (returns **nullptr** on refusal — all callers null-safe, incl. `KillParticle`); refused rounds snap `m_nextSpawnTime` (dropped, not deferred — no resume-burst, no refused-churn spin). Over budget the engine **suppresses spawning** (live particles decay naturally; authored values never clamped). Flag rides the existing 4 Hz `stats/tick` (`overload: boolean`); [`OverloadBanner.tsx`](../web/apps/editor/src/components/OverloadBanner.tsx) shows a self-clearing amber pill over the viewport (occlusion-registered via `useViewportOcclusion`, `pointer-events-none`), StatusBar particle count tints amber. Native regression spec [`web/apps/editor/tests/preview-overload.spec.ts`](../web/apps/editor/tests/preview-overload.spec.ts) drives a 1e9/s bomb **and** a death-child refusal storm → asserts plateau ≤ budget + overload latch + recovery + editor-alive. Built subagent-driven (Task A engine+host+spec, Task B banner+tint), each spec+quality reviewed.

3. **Two latent counting bugs fixed** (actual-vs-intended counter class): `SpawnParticle` now returns **bool** — index-cap-refused spawns were counted into `m_numParticles` forever (`+1`, never decremented); `ParticleSystemInstance::RemoveEmitter` destroyed instances holding live particles without subtracting them (a permanent budget leak). Counters must track *actual* work, not intended.

4. **a11y harness pause-leak fixed** (was a user-flagged background chip). All **8** a11y spec files (`a11y-{chrome,keyboard,dialogs,curve-spinner}{,-composition}.spec.ts`) paused the preview clock in `beforeEach` and never reverted it — so every later spec file in the shared-host harness ran with a **frozen sim clock**. All 8 now `engine/set/paused {paused:false}` in `afterAll`; the overload spec also unpauses defensively. The chip is now stale — **dismiss it** (its `task_id` isn't in scope, so do it manually).

5. **Post-feel-test banner revisions (#121, user-driven).** Readability: the first cut used `text-amber-400` on `bg-bg-2`, near-invisible in light mode (light yellow on off-white) — replaced with a **filled** pill (`bg-warning` = theme-independent `#e0a14b`) + fixed near-black text `#1a1200`, plus `useViewportOcclusion`'s new opt-in `observeParent` (the banner is content-sized but `left-1/2`-centered, so a splitter drag moved it without resizing it → stale cut-out). Copy (user-worded): **"Preview spawning limited — lower spawn rates to resume. ⚠ marks heavy emitters."**

6. **ROADMAP [NT-12] noted** (user-flagged, not started): styled/animated **tooltip pass** — replace native `title` attributes app-wide with one shared styled+animated tooltip primitive; explicit requirement that the heavy-emitter ⚠ glyph gets a clear "what this means" tooltip (it currently carries a native `title` with the breakdown + an `aria-label`). Overlaps the queued animation work.

### Verified baseline at `2ed99d5`
Web **670/670** (`pnpm --filter @particle-editor/editor test` from `web/`), `tsc -b` 0, vite build clean; native harness **177/0** (30 skipped; the 2 overload specs run LAST by registration); host Debug + Release x64 clean (benign LNK4098). Worktree restored (L-039 NuGet + L-040 dist current; the served `dist` was rebuilt after the banner copy change). User live-tested: NT-11 glyph + tooltip (approved), overload banner styling + copy (approved).

### Lessons / validated notes this session
- **PS 5.1 UTF-8 mojibake** (saved to memory `project_ps51_utf8_file_edits.md`): `Get-Content -Raw` / `Set-Content` round-trips corrupt em-dashes / × / ★ in repo docs — use `[System.IO.File]::ReadAllText/WriteAllText` with `UTF8Encoding($false)` for scripted edits. Bit a ROADMAP renumber; recovered via `git checkout`.
- **Banner contrast:** a "theme-independent token" only helps if the **fill** is theme-independent — colored *text on a theme-flipping surface* fails in one mode. Filled pill + fixed dark text is the robust pattern. (Caught only at the user's feel test — Task B's component tests don't assert contrast.)
- **The plan's premise can be wrong:** "1e9/s alone exceeds the budget" was false — the per-instance index cap plateaus a single emitter at 16,384, *below* the global budget; the refused-allocation churn (~80k rounds/frame) tanked FPS to 3, forcing the refused-round snap. Diagnosed live via CDP.

### Kickoff (full) — paste into a fresh session
> Pick up `new-particle-editor` (AloParticleEditor — Win32 host + WebView2/React + D3D9Ex-via-DComp particle editor for SW:EaW). **`master` is the trunk**; **`origin/master` = `2ed99d5`** (+ the docs-only `session-35-handoff` PR if not yet merged). Session 35 shipped **NT-11** (soft chain-load warning, #120 `e67e5e5`) and the **preview overload guard** (#121 `2ed99d5` — engine particle/instance budgets + spawn suppression + self-clearing amber banner; fixes the OOM crash the NT-11 feel test exposed). Read `tasks/HANDOFF.md` top (session 35), `tasks/todo.md` (both plans + reviews), `tasks/lessons.md` (L-077/L-078/L-079; L-039/L-040/L-046 fresh-worktree restore; L-033 feel-tests are user-launched), and `ROADMAP.md` **[NT-12]**. VERIFY claims against code. Pre-flight: `git fetch origin master` + lineage check; fresh worktree needs L-039 (NuGet copy) + L-040 (`pnpm build`); from `web/` `pnpm install`, `pnpm --filter @particle-editor/editor test` → **670**, `tsc -b` 0; host Debug x64 (MSBuild VS18, L-046); native harness → **177/0**. **NEXT (greenlit, not started): the NT-12 styled/animated tooltip pass + modal/banner entrance-exit animations** — one motion family, do together on a fresh branch off `2ed99d5`. Brainstorm the motion language FIRST and inventory each animatable surface's mount mechanism (Radix `data-state` → pure CSS; custom mount/unmount → presence shim) before building. Also **dismiss the stale a11y-pause-leak background chip** (resolved this session). Diagnostic tooling that exists: `preview-overload.spec.ts` (the crash regression), `[resize-perf]`/`[PERF]` 1 Hz host.log probes. Summarize understanding + confirm scope before changing anything.

### Kickoff (short)
> Pick up AloParticleEditor. **`origin/master` = `2ed99d5`.** Session 35 shipped NT-11 chain warning (#120) + the preview overload guard (#121 — budgets + suppression + amber banner; fixed the Shift-×10 OOM crash). Read HANDOFF top (s35) + `tasks/todo.md` + `ROADMAP.md` [NT-12] + lessons (L-033 feel-tests user-launched; fresh worktree → L-039/L-040). Pre-flight: web **670**, `tsc -b` 0, native **177/0**, host Debug x64 (VS18). **NEXT (greenlit): NT-12 styled/animated tooltips + modal/banner entrance-exit animations** — one motion family, fresh branch off `2ed99d5`; brainstorm + inventory mount mechanisms first. Dismiss the stale a11y-pause-leak chip.

---

## 2026-06-10 (session 34) — **Resize-perf SHIPPED end-to-end (#116 Phase 0+1, #118 Phases 2+3) + closed-dock hardening shipped (#117). Window resize: smooth, no snap, no zoom (user-approved). Splitter drag: contention removed; residual stutter is Chromium's own relayout rate (L-079 — NOT host-fixable by smoothing). The resize-perf plan is CLOSED (A2 absorbed; C3 built→reverted). NO follow-up task is queued — next session asks the user.**

**`origin/master` = `b37f198`** (advanced this session: #116 → `e7cf484`, #117 → `9b4dcc8`, #118 → `b37f198`). Nothing open at handoff; CHANGELOG hashes backfilled through #118; working branches `claude/cool-jones-7cb68a` + `claude/resize-perf-phase2` both fully merged.

### What shipped — all on `master`, all CI-green, all user-feel-tested
1. **#116 (`e7cf484`) — smooth window resize.** Phase 0 probes (always-on 1 Hz `[resize-perf]` lines in host.log: wmpos tick cost + resets/sec + `Engine::ResetPerf` sub-stages; per-KIND bridge msg tally; `rps=` true cadence in `[PERF]`). Phase 1 **as revised by two feel verdicts**: (a) defer-to-settle REJECTED ("dislike the snap") → **`Engine::ResetForResize()`**: `IDirect3DDevice9Ex::ResetEx` ("all other surfaces persistent" — first-party-doc-verified) + rebuild of size-keyed RTs only → per-tick resize resets at **3.5-4 ms** (was ~24 ms; the ~20 ms texture re-decode is gone) via one `LayoutBroker::ResetEngineForResize` helper (cheap → full `Reset()` → `RecoverDeviceIfNeeded`); (b) zoom-on-resize wart → **FoV anchored absolute** (45°/768 px, clamp 120°) in `SetSceneViewport` — resize reveals world, never rescales (`ResetParameters`' 45° full-RT projection deliberately untouched for legacy). Riders: main-window `WM_ERASEBKGND→1` during sizemove; `WM_ENTER/EXITSIZEMOVE` + 150 ms quiescence timer remain as a no-op settle SAFETY NET (`SettleDeferredReset` acts only if a per-tick reset failed); misleading viewport-rect comment fixed. (A `put_Bounds` 30 Hz throttle shipped in the first iteration was REVERTED — feel suspect → L-078.)
2. **#117 (`9b4dcc8`) — closed-dock hardening** (user bug report mid-session + a companion found during verification). (a) Closed dock could be DRAGGED open: react-resizable-panels 4.11 hit-tests separators by document-level pointer coordinates — CSS `pointer-events-none` can never block it; fix = the lib's `disabled` prop on BOTH Separator and Panel (imperative `expand()/collapse()` bypasses it — verified in lib source). (b) Closed dock mounted OPEN-EMPTY under a stale `alo:layout:outer:3col` blob (Group `defaultLayout` vs Panel `defaultSize="0%"` seed disagreement + the mount effect racing the deferred apply); fix = `collapseDockShare` folds the stored share into centre at closed mount, and the masked first-open-at-zero-width companion got an `expandDock` safety net **gated on `getSize().asPercentage`** (`.inPixels` reads a stale `offsetWidth` in the same tick — would clobber the remembered width). Verified red→green + browser positive-control drags (open: 260→360 px; closed: no-op).
3. **#118 (`b37f198`) — splitter-drag package.** KEPT: **B1** pump paced to display cadence (QPC budget + `MsgWaitForMultipleObjectsEx` wake-on-input + `timeBeginPeriod(1)`; capture mode untouched; idle **~3000 fps → ~190 rps**, CPU **pegged core → ~20%**); **B2** `WaitEndFrameQuery` yields past a 64-poll burst; **C1** `ViewportSlot` send dedupe on (rect, DPR); **C2** log hygiene (per-msg `WebMsg` skipped for `layout/scene-rect` + `viewport/input`; `SetSceneViewport` printf 1 Hz; `[COMP-engine-transform]` quiet-on-anim-frames via a `quiet` param through `SetEngineVisualTransform`/`ApplyTransform` — settle always logs; 3 s drag = ~50 log lines, was 200+). REVERTED: **C3 chase-lerp** (`357f4fe`) — steady-state lag = one packet interval and the real stream under load is ~12/s → 80-160 ms lag + end snap; PLUS window resizes starved chases (`PredictAndApply` cancels the scene anim every size tick) → backing colour in revealed areas. → **L-079**.

### The two big diagnostic findings (verified live, keep for future perf work)
- **The user's "~104/s mystery" bridge traffic = `viewport/input` at mouse rate** whenever the cursor crosses the viewport — the arch-C input pipeline (host forwards mouse → WebView2 DOM → bridge → host). FUNCTIONAL traffic, not waste. Attribution via the per-kind probe + a **real-mouse SendInput drag driver** ([`tasks/tool-splitter-drag-probe.mjs`](tool-splitter-drag-probe.mjs) + `tool-splitter-coords.mjs`; synthetic DOM events do NOT reproduce OS input streams). The sizemove storm driver from #116 is [`tasks/tool-sizemove-storm.ps1`](tool-sizemove-storm.ps1).
- **The residual splitter stutter is producer-side:** Chromium relayouts the panels at ~12 fps under drag load — the PANEL edge itself steps at that rate; instant scene-rect application already tracks it as tightly as the architecture allows. Host smoothing cannot fix this (L-079). The only real levers: make panels cheaper to relayout (virtualization — new web-side scope) or raise the data rate. NOT attempted; scope with the user first.

### Verified baseline at `b37f198`
Web **637/637** (`pnpm --filter @particle-editor/editor test` from `web/`), `tsc -b` 0, vite build clean; native harness **174/0** (30 legacy skipped); host Debug + Release x64 clean (benign LNK4098). This worktree (`cool-jones-7cb68a`) is fully restored (L-039 NuGet + L-040 dist done; exe + dist current). User live-tested: window resize (approved), splitter drag (residual = Chromium relayout, accepted-for-now), dock fixes.

### Lessons this session
- **L-078 (new):** prefer making per-tick work CHEAP over deferring it to gesture end (a settle snap reads as a regression despite 30× measured wins); never bundle feel-degrading riders with a headline fix; the USER launches feel-test builds (agent `Start-Process` launches are L-033-tainted).
- **L-079 (new):** stream smoothing cannot beat the data rate — a chase-lerp's steady-state lag is one packet interval, and the producer slows exactly when the user is looking; audit every `CancelSceneAnim`-style interaction the moment anim machinery gains a second client.
- **Process note:** three user feel-verdicts redirected this plan (snap → ResetEx; zoom → FoV anchor; chase → revert). Each redesign was cheaper than shipping blind; the probes' numbers made each pivot defensible (the ResetEx redesign came straight from the probe's 20-of-24-ms reload measurement).

### Kickoff (full) — paste into a fresh session
> Pick up `new-particle-editor` (AloParticleEditor — Win32 host + WebView2/React + D3D9Ex-via-DComp particle editor for SW:EaW). **`master` is the trunk**; **`origin/master` = `b37f198`**. Session 34 CLOSED the resize-perf effort end-to-end: #116 (cheap per-tick ResetEx resize + FoV anchor + probes), #117 (closed-dock hardening), #118 (paced pump + GPU yield + send dedupe + log hygiene; the C3 chase-lerp was built, user-rejected, REVERTED — read L-079 before attempting ANY stream-smoothing). Window resize is user-approved smooth; the residual splitter stutter is Chromium's own ~12 fps relayout under drag (producer-side; host smoothing can't fix it — the lever would be panel-content virtualization, NOT attempted, new scope). Read `tasks/HANDOFF.md` top (session 34), `tasks/todo.md` (plan + review), and `tasks/lessons.md` (L-077/L-078/L-079; L-039/L-040/L-046 fresh-worktree restore; L-033 feel-tests are user-launched). VERIFY claims against code. Pre-flight: `git fetch origin master` + lineage check; fresh worktree needs L-039 (NuGet copy) + L-040 (`pnpm build`); from `web/` `pnpm install`, `pnpm --filter @particle-editor/editor test` → **637**, `tsc -b` 0; host Debug x64 (MSBuild VS18, L-046); native harness → **174/0**. Diagnostic tooling that exists: `[resize-perf]`/`[PERF]` 1 Hz probes in host.log (per-kind bridge tally, reset sub-stages, `rps=` true cadence), `tasks/tool-sizemove-storm.ps1` (programmatic resize storm), `tasks/tool-splitter-drag-probe.mjs` + `tool-splitter-coords.mjs` (CDP + real-mouse SendInput splitter drags). **NO task is queued — ask the user what's next** (candidates they may raise: ROADMAP NT-11 soft chain warning, MT-13 legacy removal, panel-relayout cost reduction for the splitter residual). Summarize understanding + confirm scope before changing anything.

### Kickoff (short)
> Pick up AloParticleEditor. **`origin/master` = `b37f198`.** Session 34 shipped the whole resize-perf effort (#116 ResetEx per-tick resize + FoV anchor; #117 dock hardening; #118 paced pump + dedupe + log hygiene; C3 chase REVERTED — L-079: smoothing can't beat the data rate). Splitter residual = Chromium relayout rate (producer-side, accepted). Read HANDOFF top (s34) + lessons L-078/L-079. Pre-flight: web **637**, `tsc -b` 0, native **174/0**, host Debug x64 (VS18; fresh worktree → L-039/L-040). **Nothing queued — ask the user what's next.**

---

## 2026-06-09/10 (session 33) — **Drag audit FIXED+SHIPPED (#110/#111, host-smoked); chain investigation RESOLVED in-game (#113, depth-3 chains WORK, no guard; NT-11 scheduled #114); resize-perf investigation COMPLETE (root causes found + fix plan written — THE NEXT TASK).** Headline drag fix: a routine root reorder silently **swapped a parent's life/death child slots** (RMW index-aliasing across 3 hand-copied KEEP-IN-SYNC fns) → corrupted the `.alo` on save. NEXT SESSION: **implement the resize-perf fixes** per [`tasks/resize-perf-investigation.md`](resize-perf-investigation.md) — user-confirmed as the first task.

**`origin/master` = `cee5508`** (advanced this session: #110 → `e4ef42b`, #111 → `5550cb4`, #112 → `4590fa7`, #113 → `d974dfc`, #114 → `cee5508`). Open at handoff: the docs-only **resize-perf investigation note PR** (branch `claude/resize-perf-investigation`).
...
### Chain investigation — RESOLVED in-game (2026-06-10)
The v2 test came back "sparkles only" (chain starved — it hung off `flash`, a single-burst/single-particle emitter); v3 re-rooted the chain on a proven-firing sparkle emitter, authored with the editor's own data model (`tests/make_chain_test_alo.cpp`: load → `reparentEmitter` ×2 → rate clamps → write → reload-assert; shipped #113). Fired in-game: **all three generations visible (sparkles → highlights → smoke puffs) — depth-3 chains WORK in `StarWarsG.exe`; the v1 crash was the particle bomb; NO editor depth-guard needed.** Mod file restored to vanilla — nothing planted. Verdict in `tasks/multi_child_emitter_investigation.md` (addendum) + `tasks/next-emitter-chain-investigation.md` (RESOLVED). Follow-up scheduled as ROADMAP **[NT-11]** (#114): a *soft* warning when a chain's per-particle multiplication explodes.

### Resize-perf investigation — COMPLETE, fixes NOT built (THE NEXT TASK)
User report: performance tanks on window resize and splitter drags; the dock slide stays smooth. Investigated via a 61-agent / 6-dimension adversarially-verified workflow (run `wf_249936bf-ee0`: 33 confirmed / 21 refuted / 0 unverified). **Full write-up + ranked fix plan: [`tasks/resize-perf-investigation.md`](resize-perf-investigation.md)** — read it FIRST next session. The short version:
- **Window resize dominant:** a **full D3D9Ex device reset per mouse-move tick** — `WM_WINDOWPOSCHANGED` → `LayoutBroker::PredictAndApply` → `m_engine->Reset()` ([`LayoutBroker.cpp:251-254`](../src/host/LayoutBroker.cpp)), 10–100+ ms × 30–60/sec: full RT/shader recreate, **whole texture-cache wipe with disk/MEG re-decode**, ~40 MB realloc minting a NEW NT shared handle → forced `OpenSharedResource` + DXGI `ResizeBuffers` per tick, then a cold-frame render with a no-yield GPU busy-spin (`WaitEndFrameQuery`, [`engine.cpp:1521-1535`](../src/engine.cpp)) — all serialized inside the modal sizemove loop.
- **Splitter drag dominant (no reset fires):** the **uncapped render loop** ([`HostWindow.cpp:3357-3428`](../src/host/HostWindow.cpp), PeekMessage free-run + per-frame busy-spin = 1 core pegged, GPU saturated → WebView2 starved) × the **60–144 msgs/sec RO→bridge scene-rect stream** (no dedupe, discarded responses, sync log flush per message, [`ViewportSlot.tsx:87-91`](../web/apps/editor/src/components/ViewportSlot.tsx)) × **multi-clock judder** (engine edge on render-loop clock vs DOM edge on vsync — named in the comment at [`BridgeDispatcher.cpp:931-935`](../src/host/BridgeDispatcher.cpp)).
- **Red herring (don't chase):** the `layout/viewport-rect`→`Engine::Reset` BRIDGE binding is test/poc-only — no production web code sends it; the live reset driver is the native PredictAndApply path. FramePublisher/arch-A composite paths confirmed dormant.
- **Fix plan (ranked, in the note):** **A** defer Engine::Reset to gesture settle via WM_ENTER/EXITSIZEMOVE + DComp-stretch mid-drag (the dock-slide template; highest leverage, ~1-2 d) → **D** cheap riders in the same PR (WM_ERASEBKGND, put_Bounds throttle, comment fix) → **B** pace the render loop + yielding GPU sync (the splitter fix, ~2-3 d) → **C** scene-rect dedupe → one-way → host-clocked targets (kills the judder) → **A2** make the settle-reset cheap (ResetEx semantics — verify D3D9Ex pool docs first). **Recommended: the ~half-day instrumentation pass FIRST** (QPC the Reset, count msgs/sec, measure the spin — exact probes in the note §Instrumentation) to confirm the ranking with real numbers.

### Kickoff (full) — paste into a fresh session
> Pick up `new-particle-editor` (AloParticleEditor — Win32 host + WebView2/React + D3D9Ex-via-DComp particle editor for SW:EaW). **`master` is the trunk**; `origin/master` = **`cee5508`** (+ the docs-only resize-investigation PR if not yet merged). Session 33 closed out everything previously open: drag audit (10 fixes shipped #110/#111, host-smoked), emitter chains (RESOLVED in-game: depth-3 chains WORK, no guard; tool #113; NT-11 scheduled #114). **THE TASK: implement the resize-performance fixes.** Read `tasks/HANDOFF.md` top (session 33), **`tasks/resize-perf-investigation.md` (the root-cause report + ranked fix plan — the primary context)**, and `tasks/lessons.md` (L-077; L-039/L-040/L-046 fresh-worktree build; L-033 feel-changes need the user's eye). VERIFY claims against code. Pre-flight: `git fetch origin master`; fresh worktree needs L-039 (NuGet copy) + L-040 (`pnpm build`); from `web/` `pnpm install`, `pnpm --filter @particle-editor/editor test` → **636**, `tsc -b` 0; host Debug x64 (VS18); native **174/0**. Root causes (verified): window resize = per-tick full D3D9Ex `Engine::Reset` storm (`LayoutBroker.cpp:251-254` via `HostWindow.cpp:2344-2348`); splitter drag = uncapped render loop + `WaitEndFrameQuery` busy-spin starving WebView2 (`HostWindow.cpp:3357-3428`, `engine.cpp:1521-1535`) × 60-144/sec scene-rect stream (`ViewportSlot.tsx:87-91`) × multi-clock judder. The dock slide is the proven fix template (suppress per-tick work + host-side DComp transform + one settle commit). **Plan mode first** (★★★+ work): propose the fix sequence — recommended: instrumentation pass (~half-day, probes in the note) → Fix A + D (one PR) → B → C — and confirm scope with the user before coding. This is host-feel work: every fix needs the user's manual resize/drag verdict (L-033), plus the standard suites.

### Kickoff (short)
> Pick up AloParticleEditor. `origin/master` = **`cee5508`**. Session 33: drag audit shipped (#110/#111), chains resolved in-game (#113/#114 — depth-3 WORKS, no guard). **NEXT TASK (user-confirmed): implement the resize-perf fixes** — read **`tasks/resize-perf-investigation.md`** (verified root causes: window resize = per-tick D3D9Ex `Engine::Reset` storm at `LayoutBroker.cpp:251-254`; splitter = uncapped loop + busy-spin + 60-144/s scene-rect stream + judder; dock slide = the fix template) + `tasks/HANDOFF.md` top + lessons (L-077, L-039/L-040/L-046, L-033). Pre-flight: web **636**, `tsc -b` 0, native **174/0**, host Debug x64 (VS18). Plan mode: propose instrumentation-first → Fix A+D → B → C; confirm scope with the user before coding. Host-feel work — user smokes every fix live.
...
### What shipped this session — all on `master` via #108 (`71b5c41`)
1. **Preview polish (`dea7f82`, user-smoked)** — subtree dimming (both drag paths), dragged≠hidden styling (`opacity-40 saturate-50`), chip cap (4 names + "+k more"), **geometric gap resolver** (drag-activation snapshot + un-shift + midpoint rule; no dead zones; gap = true block height, renders by gap index, end-gap after the last subtree), **chip magnetize** (`computeChipTarget` blends chip Y 60% toward the gap center + rAF spring). Stability pinned by a vitest **fixed-point/no-cycle property**.
2. **Single-drag parity + highlight-follow (`154514d`)** — single root = a size-1 block (same gap+chip, commits via `reorder-many` → follow via `newIds`); reparent keeps the onto-ring, resolved geometrically (`resolveSingleRootDrop` + per-row `captureRowGeometry`). 2px insertion line gone; indicator `{kind:"gap"|"onto"}`. Follow-bug root cause: `emitters/drop` returned only `{ok}` and never re-selected — host now re-selects the moved `Emitter*` + emits `emitters/selected`; mock parity. → **L-076**.
3. **Reorder GLIDE animation (`49dc692`+, the glide branch; user-smoked, approved)** — rows glide to new positions on every reorder (drag-drop, Move Up/Down, delete, paste) **and during a drag** as the gap moves (`FLIP_DRAG_MS` 120 ms / `FLIP_SETTLE_MS` 200 ms; interrupted glides fold the in-flight transform residual). Root cause fixed first: runtime-only **`stableId`** on `Emitter` (all 3 ctors + preserved through `copySharedParamsFrom`), surfaced on the DTO/schema/mock; React keys rows by it + a FLIP `offsetTop` pass. **Chip spawn pop-in + despawn flight into the landing gap / reparent row** (`CHIP_EXIT_MS`). `prefers-reduced-motion` disables all motion. User smoked mid-drag glide + chip spawn/despawn in the host and approved.
4. **Two adversarial review passes** (multi-agent, refute-style verification) hardened items 1–3 before merge: a **critical `stableId` identity clobber** in `copySharedParamsFrom` (every linked-emitter edit stamped the source's id onto siblings → duplicate React keys; now restored + `NDEBUG` assert), a **stale reparent latch**, **mid-glide geometry-snapshot corruption**, **FLIP map staleness**, **focus-drop on undo remount**, a phantom-`stableId` test param, and a missing stableId contract block — all fixed + regression-tested.

### DEFERRED mid-experiment: emitter chains in-game (children of children)
**Read [`tasks/next-emitter-chain-investigation.md`](next-emitter-chain-investigation.md) FIRST on resume — the v2 test file is still planted:** `P_S_ASSAULTCONC.ALO` in `D:\SteamLibrary\...\Mods\EmpireAtWarExpanded` is patched to a depth-3 chain (flash→detail→Smoke, sparkles as control), **unfired**. Restore via `git checkout` in the mod repo. Evidence so far: game-binary RE says chains are structurally supported (recursive child pointers; chains were the recorded multi-child workaround), but Petroglyph's own 409 particle files have **zero** chains (21 link users, all exactly depth 2 — scanned via `tasks/tool-alo-chain-scan.py`), and the v1 depth-6 test **crashed the game on fire** (confounded: combinatorial particle bomb, my design error — every generation spawns per-particle). The v2 decision matrix is in the note; outcome decides whether the editor needs a depth guard.

### DEFERRED: drag reorder/reparent end-to-end audit (rate-limited mid-run)
**Read [`tasks/next-drag-audit.md`](next-drag-audit.md) before trusting the audit workflow's output** — run `wf_702b08d9-c73` LOOKS like a clean pass (`confirmed: []`) but only 2/6 dimensions ran and ALL verifiers died on the session token limit (no-verdict counts as "refuted" in the script). 10 findings stand UNVERIFIED (mid-drag tree mutation committing stale positional ids, no blur/pointer-capture safety net, startDrag re-entrancy, chip occlusion/width/perf, …) + a junk-undo-on-refused-op note. The note has the resume recipe (cached `resumeFromRunId`) and per-finding triage priors. The glide + the two earlier review passes' fixes (stableId clobber, reparent latch, mid-glide snapshot, FLIP staleness) are NOT affected — those are fixed and tested.

### BUILT + adversarially reviewed: reorder glide animation (`claude/reorder-glide`)
Design pre-agreed ([`tasks/next-reorder-glide-animation.md`](next-reorder-glide-animation.md)): **fix the root cause first** — host-side stable per-emitter id (assigned in all 3 `Emitter` ctors from a monotonic counter, surfaced as `stableId` on the DTO/schema/mock) — then key the React rows by it and FLIP on order changes (`offsetTop` deltas, ~200ms ease, gated to NOT fire mid-drag, reduced-motion skips). Full plan + risks in [`tasks/todo.md`](todo.md) Part 3. Undo/redo rebuilds emitters → fresh stableIds → remount-no-glide: accepted.

### Verified baseline on `master` (`71b5c41`, post-merge)
web **630/630**, `tsc -b` 0, vite build clean, native **174/0**, host Debug x64 clean (VS18) — all run on the reconciled tree before the #108 merge. This worktree (`elastic-rosalind-5cd21d`) is fully restored (L-039/L-040 done; exe + dist built with the latest). The exe serves `dist`, so a plain `--new-ui` launch shows the shipped feature.

### Lessons this session
- **L-076 (new)** — ops that reindex positional ids must re-select the moved entity (`newIds` or a `selected` event); a stable-id mock masks the bug — verify follow on the real host.
- **stableId clobber (from review, candidate lesson):** adding an identity field to a struct means auditing **every** copy path, not just constructors — `*this = src` in a params-propagation helper (`copySharedParamsFrom`) was a fourth copy path that silently duplicated the id. Grep for `*this =` / `operator=` / `memcpy` of the struct.
- **Experiment design (chain test):** testing a *depth* question on a per-particle-spawning system needs combinatorics control — chain the lowest-count emitter, keep depth minimal, or a crash proves nothing. Our editor accepts a `.alo` CLI arg — load-check planted test files before costing a game launch.
- **Workflow rate-limit trap:** a workflow that scores no-verdict as "refuted" reports `confirmed: []` when its verifiers die on the token limit — **a clean-looking pass can be a dead run.** Check `<failures>` and the agent_count vs expected.
- **Radix long-press gotcha:** synthetic `pointerdown` with default `pointerType: ""` over a ContextMenu.Trigger fires the touch long-press — drive synthetic drags with `pointerType: "mouse"`.

### Kickoff (full) — paste into a fresh session
> Pick up `new-particle-editor` (AloParticleEditor — Win32 host + WebView2/React + D3D9Ex-via-DComp particle editor for SW:EaW). **`master` is the trunk**; `origin/master` = **`71b5c41`**. The emitter-tree **drag feature SHIPPED** via PR [#108] (multi-select + single-drag drag-reorder, reparent, highlight-follow, reorder **glide** animation, chip spawn/despawn) — #106 was superseded (auto-merged into #108). The only OPEN PR is **`claude/post-108-backfill`** (CHANGELOG `TODO`→`71b5c41` + handoff; housekeeping, awaits OK). Read `tasks/HANDOFF.md` top (session 32), `tasks/next-emitter-chain-investigation.md` (**a patched `.alo` test is still planted in the user's EmpireAtWarExpanded mod — see Live state; restore via `git checkout` in the mod repo**), `tasks/next-drag-audit.md` (**a 6-dimension drag audit was rate-limited mid-run — its `confirmed: []` is NOT a pass; 10 findings UNVERIFIED + a junk-undo note; resume recipe inside**), and `tasks/lessons.md` (L-076; L-022/L-033/L-046/L-039/L-040/L-068/L-070/L-072). VERIFY claims against code. Pre-flight: `git fetch origin master`; fresh worktree off master needs L-039 (NuGet) + L-040 (`pnpm build`); from `web/` `pnpm install` if needed, `pnpm --filter @particle-editor/editor test` → **630**, `tsc -b` 0; host Debug x64 (VS18); native **174/0**. **NEXT — confirm with the user:** (1) merge the **`post-108-backfill`** housekeeping PR (CI green); (2) **resume the drag audit** (`tasks/next-drag-audit.md` — re-run the 4 missing dimensions + verify the 10 findings, then fix confirmed ones); (3) **resume the chain test** (user fires the planted v2 `.alo`; decision matrix in its note → editor depth-guard or not). The drag feature itself is shipped + user-approved — no smoke outstanding. Summarize understanding + confirm scope before changing anything.

### Kickoff (short)
> Pick up AloParticleEditor. `origin/master` = **`71b5c41`**. Emitter-tree drag feature (multi+single reorder, reparent, glide, chip anims) **SHIPPED** via PR [#108]; #106 superseded. Only open PR: **`claude/post-108-backfill`** (CHANGELOG/handoff housekeeping, awaits OK). TWO deferred + planted: `tasks/next-emitter-chain-investigation.md` (`.alo` test planted in the user's mod, unfired) + `tasks/next-drag-audit.md` (drag audit rate-limited mid-run — `confirmed: []` is NOT a pass; 10 UNVERIFIED findings). Read HANDOFF top (s32) + those two notes + lessons (L-076). Pre-flight: web **630**, tsc 0, native 174/0. NEXT (confirm): merge backfill PR / resume drag audit / fire chain test. (Feature shipped + user-approved; no smoke outstanding.)

---

## 2026-06-09 (session 31) — **Shipped the held session-30 work (PR [#104] merged `6f462e0`, CHANGELOG backfill [#105] `f4cd8d0`); BUILT the multi-select drag-reorder feature → PR [#106] (OPEN, host-smoke-polished, NOT merged — user is continuing to polish the preview next session); fixed a mock dirty-bit gap → PR [#107] merged `3479fe6`.** NEXT: **continue polishing the multi-drag preview in the host** (L-033), then **reconcile + merge #106**.

**Branch state (read carefully).** `origin/master` = **`3479fe6`** (this session advanced master: #104 → `6f462e0`, #105 → `f4cd8d0`, #107 → `3479fe6`). **PR [#106]** (`feat(new-ui): multi-select drag-reorder for the emitter tree`, branch **`claude/multiselect-drag`**, HEAD **`16d1988`**) is **OPEN**, **17 ahead / 2 behind** master — the feature + the host-smoke polish. **NOT merged** (user wants to keep tuning the preview feel). CI re-running on the polish push (was green before `16d1988`). **Before merging #106, reconcile with master** (`git merge origin/master`) — expect conflicts in: **`CHANGELOG.md`** (both #106 and #107 prepend a top entry → order reverse-chron), **`web/apps/editor/src/bridge/mock.ts`** `isMutating` (#107 added `emitters/move-many` + `emitters/duplicate-many`; #106 added `emitters/reorder-many` — **keep ALL THREE**), and **`.gitignore`** (`.superpowers/` added near the top on #106, near the bottom on master → dedupe to one). `bridge-contract.test.ts` (#107) shouldn't conflict.

### What shipped / built this session (in order)
1. **Reconciled + shipped the held session-30 work.** The harness auto-provisioned a *fresh empty* session branch off master; the actual 24-commit work was on `claude/modest-hypatia-ebdbc4` (what the s30 handoff described). Switched onto it, `git merge origin/master` (HANDOFF.md both-prepend conflict resolved reverse-chron, no content lost), pushed the 8 held commits, refreshed + **merged PR [#104]** (`6f462e0` — save/delete-safety + multi-aware delete/dup/move + ROADMAP §1→§5 tidy), then **CHANGELOG hash backfill → PR [#105]** (`f4cd8d0`).
2. **Multi-select drag-reorder feature → PR [#106].** Brainstorm → spec → adversarial-verify → plan → implement → host-smoke polish:
   - **Spec** [`docs/superpowers/specs/2026-06-09-multiselect-drag-reorder-design.md`](../docs/superpowers/specs/2026-06-09-multiselect-drag-reorder-design.md); **plan** [`docs/superpowers/plans/2026-06-09-multiselect-drag-reorder.md`](../docs/superpowers/plans/2026-06-09-multiselect-drag-reorder.md).
   - **A 15-agent adversarial design-verify Workflow** (run `wf_4479ffea-4ec`) BEFORE any code caught the **no-op-breadth bug** (refuse the *whole* contiguous footprint `[first, last+1]`, not just the two edges) **and** the **animation identity blocker** (no stable emitter id → the smooth glide was **deferred**: [`tasks/next-reorder-glide-animation.md`](next-reorder-glide-animation.md)).
   - **8 TDD tasks via subagents** (each spec-compliance + code-quality reviewed): host op `emitters/reorder-many { ids, rootIndex } → { newIds }` ([`ParticleSystem::reorderManyRootsToIndex`](../src/ParticleSystem.cpp:1552) + the [`BridgeDispatcher.cpp`](../src/host/BridgeDispatcher.cpp) handler, ~`:4657`), mock parity ([`reorderManyRoots`](../web/apps/editor/src/bridge/mock-state.ts)); web [`lib/multi-drag.ts`](../web/apps/editor/src/lib/multi-drag.ts) (`resolveMultiDropIntent`), [`reorderManyEmitters`](../web/apps/editor/src/lib/emitter-reorder.ts), the [`EmitterTree.tsx`](../web/apps/editor/src/screens/EmitterTree.tsx) multi-drag controller + preview.
   - **Host-smoke polish (L-033, commit `16d1988`)** — tuned live with the user: the destination band became a **flow "make-room" gap** (a `<li>` spacer that pushes the other rows aside so you see the relative drop position); the cursor chip is now a **vertical name list in tree order**; the **whole lifted block dims** (`draggingIds`, not just the grabbed row); the indicator **HOLDS on a dead zone** (over the gap / onto-third / child) instead of clearing → kills the shift→re-hittest→snap **flicker**; `resolveMultiDropIntent` returns a **3-state** result (`{rootIndex}` | `"noop"` | `null`) so the block's **own footprint reads as a deliberate no-op** (clear → a release there leaves the order unchanged) vs a dead zone (hold); **`ring-inset`** so the gap isn't clipped at the list's scroll edge. User verdict: "much better."
3. **Mock `isMutating` dirty-bit fix → PR [#107] merged `3479fe6`.** `emitters/move-many` + `emitters/duplicate-many` (shipped in #104) were missing from the mock allowlist, so a multi-move/dup didn't mark the doc dirty in mock mode (native host was fine — it marks dirty per-handler). TDD'd red→green with bridge-contract tests.

### Verified baseline
- **#106 branch (`16d1988`):** web **604/604** (`pnpm --filter @particle-editor/editor test` from `web/`), `tsc -b` 0, `vite build` clean (benign >500 kB chunk warning); host **Debug x64** (MSBuild **VS18**, L-046) clean (benign LNK4098); native a11y/bridge harness **174/0** (30 legacy-hwnd skipped) — no a11y surface change; host↔mock parity hand-traced on the algorithm. This worktree's L-039 (WebView2 1.0.3967.48 NuGet → `packages/`) + L-040 (`pnpm build`) restore is **done** (`x64\Debug\ParticleEditor.exe` + `dist` are built; launched repeatedly for the smoke).
- **master (`3479fe6`):** web **587/587** (585 + #107's 2 dirty tests).

### Open decisions / NEXT
1. **Continue polishing the multi-drag preview (L-033)** — the user's chosen next step. Candidate items: the **hold approach's dead-travel tradeoff** (on a *tall* block the gap "sticks" ~block-height before jumping to the next slot — the smoother **geometric** alternative, computing the drop gap from a fixed row-geometry snapshot instead of live DOM hit-testing, was sketched in-conversation if the stickiness bites); tune the **dim opacity** / gap styling; **subtree dimming** (today only the selected ROOT rows dim, not their children); chip styling. Tune values live in the real host.
2. **Reconcile #106 with master + merge** — see Branch state for exact conflicts. Merge → `master` is the explicit-OK gate (the host *feel* is already user-approved; CI must be green).
3. **CHANGELOG backfills (`TODO` date-lines).** #107's entry on master is **unbackfilled** (`TODO` → `3479fe6` / #107). #106's entry also carries `TODO` (backfill after #106 merges). Batch both into one follow-up PR when #106 lands (prior art #102/#105).
4. The spun-up chip `task_73fe301e` is **done** (it was the #107 fix).

### Lessons / gotchas this session
- **L-022 reinforced (twice):** a mapping subagent falsely claimed `emitters/move-many`/`duplicate-many` "don't exist in the schema" — grepping the wire-kind disproved it. The adversarial design pass's own findings were re-verified by hand-tracing before trusting (the no-op-breadth fix was checked on `[A..F]` by hand). Verify agent claims against the wire-kind/code.
- **New (web preview):** a **make-room drag gap that reflows the list will flicker** if the drop-target is resolved from the live (gap-shifted) DOM — inserting the gap moves rows, the pointer lands on the gap (`pointer-events-none` → null target) → gap clears → rows snap back → repeat. Cures: **hold the gap on a null/dead-zone resolution** (shipped), or compute the gap from a **fixed geometry snapshot** (smoother, deferred). And distinguish the footprint **no-op** (clear, so "drop it back" works) from a dead zone (hold).
- **L-033 held:** every preview decision (band→gap, chip shape, block dimming, flicker, edge-clip, the "forced to move" no-op) was judged by the user in the real host — agent screenshots could not have caught the flicker or the forced-move feel.

### Kickoff (full) — paste into a fresh session
> Pick up `new-particle-editor` (AloParticleEditor — Win32 host + WebView2/React + D3D9Ex-via-DComp particle editor for SW:EaW). **`master` is the trunk** (new-UI x64 default, `--legacy` opts out; `lt-4` retired). **`origin/master` = `3479fe6`.** This session built the **multi-select drag-reorder** feature → **PR [#106]** (branch **`claude/multiselect-drag`**, HEAD **`16d1988`**, **OPEN, NOT merged** — the user is continuing to polish the preview *feel*). Read `tasks/HANDOFF.md` top (session 31), the spec `docs/superpowers/specs/2026-06-09-multiselect-drag-reorder-design.md`, the plan `docs/superpowers/plans/2026-06-09-multiselect-drag-reorder.md`, the deferred-glide note `tasks/next-reorder-glide-animation.md`, the CHANGELOG top entries, and `tasks/lessons.md` (**L-022** verify agent claims via the wire-kind; **L-033** preview *feel* needs the user's eye; **L-046** MSBuild VS18; **L-039/L-040** fresh-worktree restore; **L-068** `pnpm build` before native; **L-070** `tsc -b` not `--noEmit`; **L-072** never rewrite UTF-8 with PowerShell `Set-Content`). **VERIFY claims against actual code.** **Pre-flight:** `git fetch origin master`; check out `claude/multiselect-drag`; from `web/` `pnpm install` if needed, then `pnpm --filter @particle-editor/editor test` → **604**, `tsc -b` 0; host **Debug x64** (VS18) clean; native **174/0** (fresh worktree needs L-039 NuGet copy + L-046 + L-040). To smoke the drag: `pnpm --filter @particle-editor/editor build`, then launch `x64\Debug\ParticleEditor.exe --new-ui`, Ctrl+click several root emitters and drag one. **NEXT — confirm with the user:** (1) **keep polishing the multi-drag preview** (L-033 — the make-room gap's hold/dead-travel tradeoff, dim opacity, subtree dimming, chip styling; tune live in the host); OR (2) **reconcile + merge #106** — `git merge origin/master` first (conflicts: `CHANGELOG.md` both-prepend reverse-chron; `mock.ts` `isMutating` keep move-many + duplicate-many + reorder-many; `.gitignore` `.superpowers/` dedup), then merge with explicit OK + green CI; OR (3) start the **deferred glide animation** (`tasks/next-reorder-glide-animation.md` — add a stable emitter id first). Also backfill the CHANGELOG `TODO` date-lines (#107 → `3479fe6`; #106 after it merges). Summarize understanding + confirm scope before changing anything.

### Kickoff (short)
> Pick up AloParticleEditor. **`master` = `3479fe6`.** PR [#106] (`claude/multiselect-drag`, HEAD `16d1988`, **OPEN, NOT merged**) = the multi-select drag-reorder feature + host-smoke polish; the user is still polishing the preview. Read `tasks/HANDOFF.md` top (session 31) + the spec/plan under `docs/superpowers/` + `tasks/next-reorder-glide-animation.md` + `tasks/lessons.md` (L-022, L-033, L-046, L-039/L-040). VERIFY against code. Pre-flight on `claude/multiselect-drag`: `pnpm --filter @particle-editor/editor test` → **604**, `tsc -b` 0; native **174/0** (fresh worktree L-039/L-046/L-040). **NEXT (confirm with user):** keep polishing the multi-drag preview (L-033, in the host), OR reconcile (`git merge origin/master` — `CHANGELOG.md` / `mock.ts isMutating` / `.gitignore` conflicts) + merge #106, OR start the deferred glide. Confirm scope first.
...
#95 → `f729e6c`). **Working branch `claude/ui-polish-2` = `92682e1`** (2 ahead of master,
**pushed** — branch from it). **PR #96** (`claude/fix-pal14-flake`) is **OPEN into master** —
the flaky-test fix, awaiting the user's merge. Items 1/2/4 are **committed + verified but NOT
PR'd/merged** (ready to PR, master-gate OK pending). Baseline on `claude/ui-polish-2`: web
**515/0**, `tsc -b` 0, native harness **174/0**, host Debug x64 clean. Fresh worktree needs
L-039 (NuGet copy) + L-046 (MSBuild **VS18** Debug x64) + L-040 (`pnpm build`).

### Part A — session-26 carryover landed
- **Merged PR #94** (session-26 UI-polish batch + the `xtree:181` link-group crash fix) →
  `master` `1239ff9`, after waiting for all 3 CI legs green.
- **Backfilled** the two CHANGELOG `TODO` merge-hashes → PR #95 (`f729e6c`).
- **Fixed a flaky test** → **PR #96 (OPEN)**: `TexturePalettePopover.test.tsx` PAL-14 cases
  read `data-thumb-status` synchronously after `findByTestId`, racing the async
  `textures/palette/thumbnail` bridge resolve (`useEffect`→`setThumb`). Caught it failing a
  CHANGELOG-only PR; wrapped both assertions in `waitFor`. 514→514, re-ran 3×.
  **NOTE: this fix is on `claude/fix-pal14-flake` (#96), NOT on `claude/ui-polish-2`** — so
  that branch still carries the master (unfixed) test; rare flake possible until #96 merges
  and propagates.

### Part B — UI-polish round 2 (items 1/2/4) — committed `6307ba5`, user-confirmed in host
4 user requests; **#3 became Item 3** (its own effort, Part C). Items 1/2/4:
- **Item 1 — selected curve key** (`CurveEditor.tsx`, `components.css`, `CurveEditor.test.tsx`).
  Dropped the flat-blue selection override → selected keys keep their OWN colour intensified
  via `filter: saturate(1.8)`, enlarged (`r` 4→6 single / `visR` 5→8 multi), stronger shadow;
  **unselected keys now have NO shadow**; select/deselect **ANIMATES** (`transition: r 150ms,
  filter 150ms` — Chromium transitions the SVG `r` attribute, no JS lib).
- **Item 4 — popover anims** (`OccludingPopover.tsx`, `components.css`). Background/Ground/
  texture-picker popovers fade in/out, centralised in the shared wrapper. **Hand-written
  `@keyframes`** keyed off Radix `data-state` (the `animate-in` Tailwind utilities are NOT in
  this build — no `tailwindcss-animate`). **Opacity-only, deliberately**: a corner-anchored
  scale coupled the popover's position to its animation state (the "shift on press" the user
  reported — *measured* via real-input Playwright: rest stuck at `scale(0.95)`, press snapped
  to full width). Also `.tb-btn[data-state="open"]:active { transform: none }` so the trigger
  doesn't scale (move the anchor) on the dismiss-press.
- **Item 2 — instant modals** (`AlphaCompositor::CaptureSnapshotPng`). The frosted-glass
  backdrop snapshot is **downscaled before PNG-encode** (`min 2×, cap 1024px long edge`) — it's
  blurred behind the dialog, so full-res was pure latency. **Windowed ~18ms (was ~50),
  maximized ~69ms** (measured via the `[INSTANT-MODAL]` debug log). Covers EVERY modal (all use
  the shared `Modal`). Native snapshot-dim tests re-baselined to the 2× dims. `CaptureSnapshotToFile`
  (`--capture`) left full-res. **ROADMAP [NT-10]** tracks the residual maximized latency
  (StretchRect-before-readback the likely win).

### Part C — Item 3 (dock-slide viewport stutter) — READY TO BUILD (`92682e1`)
**Spec:** [`docs/superpowers/specs/2026-06-08-item3-viewport-stutter-design.md`](../docs/superpowers/specs/2026-06-08-item3-viewport-stutter-design.md) (rev 2, red-team folded in).
**Two workflow outputs (rich detail):** `tasks/wa0o0il6r.output` (9-agent root-cause
investigation), `tasks/w6v635b1y.output` (6-agent spec red-team).
- **Root cause (verified):** *multi-clock sampling.* The uncapped, vsync-less host render loop
  samples a **clumpy, gap-ridden** `scene-rect` message stream at irregular Δt → the viewport's
  cropped edge advances in irregular integer steps against the browser-smooth panel. **NOT** a
  1-frame clip lag (the investigation's adversarial pass proved engine render + DComp clip are
  *co-applied* per host iteration — corrected the initial synthesis).
- **Phase 0 DONE (diagnosis confirmed):** `[STUTTER-PROBE]` at `BridgeDispatcher.cpp:919-938`
  (`#ifndef NDEBUG`, **TEMPORARY — remove before the fix lands**). 17 user slides → **16–30
  distinct-integer rects/slide**, Δt **mean 13ms / stdev 20ms / gaps to 109ms**, width jumps up
  to **~69px** after gaps. Width-only (h const), `from/to 658↔918` at the test window
  (grow=dock-close, shrink=dock-open). The stream is irregular *already on the emit side* →
  validates time-interpolation over the rejected rAF-coalesce.
- **Design = host-side time-interpolated viewport rect.** Web sends ONE
  `animate-scene-rect {from,to,durationMs,easing,msElapsedAtSend}`; host re-renders the engine
  at a wall-clock-lerped rect every frame (matched cubic-bezier, back-dated QPC clock). Chosen
  over DComp-scale (#4 — distorts geometry: per-pixel FoV ⇒ squished slice + end-snap).
- **Red-team corrections folded into the spec (do NOT skip):** the "synchronous flexGrow read"
  for `to` is **impossible** (react-resizable-panels emits on the next React commit) → compute
  `to` **analytically**; add a **`prefers-reduced-motion`** branch (else a 200ms seam every
  toggle); add the **`dockAnimating` zustand signal channel** + **RO-only** send suppression
  (the single shared `send` closure also serves scroll/resize/DPR — don't gate all of them);
  **host self-defense** (ignore stray `scene-rect`s while animating); **web-stamped start phase**
  (the IPC hop is ~16–33ms, not "sub-ms"); **gate the anim to arch-C** (no-op under `--legacy`).
- **NEXT (the build), per spec §3/§5:** Phase 1 = web first (the store + RO-only suppression +
  analytic `to` + reduced-motion + the new message), then host (`ViewportAnim` + back-dated
  `PerfQpcNow()` clock + render-loop advance + bezier + self-defense, arch-C-gated), then **live-
  tune with the user** (L-033), then the regression gate (web 515/0, native 174/0, drag-resize +
  rapid re-toggle = zero clear-strip frames, `--legacy` smoke, **remove the probe**). Spec
  Open-questions: confirm arch-C gating (recommended) + analytic-`to` (recommended).

### Verified baseline (run before changing anything)
- Work is on **`claude/ui-polish-2`** (pushed, `92682e1`); `git fetch origin claude/ui-polish-2`
  and **branch from it** (it has items 1/2/4 + the Item-3 spec/probe).
- web: from `web/`, `pnpm install` if `node_modules` absent (install from **`web/`**, never
  per-app); `pnpm --filter @particle-editor/editor test` → **515**; **`tsc -b`** (NOT `--noEmit`
  — L-070) → 0.
- native: fresh worktree needs L-039 + L-046 (**VS18**) + L-040. After any web change `pnpm
  build` before the harness (L-068). `pnpm --filter @particle-editor/editor test:native` →
  **174/0** (exit 1/2 + "browser closed" can be an L-066/L-071 phantom — re-run).
- host: MSBuild **VS18** Debug x64 (`C:\Program Files\Microsoft Visual Studio\18\Community\…`);
  **kill any running `ParticleEditor.exe` before the link** (it locks the exe). Launch
  `x64\Debug\ParticleEditor.exe` (new-UI default; `ComputeEditorDistPath` is **exe-relative**
  — `HostWindow.cpp:265-271` — so cwd doesn't matter, but launch from the worktree root anyway).

### Lessons / gotchas this session
- **L-022 reinforced twice:** the investigation's "1-frame clip lag" AND the spec's "synchronous
  flexGrow read" were both plausible-but-FALSE; the adversarial workflow passes caught both
  before any wasted code. Trust *verified* code, not the first synthesis.
- **L-033 extends to timing:** the Claude **preview tab backgrounds → throttles CSS animations/
  rAF** (it froze the popover entrance at `scale(0.95)`, which by accident *revealed* the
  coupling bug). Real-input **Playwright (foreground)** measures genuine `:active` + animation
  timing; use measured **geometry / CSSOM**, not screenshots, in both harnesses.
- PowerShell 5.1 has **no ternary `? :`** (parser error — use `if/else`). `Start-Process
  -RedirectStandardError <file>` captures the GUI host's `fprintf(stderr,...)` perf logs.

### Kickoff (full) — paste into a fresh session
> Pick up `new-particle-editor` (AloParticleEditor — Win32 host + WebView2/React + D3D9Ex-via-
> DComp particle editor for SW:EaW). **`master` is the trunk** (new-UI is the x64 DEFAULT,
> `--legacy` opts out; `lt-4` retired). The active task is **building Item 3 — the dock-slide
> viewport stutter** — which is fully investigated, designed, adversarially reviewed, and
> Phase-0-confirmed; only the implementation remains. **Pre-flight:** `git fetch origin
> claude/ui-polish-2` and branch from it (`92682e1` — it has UI-polish items 1/2/4 + the Item-3
> spec/probe, 2 ahead of master, pushed). From `web/` `pnpm install` if needed (from `web/`, not
> per-app), then `pnpm --filter @particle-editor/editor test` → **515**, `tsc -b` 0; native
> **174/0** (fresh worktree needs L-039 NuGet copy + L-046 MSBuild **VS18** + L-040 `pnpm build`).
> **Read first:** `tasks/HANDOFF.md` top (session 27) and the design spec
> `docs/superpowers/specs/2026-06-08-item3-viewport-stutter-design.md` (rev 2). The rich
> investigation + red-team detail is in `tasks/wa0o0il6r.output` + `tasks/w6v635b1y.output`. Read
> `tasks/lessons.md` (esp. **L-022** verify code over claims — TWO load-bearing premises were
> proven false this session; **L-033** arch-C visuals + animation TIMING need the user's eye /
> real-input Playwright, not agent screenshots or the throttled preview tab; **L-070** `tsc -b`
> gate; **L-068** `pnpm build` before native; **L-066/L-071** native phantom re-run; **L-046**
> MSBuild VS18; **L-039/L-040** fresh-worktree restore). **VERIFY claims against actual code.**
> Item 3 = a host-side **time-interpolated viewport rect** (host re-renders at a wall-clock-lerped
> rect each frame, synced to the CSS curve) — chosen over DComp-scale (#4, distorts geometry).
> Root cause = multi-clock sampling of a clumpy `scene-rect` stream (NOT a 1-frame clip lag — the
> spec explains). Build per spec §3: **web side first** (a `dockAnimating` zustand signal channel
> + **RO-only** send suppression + **analytic** `to`-width + a **`prefers-reduced-motion`** branch
> + the new `animate-scene-rect` message), then **host side** (`ViewportAnim` + back-dated
> `PerfQpcNow()` clock + render-loop advance + a matched cubic-bezier + stray-`scene-rect`
> self-defense, **gated to arch-C**), then **live-tune the easing/phase with the user** (the
> viewport edge must glide with the panel — your render can't judge it). The `[STUTTER-PROBE]` at
> `BridgeDispatcher.cpp:919-938` is `#ifndef NDEBUG` and **TEMPORARY — remove before sign-off**.
> Also OPEN: **PR #96** (flaky PAL-14 fix) awaits the user's merge; items **1/2/4** are committed
> on `claude/ui-polish-2` but NOT yet PR'd (master-gate OK pending). MT-13 (legacy removal) is
> greenlit but separate. Summarize your understanding + confirm scope (esp. the spec's two
> open-questions: arch-C gating + analytic-`to`) before changing anything.

### Kickoff (short)
> Pick up AloParticleEditor. **`master` = trunk; new-UI x64 default.** Task: **build Item 3
> (dock-slide viewport stutter)** — investigated/designed/red-teamed/Phase-0-done, only the build
> remains. Pre-flight: `git fetch origin claude/ui-polish-2` + branch from it (`92682e1`, has
> items 1/2/4 + the spec); `pnpm --filter @particle-editor/editor test` → **515**, `tsc -b` 0;
> native **174/0** (fresh worktree: L-039/L-046 **VS18**/L-040; install pnpm from `web/`). Read
> `tasks/HANDOFF.md` top (session 27) + `docs/superpowers/specs/2026-06-08-item3-viewport-stutter-design.md`
> (rev 2) + lessons **L-022/L-033/L-070/L-068/L-046**. VERIFY against code. Item 3 = host-side
> **time-interpolated viewport rect** (web `animate-scene-rect` → host re-renders at a wall-clock
> bezier-lerped rect each frame, arch-C-gated); root cause = multi-clock sampling of a clumpy
> `scene-rect` stream. Build web-side first (zustand `dockAnimating` + RO-only suppression +
> analytic `to` + reduced-motion), then host (`ViewportAnim` + back-dated `PerfQpcNow()` +
> render-loop advance + matched bezier + self-defense), then **live-tune with the user** (L-033).
> Remove the `#ifndef NDEBUG` `[STUTTER-PROBE]` before sign-off. PR #96 + items 1/2/4 await
> user merge-OK. Confirm scope before changing anything.
