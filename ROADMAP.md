...
### 5.1 [MT-3] ~~Selectable skydome backgrounds~~ ✅ Shipped (#73)

A new **Skydome** toolbar preview button (24×24, just left of the existing *Ground Texture:* preview) opens a modeless **Skydome** picker dialog — a 12-slot icon-mode `SysListView32` laid out as a 4×3 grid of 192×192 thumbnails. Slot 0 is **Off** (preserves the pre-MT-3 behaviour — flat-colour clear with the existing `Background:` button driving the colour); slots 1–8 are bundled scenes (Space / Atmosphere / Sunset / Dawn / Night / Overcast / Studio / Indoor); slots 9–11 are user-customisable.

**Interactions** match MT-2's ground-texture picker exactly so the muscle memory carries: single-click any populated slot to swap live; single-click an empty Custom slot to open `GetOpenFileName` filtered to `*.dds;*.tga` (EaW's native texture formats — point at game environment textures directly with zero conversion); right-click a Custom slot for *Set custom skydome…* / *Change skydome…* / *Clear slot*; the dialog's *Reset custom slots* button at the bottom wipes only the user-supplied paths after a confirmation prompt. The Esc key / title-bar X hides the dialog (modeless, sticky, position persisted). View → Reset View Settings resets the active slot to Off and clears `SkydomePickerPos` but deliberately preserves the three `SkydomeCustomSlot*` registry values (same "slot assignments are user data, not view settings" convention as MT-2).
...
**Persistence**: 5 new `HKCU\Software\AloParticleEditor` values — `SkydomeIndex` (DWORD, slot 0..11), `SkydomeCustomSlot{9,10,11}` (REG_SZ, custom paths), and `SkydomePickerPos` (REG_BINARY RECT). Bundled defaults restore on a fresh install; an out-of-range or missing-file path falls back to Off rather than crashing.

**Assets**: ships with **procedural-gradient TGA placeholders** generated by [`tools/generate_skydome_textures.py`](tools/generate_skydome_textures.py) at 1024×512 each — simple top-to-bottom colour ramps approximating each scene. ~12 MB of `RCDATA` in the .exe. Production-quality BC1-compressed DDS assets (potentially curated from game art) are a separate follow-up PR; the engine loader (`D3DXCreateTextureFromFileInMemory`) handles both formats identically so swap-in is a content-only change.

**Win11 GUI quirk avoided**: the spinner read-only rendering issue from MT-4 doesn't recur here — the picker uses the same ListView + WM_PAINT-subclass pattern MT-2 established (where MT-2 had to escape the native ListView's selection-chrome leakage; the skydome picker reuses that subclass verbatim so the 12 cells render cleanly without native blue selection borders bleeding through).

- **Difficulty**: ★★★★☆ (4/5)
- **Estimated effort**: 8–14 hours
- **Actual**: ~7 hours across 16 commits, executed via the `subagent-driven-development` skill (one implementer subagent + spec-compliance reviewer + code-quality reviewer per task, with one or two fix loops per task where reviewers flagged Important issues — `HRESULT` checks in `InitSkydomeMesh`, magic-number array size for `m_skydomeCustomSlotPaths`, unused-variable in `RenderSkydome`'s effect-pass loop, comment correctness on `RCDATA` cubemap-vs-equirectangular and on the theta direction in the sphere parameterisation).
