...
*No near-term items currently. The [NT-12] app-wide tooltip pass shipped
2026-06-10 — see the [NT-12] entry in Shipped §5.*
...
> The next new medium-term tag is **MT-18** (max+1 over every `[MT-K]` ever
> used). Shipped/retired: `[MT-11]`/`[MT-12]` (arch-C, stats-freeze — §5),
> `[MT-14]` (ground lighting — §5), `[MT-15]` (game/mod skydome — §5),
> `[MT-16]` (transparency parity — §5), `[MT-17]` (spawner path — §5).
> **`[MT-13]` is reserved** for the greenlit legacy/arch-A removal (see
> `project_daily_driving_legacy` + the `MT-13` references in `CLAUDE.md`) —
> not yet written up here.

*No open medium-term items — `[MT-15]` (the last one) shipped, see §5.1. New
medium-term work takes `[MT-18]`.*
...
`reference/foc-shaders/`. Carries the **colour-grade / tone** half of the [MT-16] report; the
transparency half shipped separately as an editor-compositing fix with no
shader edit (#155). Likely also retires the other editor shader forks (`Skydome.fx`,
...
### 5.1 [MT-15] ~~Skydome background from the game / mod (primary + secondary)~~ ✅ Shipped (#163)

*Estimate: ★★★★ (4/5), 12–20 hours.*
*Actual: re-tiered to ★★★★★ and ~five landable steps across sessions 42–44.
Sessions 42–43 built the device-free leaf cores (`AloModel` + `SkydomeEnvironment`),
a Step-0 device spike, the render core (`SkydomeMesh` + `Engine::RenderSkydomes`),
the nebula multi-submesh compose, and the dual-slot bridge + persistence + React
two-section picker (#160, #163). The #163 render shipped **unverified**; a
session-44 render-capture feel-test against a real FoC install found and fixed two
render-blockers — a `SubFile::read` over-read (MEG-packed XML parsed to garbage →
zero domes enumerated) and a missing `.tga`→`.dds` texture fallback (every dome
texture missed → black dome) — before the dome rendered faithfully.*

The Background picker now offers the **real** in-game/in-mod skydomes — enumerated by
their actual GameObject `Name` from the game's `{Land,Space}{Primary,Secondary}Skydomes.xml`
— for two independent slots (primary + secondary, Land/Space context), and the editor
renders them faithfully: it loads the genuine `.alo` mesh and runs **each sub-mesh's own
named game shader** (`Skydome.fx`, `MeshGloss.fx`, `MeshAdditive.fx`, …) straight from the
game's `shaders.meg` — no fork or approximation — with the dome's authored material
params/textures, real spherical-harmonic lighting, and cloud scroll off the engine clock;
it survives device-reset / mod-switch and persists across restarts. The solid-colour +
custom-path simple background remains as a fallback. A debug-gated `debug/capture-frame`
bridge (added with the render fixes) writes the viewport to a PNG so render work is
self-verifiable. See the CHANGELOG (the #160 foundation, the #163 render core, and the
session-44 render-fix entries) for the implementation trail.

### 5.2 [MT-16] ~~Editor ↔ in-game transparency parity~~ ✅ Shipped (#155)

*Estimate: ★★★ (3/5), 4–8 hours triage + fix.*
*Actual: triage shipped in session 40 (#152, no code); the transparency-half
fix this session was ~4 lines of render state + verification. The "darker
in-game" report split into two independent causes — alpha-channel erosion
(this fix) and the game's missing scene colour-grade. Only the transparency
half is MT-16; the tone half is genuinely separate work, tracked as [LT-8].*

Particle transparency now reads the same in the editor preview as in-game —
no more washed-out / see-through look on transparent and modulate blends. The
new-UI viewport is a per-pixel-alpha layered/DComp surface, so the scene
render target's alpha is meaningful (it's the viewport's opacity); the
particle path, written for a game backbuffer where alpha is ignored, eroded
that alpha and `SceneHeat.fx` copied it straight through, so the compositor
showed the eroded alpha as transparency. The fix masks alpha writes
(`D3DRS_COLORWRITEENABLE = RGB`) around the final scene-combine draw in
`Engine::Render` so the RT keeps the opaque alpha it was cleared to — the
editor now presents the engine's RGB at full opacity, exactly like a game
backbuffer, blend-mode-agnostic, across both the arch-C DComp (default) and
arch-B layered paths. **No shader edit** (the no-fork render-parity rule). The
tone / colour-grade half of the original report — the game's per-map
`Scene_*.fx` grade, the dominant contributor to overall "darker in-game" — is
**not** here; it requires recreating the game's scene composite and is tracked
as [LT-8] (depends on [MT-15]).

### 5.3 [MT-14] ~~Ground plane lighting — does it respond to the lighting setting?~~ ✅ Shipped (#151)

*Estimate: ★★★ (3/5), 3–6 hours (as the contained fix the triage expected).*
*Actual: a full session. Triage took minutes — the ground was simply
**excluded** from scene lighting (drawn fixed-function with `D3DRS_LIGHTING`
off + a hardcoded white diffuse); world lighting was never broken. But by user
direction the "fix" became a game-faithful **bump-mapped terrain** render — a
net-new bundled shader + effect pipeline + runtime normal-map resolution —
landing closer to a medium-term feature than the near-term slot it was filed
under. Re-tier future references accordingly.*

The ground plane now renders with the game's bump-mapped terrain lighting and
responds to the Lighting panel (sun / fill / ambient). New bundled effect
[`GroundLit.fx`](src/Resources/Engine/GroundLit.fx) — a **per-pixel world-space**
adaptation of the game's `TerrainRenderBump` path: SPH fill per-vertex, dot3 sun
off a tangent-space normal map, gloss-gated specular with the **gloss in the
`_bc` map's alpha**, diffuse ×2 / specular ×1 (cloud/FOW dropped, no editor
equivalent). Driven by `Engine::RenderGroundLit()` + `m_pGroundEffect`,
mirroring the skydome effect lifecycle, with a compile-failure graceful-degrade
to the original unlit quad. Normal maps resolve `<base>_bc.dds` from the
game/mod via `FileManager` (grass/sand/snow + custom-slot companion), matte
flat-normal fallback otherwise. Petroglyph FoC shader source vendored under
`reference/foc-shaders/`. *(Per-pixel-on-a-flat-quad is an adaptation, not the
game's literal per-vertex terrain shader — see the 1:1-rendering note in
`CHANGELOG.md`.)*

### 5.4 [MT-17] ~~Spawner jitter should perturb the path, not exit velocity~~ ✅ Shipped (#149)

*Estimate: ★★★ (3/5), ~4-7 hours.*
*Actual: ~3 hours (locked three design forks up front — remove velocity
jitter, sinusoidal squiggle, acceleration Vec3 — then a clean fan-out
through engine → bridge → UI; the analytic closed form fell out of the
EmitterInstance velocity-inheritance constraint).*

The spawner's old one-time **velocity jitter** (each instance flew off on
a random straight line) is gone, replaced by **per-instance path shaping**
over the instance lifetime: a deterministic **arc** (`acceleration` Vec3,
gravity-like) plus a smooth **squiggle** (`squiggleAmplitude` Vec3 +
`squiggleFrequency` Hz, with a per-instance random phase per axis so burst
siblings diverge organically). The emit point traces the shaped path, so
the particle trail arcs/squiggles. Absorbs the deterministic arc-path idea
from the retired [LT-1]. Position jitter (spawn-point scatter) is
unchanged. Motion is computed analytically from elapsed time — exact arc,
frame-rate independent — with the squiggle zeroed at τ=0 (`−sin(φ)` term)
so instances still emanate from the spawn point, and the instantaneous
velocity written back each tick so emitted particles inherit it correctly.
React-UI-only; the legacy Win32 dialog keeps spawn-point jitter and drops
the velocity column (legacy is `--legacy` opt-out, slated for [MT-13]).

### 5.5 [NT-12] ~~Styled, animated tooltips (app-wide tooltip pass)~~ ✅ Shipped (#123)

*Estimate: ★★ (2/5), ~3-5 hours.*
*Actual: ~5 hours (visual brainstorm of the motion language + 13-task
subagent-driven build; the sweep itself was mechanical, the time went to
the primitive's occlusion/Radix edge cases and two flaky a11y goldens).*

One shared `Tip` primitive (`@radix-ui/react-tooltip`) replaced every
native `title` hover hint (~45 sites): theme-following surface, soft
`--shadow-soft` drop shadow, fade + 4px directional slip, 400ms delay with
sweep skip-delay, opt-in viewport occlusion. The NT-11 ⚠ glyph got its
rich "what this means" tooltip (amber band + per-generation breakdown).
Shipped together with the greenlit modal/banner entrance-exit animations
as one motion family (`--motion-*`/`--slip-*` tokens; Modal keyframes
replace the dead `animate-in` utilities; OverloadBanner exits via the new
`usePresence` shim).

### 5.6 [NT-11] ~~Soft warning when an emitter chain's particle multiplication explodes~~ ✅ Shipped (#120)
...
### 5.7 [NT-10] ~~Further reduce maximized save-modal backdrop snapshot latency~~ ✅ Shipped (#101)
...
### 5.8 [MT-12] ~~Flip default to architecture C + retire env-var dual-toggle~~ ✅ Shipped (#92)
...
### 5.9 [MT-11] ~~Migrate engine rendering to DOM `<canvas>` (architecture C)~~ ✅ Shipped (#92)
...
### 5.10 [NT-8] ~~Resizable splitters for left / centre / right column boundaries~~ ✅ Shipped (#92)
...
### 5.11 [NT-9] ~~Frosted-glass modal backdrop via engine-snapshot capture~~ ✅ Shipped (#92)
...
### 5.12 [NT-7] ~~Inspector layout follow-ups — tabs always visible + tab strip height + emitter list flex-grow~~ ✅ Shipped (#92)
...
### 5.13 [NT-5] ~~Engine-side single-member link-group enforcement~~ ✅ Shipped (#92)
...
### 5.14 [NT-6] ~~Visual-stability lane assignment for bracket gutter~~ ✅ Shipped (#92)
...
### 5.15 [LT-3] ~~Import emitters from other particle files~~ ✅ Shipped (#77)
...
### 5.16 [MT-3] ~~Selectable skydome backgrounds via the unified Background button~~ ✅ Shipped (#73)
...
### 5.17 [MT-4] ~~Adjustable environment lighting in the preview~~ ✅ Shipped (#71)
...
### 5.18 [MT-1] ~~Frequently-used textures palette~~ ✅ Shipped (#69)
...
### 5.19 [MT-2] ~~Selectable ground texture~~ ✅ Shipped (#67)
...
### 5.20 [MT-10] ~~Configurable exempt set per link group~~ ✅ Shipped (#65)
...
### 5.21 [MT-9] ~~Visual link-group bracket for linked emitters~~ ✅ Shipped (#63)
...
### 5.22 [MT-8] ~~Multi-select for the emitter list~~ ✅ Shipped (#60)
...
### 5.23 [MT-7] ~~Linked emitters (share parameters across a group)~~ ✅ Shipped (#58)
...
### 5.24 [NT-4] ~~Duplicate with index increment~~ ✅ Shipped (#56)
...
### 5.25 [NT-3] ~~Pause / frame-step the preview~~ ✅ Shipped (#53)
...
### 5.26 [MT-5] ~~Confirm / extend two-child emitter support~~ ✅ Shipped (#51)
...
### 5.27 [MT-6] ~~Bloom in the preview renderer~~ ✅ Shipped (#47)
...
### 5.28 [NT-2] ~~Adjustable ground-plane height in the preview~~ ✅ Shipped (#45)
...
### 5.29 ~~Autosave for in-progress particles~~ ✅ Shipped (#41)
...
### 5.30 ~~Drag-and-drop to reparent (make an emitter a child of another)~~ ✅ Shipped (#37)
...
### 5.31 ~~Drag-and-drop reordering in the emitter tree~~ ✅ Shipped (#35)
...
### 5.32 ~~Programmable particle spawner for the preview (v1)~~ ✅ Shipped (#30)
...
### 5.33 ~~Buttons to reorder emitters~~ ✅ Shipped (#25)
...
### 5.34 ~~Right-click → Duplicate Emitter~~ ✅ Shipped (#19)
...
### 5.35 ~~Scroll-wheel adjustment on numeric boxes~~ ✅ Shipped (#16)
