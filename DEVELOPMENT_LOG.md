# Particle Editor — Development Log

Per-PR engineering detail. For the public-facing version-grouped release history, see [`CHANGELOG.md`](CHANGELOG.md). For planned work, see [`ROADMAP.md`](ROADMAP.md).

This file is split into three parts:

1. **[Log](#log)** — per-PR change events in reverse chronological order, latest on top. Each entry carries a date, the merge-commit short hash on `master`, and (where applicable) the PR number.
