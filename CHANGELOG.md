...
*2026-05-15 · [`TODO`](https://github.com/DrKnickers/new-particle-editor/commit/TODO) · [#73](https://github.com/DrKnickers/new-particle-editor/pull/73)*
...
Interactions match MT-2's ground-texture picker so the muscle memory carries: single-click any populated slot to swap live; single-click an empty Custom slot to open `GetOpenFileName` filtered to `*.dds;*.tga` (the two native EaW texture formats — point a custom slot directly at a game environment texture with zero conversion); right-click a Custom slot for *Set custom skydome…* / *Change skydome…* / *Clear slot*; the dialog's *Reset custom slots* button at the bottom wipes only the user-supplied paths after a confirmation prompt. The Esc key / title-bar X hides the dialog (modeless, sticky, position persisted). View → Reset View Settings resets the active slot to Off and clears `SkydomePickerPos` but deliberately preserves the three `SkydomeCustomSlot*` registry values (same "slot assignments are user data, not view settings" convention as MT-2). The Reset View Settings confirmation prompt was updated to mention skydome alongside the other view-state entries.

Render integration is a single new pass between the existing `D3DDevice9::Clear` and the ground-plane render: a hand-rolled 32×16 UV sphere (561 vertices / 1024 triangles, `D3DPOOL_MANAGED`) rendered with `Translation(camera.Position)` as the world matrix — the sphere stays "infinite" while the camera orbits the world origin where the particles live. Render state during the pass: depth-test off, depth-write off, cull-CW (we view the sphere from inside). A new shader at [`src/Resources/Engine/Skydome.fx`](src/Resources/Engine/Skydome.fx) (vs_2_0 / ps_2_0) does standard equirectangular sampling on the (U, V) the mesh carries and pushes z to ~1.0 in NDC for belt-and-suspenders far-plane behaviour. The skydome contributes to bloom naturally since it renders into the same scene RT as everything else.

Five new `HKCU\Software\AloParticleEditor` values hold the persisted state: `SkydomeIndex` (REG_DWORD, slot 0–11), `SkydomeCustomSlot{9,10,11}` (REG_SZ, per-slot paths), and `SkydomePickerPos` (REG_BINARY RECT). Out-of-range / missing-file values fall back to Off rather than crashing. On startup the editor restores custom paths first (so a previously-active custom slot can reload), then the active index, then rebuilds the toolbar preview thumbnail.
...
**How we tackled it.** Executed via the `subagent-driven-development` skill — one implementer subagent + two reviewer subagents per task, with fix loops where reviewers flagged Important issues. Eleven implementation commits + a couple of fix commits. Engine-side: new `m_pSkydomeVB` / `m_pSkydomeIB` / `m_pSkydomeDecl` (Task 1) + `m_pSkydomeEffect` / `m_pSkydomeTexture` / `m_skydomeIndex` / `m_skydomeCustomSlotPaths[]` (Task 3) live in [`src/engine.h`](src/engine.h) alongside the existing `m_pGroundTexture` family; `InitSkydomeMesh()` / `InitSkydomeEffect()` / `RenderSkydome()` / `SetSkydomeSlot()` etc. live in [`src/engine.cpp`](src/engine.cpp). UI-side: [`src/main.cpp`](src/main.cpp) gains ~1000 LOC modelled directly on MT-2's ground-picker shape — `MakeSkydomeSlotThumbnail`, `RebuildSkydomePreviewBitmap`, `SkydomePickerProc`, `ShowSkydomePicker`, registry I/O helpers, toolbar button creation in `WM_CREATE` and positioning in `WM_SIZE`, `IsDialogMessage` chain extension, `WM_DRAWITEM` for the new owner-drawn preview button. The picker's custom-paint `ListView` subclass that escapes native selection-chrome leakage is the same pattern MT-2 established; reused verbatim. Resource layout: `IDR_SKYDOME_*` (151–158) for the 8 bundled TGAs and `IDR_SHADER_SKYDOME` (150) for the shader live in [`src/Resources/resource.h`](src/Resources/resource.h) (locale-agnostic `RCDATA` IDs); the dialog template `IDD_SKYDOME_PICKER`, control IDs, and slot-name string-table entries live in `resource.en.h` / `resource.de.h` + the `.en.rc` / `.de.rc` pair (locale-specific UI strings, English placeholder strings in the German variant per the existing convention).

**Issues encountered and resolutions.** Six worth recording.
