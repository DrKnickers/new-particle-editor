...
// `=` and `\n` would corrupt the line; control chars can confuse
// WritePrivateProfileString. NULs already terminate any C string.
bool FilenameOk(const wstring& f)
{
    if (f.empty()) return false;
    for (wchar_t c : f)
    {
        if (c < 0x20) return false;       // control chars + NUL
        if (c == L'=' || c == L'\r' || c == L'\n') return false;
    }
    return true;
}

// QueryPerformanceCounter wrapper returning ticks as uint64. We don't
// need wall-clock time — recents only need a monotonically-increasing
// "touched" stamp for LRU sort.
uint64_t Now()
{
    LARGE_INTEGER li;
...
// Get %APPDATA% — typical "C:\Users\<name>\AppData\Roaming". Fails
// silently on the rare cases where the API can't resolve it; caller
// gets an empty string and skips persistence.
wstring AppDataDir()
{
...
// Read a string value from INI. Returns the default if missing /
// can't be parsed. INI strings can be up to 32k via this helper;
// our values (filenames, integers) are well under that.
...
// Erase a whole section (all key=value lines). Pass NULL as the value
// argument — that's the documented way to delete a section.
...
Store::~Store()
{
    // Best-effort flush of any in-memory mods that haven't been
    // written through their per-mutation hooks. Currently every
    // mutation writes immediately, so this is a no-op safeguard.
}
...
wstring Store::IniPath() const
{
    ...
    // Create the dir on demand. If it already exists the call returns
    // ERROR_ALREADY_EXISTS — fine. Any other failure leaves the path
    // pointing at a directory that doesn't exist; subsequent
    // GetPrivateProfileString calls just return defaults.
...
wstring Store::SectionFor(const wstring& modPath) const
{
    ...
    // CRC32 of the path bytes. Convert to UTF-8 first so the same
    // path produces the same hash regardless of wchar_t width.
...
{
    if (modPath.empty()) { ClearActiveMod(); return; }
    ...
    {
        // Same mod (case-insensitive) — nothing to do. Update the
        // stored display path in case casing changed.
        m_activeMod = modPath;
        return;
    }
    ...
    // Touch the new mod so its INI section is loaded and the cache
    // is warm for the next read.
    LoadOrInit(modPath);
...
Store::ModPalette& Store::LoadOrInit(const wstring& modPath)
{
    ...
    if (!iniPath.empty())
    {
        ...
        for (int i = 0; i < pinCount; ++i)
        {
            ...
            e.isPinned   = true;
            e.slotMask   = m;
            e.lastUsedNs = 0;  // pins don't sort by last-used
            mp.entries.push_back(e);
        }

        // Recents (most-recent first in INI; we re-sort by lastUsedNs
        // anyway so the on-disk order is just for human readability).
        ...
        for (int i = 0; i < recentCount; ++i)
        {
            ...
            wstring v = IniGetString(iniPath, section, key2, L"");
            // Format: "<filename>|<slotmask>|<unix-ish-ts>"
            ...
            e.isPinned   = false;
            e.slotMask   = m;
            // Synthesize a lastUsedNs so the loaded order is preserved
            // — earliest read = most recent. The literal timestamp
            // string after pipe2 is currently informational only.
...
void Store::TouchRecent(const wstring& filename, SlotMask usedAs)
{
    ...
    // If the entry already exists (pinned or recent), update its
    // slotMask + lastUsedNs in place.
    ...
    // New entry. Insert as a recent. If we're at MAX_RECENTS for any
    // slot, evict the oldest recent matching that slot to keep the
    // per-filter cap honest (the spec specifies the cap is per filter).
    Entry e;
    e.filename   = filename;
    e.isPinned   = false;
    e.slotMask   = (uint8_t)usedAs;
    ...
    // Count current recents matching this slot. If at cap, evict
    // the oldest one (smallest lastUsedNs).
    ...
    {
        // Find oldest recent matching this slot and remove it.
...
bool Store::TogglePin(const wstring& filename)
{
    ...
    for (Entry& e : mp.entries)
    {
        if (e.filename == filename)
        {
            if (e.isPinned)
            {
                // Un-pin: becomes a recent at "now" so it sorts to
                // the front of the recents row.
                e.isPinned   = false;
                e.lastUsedNs = Now();
                FlushMod(m_activeMod, mp);
            ...
            {
                // Pin: count existing pins (per-filter would be more
                // strict, but the spec's cap is "8 pinned per filter"
                // which is the same as "8 pinned matching the entry's
                // slotMask"). For simplicity and since pins are rare,
                // we cap the total pins at MAX_PINS irrespective of
                // slot — the practical user experience is identical.
                size_t pinCount = 0;
                for (const Entry& cand : mp.entries)
                {
    ...
    // Not found — TogglePin doesn't create entries; only TouchRecent does.
    return false;
}
...
vector<Entry> Store::Recents(SlotMask filter) const
{
    ...
    // Sort by lastUsedNs descending — most recent first.
...
void Store::FlushMod(const wstring& modPath, const ModPalette& mp) const
{
    ...
    // Erase the section first so removed entries don't linger.
    ...
    // Recents go in lastUsedNs-descending order so the on-disk file
    // is human-debuggable.
    ...
    for (const Entry* e : recentSorted)
    {
        ...
        // Format: "<filename>|<slotmask>|<lastUsedNs>"
